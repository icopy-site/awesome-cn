...
- [FEniCS](https://fenicsproject.org) -用于使用Python和C ++解决PDE的计算平台.
...
- [HDF5](https://support.hdfgroup.org/HDF5/) -用于存储和管理数据的数据模型，库和文件格式.
  (C/Fortran, BSD)
- [XDMF](http://www.xdmf.org/index.php/Main_Page) -高性能计算代码中数据的可扩展数据模型和格式.
...
- [Qhull](http://www.qhull.org) -凸包，Delaunay三角剖分，Voronoi图，某个点的半空间交点等.
