...
有 [other sites with curated lists of elixir packages](#other-awesome-lists) 您可以看看.
...
* [sfmt](https://github.com/jj1bdx/sfmt-erlang/) -面向SIMD的用于Erlang的Fast Mersenne Twister（SFMT）.
...
* [tfidf](https://github.com/OCannings/tf-idf) -术语频率-文档频率的逆向执行.
...
* [Startup Job](https://github.com/tsurupin/job_search) -一个伞形项目，用于搜索从用Elixir / Phoenix和React / Redux编写的网站中抓取的启动工作.
...
* [ueberauth_facebook](https://github.com/ueberauth/ueberauth_Facebook) -Überauth的Facebook OAuth2策略.
...
* [memoize](https://github.com/melpon/memoize) -可以轻松缓存功能的备注宏.
...
* [ex_crypto](https://github.com/ntrepid8/ex_crypto)  -用于Erlang`crypto`和`public_key`模块的Elixir包装器.  为许多加密功能提供合理的默认值，以使其更易于使用.[Docs](https://hexdocs.pm/ex_crypto/readme.html)).
...
* [beaker](https://github.com/hahuang65/beaker) -Elixir的统计和指标库.
...
* [ex_iss](https://github.com/cryptobird/ex_iss) -该软件包用于与Open Notify API交互以获取诸如ISS的当前位置，人员，何时经过某个位置等信息.
...
* [magnetissimo](https://github.com/sergiotapia/magnetissimo) -索引所有流行的Torrent网站并将其保存到本地数据库的Web应用程序.
* [oop](https://github.com/wojtekmach/oop) -万灵药中的DOP！
* [phoenix-chat-example](https://github.com/dwyl/phoenix-chat-example)  -在Phoenix中为完整的初学者构建聊天应用的分步示例/教程.  涵盖测试，文档和部署.  凤凰1.4.
...
* [rubix](https://github.com/YellowApple/Rubix) -针对Elixir的非常简单（且功能不佳）的Ruby运行程序.
...
* [elli](https://github.com/knutin/elli) -Elli是一个网络服务器，您可以在Erlang应用程序中运行以公开HTTP API.
...
* [appsignal-elixir](https://github.com/appsignal/appsignal-elixir/) -从您的Elixir应用程序中收集错误和性能数据，并将其发送到 [AppSignal](https://appsignal.com/).
...
* [prometheus-plugs](https://github.com/deadtrickster/prometheus-plugs) -用于prometheus.ex的插件工具/出口商.
...
* [jsx](https://github.com/talentdeficit/jsx) -一个用于使用，产生和操作json的Erlang应用程序.
...
* [bunyan](https://github.com/bunyan-logger/bunyan) -Bunyan：药剂药剂师.
...
* [quiet_logger](https://github.com/Driftrock/quiet_logger/pull/1) -一个简单的插件，用于禁止运行状况检查日志记录（例如：使用Kubernetes时）.
...
* [youtrack_logger_backend](https://github.com/unifysell/youtrack_logger_backend) -记录器后端，它将消息发布到 [YouTrack](https://www.jetbrains.com/youtrack/) （JetBrains制造的问题跟踪器）.
...
* [kwfuns](https://github.com/RobertDober/lab42_defkw) -宏，用于创建具有基于语法的关键字参数和默认值的函数.
...
* [opus](https://github.com/zorbash/opus) -用于可插拔业务逻辑组件的框架.
...
* [exsync](https://github.com/falood/exsync) -另一个Elixir装载机.
...
* [Petrovich](https://github.com/petrovich/petrovich_elixir) -Elixir库可使用俄语的姓氏，名字和中间名进行变形.
...
* [mac](https://github.com/ephe-meral/mac) -可用于查找以十六进制字符串给出的MAC的供应商（根据IEEE）.
...
* [dexts](https://github.com/meh/dexts) -Disk Elixir术语存储，目标包装.
...
* [ecto_cassandra](https://github.com/cafebazaar/ecto-cassandra) -适用于Ecto的Cassandra数据库适配器.
...
* [ecto_validation_case](https://github.com/danielberkompas/ecto_validation_case)  -简化您的Ecto模型验证测试.  受到斗牛匹配者的启发，但更为简单.
...
* [eredis](https://github.com/wooga/eredis) -Erlang Redis客户.
...
* [extreme](https://github.com/exponentially/extreme) -使用的Elixir库 [Eventstore](https://geteventstore.com) 用于持续生成聚合事件（CQRS）.
* [exts](https://github.com/meh/exts) -Elixir条款存储和包装.
...
* [redix](https://github.com/whatyouhide/redix) -用于Elixir的超快，流水线，弹性Redis驱动程序.
...
* [immortal](https://github.com/danielberkompas/immortal) -不朽-一小撮帮助程序模块，旨在简化构建容错OTP应用程序的过程.
...
* [mqs](https://github.com/synrc/mqs) -RabbitMQ客户端库，路由密钥，基于MQ的RPC和其他内容.
...
* [relex](https://github.com/yrashk/relex) -Erlang / Elixir发布汇编程序.
...
* [eml](https://github.com/zambal/eml) -在Elixir中用于编写和操作（HTML）标记的库.
...
* [amrita](https://github.com/josephwilk/amrita) -针对Elixir的礼貌，礼貌和彻底的测试框架.
...
* [chemistry](https://github.com/genericlady/chemistry) -Elixir测试框架.
* [cobertura_cover](https://github.com/PSPDFKit-labs/cobertura_cover) -从与Jenkins的Cobertura插件兼容的`mix test --cover`文件中写入coverage.xml.
* [double](https://github.com/sonerdy/double) -创建存根依赖关系以进行测试，而不会覆盖全局模块.
...
* [fake_server](https://github.com/bernardolins/fake_server) -FakeServer是一个HTTP服务器，它可以模拟响应并使测试外部API更加容易.
...
* [airbrake](https://github.com/romul/airbrake-elixir) -空气制动的Elixir通知程序.
...
* [exgenius](https://github.com/jeffweiss/exgenius) -用于（未记录）Rap Genius API的Elixir库.
...
* [gringotts](https://github.com/aviabird/gringotts) -Elixir和Phoenix框架的完整付款库，类似于 [ActiveMerchant](https://github.com/activemerchant/active_merchant) 来自Ruby世界.
...
* [random_user](https://github.com/katgironpe/random_user) -针对randomuser.me API的Elixir客户端.
...
* [telephonist](https://github.com/danielberkompas/telephonist) -Twilio呼叫的Elixir状态机.
...
* [exkanji](https://github.com/ikeikeikeike/exkanji)  -用于在平假名，片假名，罗马字和汉字之间进行翻译的Elixir库.  它使用Mecab.
...
* [trans](https://github.com/belaustegui/trans) -一个Elixir库，用于利用PostgreSQL JSONB数据类型将嵌入式翻译转换为模型.
...
* [erlang-history](https://github.com/ferd/erlang-history) -黑客将Shell历史记录添加到Erlang的shell中.
...
* [is](https://github.com/bydooweedoo/is) -具有嵌套结构支持的Elixir快速，可扩展且易于使用的数据结构验证.
...
* [Adopting Elixir](https://pragprog.com/book/tvmelixir/adopting-elixir)  -根据建立Elixir并成功成功使用Elixir的人们的现实生活策略，将Elixir引入您的公司.  本书提供了将应用程序从概念应用到生产所需的全部信息（2017年）.
...
* [Elixir Cookbook](https://www.packtpub.com/application-development/elixir-cookbook) -这是Paulo A Pereira（2015）按主题分组的一组食谱.
...
* [elixir_generator](https://github.com/jadercorrea/elixir_generator.vim) -Vim插件，可使用一个命令生成Elixir模块并测试文件.
...
* [vscode-elixir-ls](https://github.com/JakeBecker/vscode-elixir-ls) -由ElixirLS支持的VS Code的Elixir语言支持和调试器.
...
* [ElixirWeekly](https://elixirweekly.net) -Elixir社区新闻通讯，涵盖了您容易错过的内容，并在 [ElixirStatus](http://elixirstatus.com) 和网络.
...
* [Awesome Erlang](https://github.com/drobakowski/awesome-erlang) -精选的Erlang库，资源和闪亮内容的精选列表.
...
* [Elixir Fountain](https://soundcloud.com/elixirfountain) -每周播客，播报由Elixir社区周围的新闻和访谈 [Johnny Winn](https://twitter.com/johnny_rugger).
