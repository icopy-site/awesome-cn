 ...
 **我们没有每月的费用** _，但我们有员工**努力工作**，以维持Awesome Go，有了筹集的资金，我们可以偿还每个参与人员的工作！ 所有的帐单和分配方式将向整个社区开放._
...
* [go-sox](https://github.com/krig/go-sox) -libsox绑定进行.
...
* [authboss](https://github.com/volatiletech/authboss)  -用于网络的模块化身份验证系统. 它尝试删除尽可能多的样板文件和“硬东西”，以便每次在Go中启动新的Web项目时，都可以插入，配置和开始构建应用程序，而不必每次都构建身份验证系统.
...
* [go-oauth2-server](https://github.com/RichardKnop/go-oauth2-server) -使用Golang编写的独立，符合规范的OAuth2服务器.
...
* [jwt](https://github.com/cristalhq/jwt) -安全，简单，快速的Go Go JSON Web令牌.
...
* [osin](https://github.com/openshift/osin) -OAuth2服务器库组.
...
* [micha](https://github.com/onrik/micha) -为电报bot API进入库.
...
* [box-cli-maker](https://github.com/Delta456/box-cli-maker) -为您的CLI制作高度自定义的框.
...
* [cfmt](https://github.com/mingrammer/cfmt) -受引导颜色类启发的内容相关的fmt.
...
* [configuro](https://github.com/sherifabdlnaby/configuro) -来自ENV和Files的有针对性的配置加载和验证框架，专注于12因子兼容应用程序.
...
* [go-up](https://github.com/ufoscout/go-up) -具有递归占位符解析度且没有魔术的简单配置库.
...
* [ini](https://github.com/go-ini/ini) -转到软件包以读取和写入INI文件.
* [joshbetz/config](https://github.com/joshbetz/config) -用于Go的小型配置库，可解析环境变量，JSON文件，并在SIGHUP上自动重新加载.
...
* [drone](https://github.com/drone/drone) -Drone是基于Go编写的基于Docker的持续集成平台.
...
* [bit](https://github.com/yourbasic/bit) -Golang设置的数据结构具有额外的位旋转功能.
...
* [hilbert](https://github.com/google/hilbert) -Go软件包，用于在空间填充曲线（例如希尔伯特曲线和Peano曲线）之间映射值.
...
* [levenshtein](https://github.com/agext/levenshtein) -Levenshtein距离和相似性指标，带有可自定义的编辑费用和通用前缀类似Winkler的奖金.
...
* [nan](https://github.com/kak-tus/nan) -零分配一个库中的可空结构，具有方便的转换功能，编组器和解组器.
...
* [slices](https://github.com/srfrog/slices)  -在切片上运行的功能； 类似于“包装字符串”，但适用于切片.
...
* [ttlcache](https://github.com/ReneKroon/ttlcache) -内存中的字符串接口{}缓存，具有各种基于时间的到期选项和回调.
...
* [immudb](https://github.com/codenotary/immudb) -immudb是用于Go语言编写的系统和应用程序的轻量级高速不可变数据库.
...
* [ttlcache](https://github.com/cheshir/ttlcache) -内存中键值存储，每个记录带有TTL.
...
* [pravasan](https://github.com/pravasan/pravasan) -简单迁移工具-目前适用于MySQL，但计划很快支持Postgres，SQLite，MongoDB等.
* [schema](https://github.com/adlio/schema) -用于在Go二进制文件中嵌入数据库/ sql兼容数据库的架构迁移的库.
...
* [xo](https://github.com/knq/xo) -根据现有架构定义或支持PostgreSQL，MySQL，SQLite，Oracle和Microsoft SQL Server的自定义查询为数据库生成惯用的Go代码.
...
*关系数据库
    ...
    * [pgx](https://github.com/jackc/pgx) -PostgreSQL驱动程序支持的功能超出了数据库/ sql所提供的功能.
...
* NoSQL数据库
    ...
    * [gocosmos](https://github.com/btnguyen2k/gocosmos) -REST客户端和Azure Cosmos DB的标准`database / sql`驱动程序.
    * [gocql](http://gocql.github.io) -用于Apache Cassandra的Go语言驱动程序.
    * [godis](https://github.com/piaohao/godis) -受jedis启发，由golang提供的redis客户工具.
    * [godscache](https://github.com/defcronyke/godscache) -Google Cloud Platform Go数据存储区包装的包装，可使用memcached添加缓存.
    ...
    * [redeo](https://github.com/bsm/redeo) -Redis协议兼容的TCP服务器/服务.
...
* [date](https://github.com/rickb777/date) -扩充时间，用于处理日期，日期范围，时间跨度，期间和一天中的时间.
...
* [celeriac](https://github.com/svcavallar/celeriac.v1) -用于在Go中添加对交互和监视Celery工作者，任务和事件的支持的库.
...
* [dragonboat](https://github.com/lni/dragonboat) -Go中功能完善的高性能多组Raft库.
...
* [emitter-io](https://github.com/emitter-io/emitter) -借助MQTT，Websockets和love构建的高性能，分布式，安全和低延迟的发布订阅平台.
* [flowgraph](https://github.com/vectaport/flowgraph) -基于流的编程包.
* [gleam](https://github.com/chrislusf/gleam) -以纯Go和Luajit编写的快速且可扩展的分布式地图/缩减系统，结合了Go的高并发性和Luajit的高性能，可独立运行或分布式.
...
* [errlog](https://github.com/snwfdhmp/errlog)  -可破解的软件包，用于确定错误的负责任的源代码（以及其他一些快速调试功能）. 可插入任何现场的记录器.
...
* [qs](https://github.com/sonh/qs) -Go模块，用于将结构编码为URL查询参数.
...
* [artifex](https://github.com/borderstech/artifex) -使用基于工作程序的调度，为Golang提供简单的内存中作业队列.
...
* [conexec](https://github.com/ITcathyh/conexec)  -并发工具包，可帮助以高效，安全的方式并发执行功能. 它支持指定总体超时以避免阻塞，并使用goroutine池提高效率.
...
* [goccm](https://github.com/zenthangplus/goccm) -Go Concurrency Manager软件包限制了允许并发运行的goroutine的数量.
...
* [queue](https://github.com/AnikHasibul/queue)  -为您提供“ sync.WaitGroup”，例如队列组可访问性. 帮助您节流和限制goroutine，等待所有goroutine结束等等.
...
* [go-appindicator](https://github.com/dawidd6/go-appindicator) -为libappindicator3 C库进行绑定.
...
* [darkroom](https://github.com/gojek/darkroom) -具有可更改的存储后端的图像代理和专注于速度和弹性的图像处理引擎.
...
* [JobRunner](https://github.com/bamzi/jobrunner) -具有内置的工作队列和实时监控功能的智能功能强大的Cron工作计划程序.
...
* [dynjson](https://github.com/cocoonspace/dynjson) -动态API的客户端可定制JSON格式.
...
* [go-log](https://github.com/subchen/go-log) -使用级别，格式化程序和编写器在Go中进行简单且可配置的日志记录.
...
* [gomol](https://github.com/aphistic/gomol) -具有可扩展日志输出的Go的多输出结构化日志.
...
* [logo](https://github.com/mbndr/logo) -Golang记录器到不同的可配置写入器.
...
* [onelog](https://github.com/francoispqt/onelog)  -Onelog是一个简单却非常高效的JSON记录器. 在所有情况下，它都是最快的JSON记录器. 而且，它是分配最低的记录器之一.
...
* [xlog](https://github.com/rs/xlog) -具有“ net / context”感知的HTTP处理程序的结构化记录器，具有灵活的调度功能.
...
* [go-featureprocessing](https://github.com/nikolaydubina/go-featureprocessing) -快速，方便的功能处理，用于在Go中进行低延迟的机器学习.
...
* [onnx-go](https://github.com/owulveryck/onnx-go) -进入开放神经网络交换（ONNX）的界面.
* [probab](https://github.com/ThePaw/probab)  -概率分布函数. 贝叶斯推理. 用纯Go语言编写.
...
* [Varis](https://github.com/Xamber/Varis) -神经网络Golang.
...
* [jazz](https://github.com/socifi/jazz) -简单的RabbitMQ抽象层，用于队列管理以及消息的发布和使用.
...
* [melody](https://github.com/olahol/melody) -处理websocket会话的简约框架，包括广播和自动ping / pong处理.
...
* [rmqconn](https://github.com/sbabiv/rmqconn)  -RabbitMQ重新连接. 包装在amqp.Connection和amqp.Dial上. 允许在断开连接之前执行重新连接，然后强制关闭Close（）方法的调用.
...
* [excelize](https://github.com/360EntSecGroup-Skylar/excelize) -Golang库，用于读写Microsoft Excel™（XLSX）文件.
...
* [scaffold](https://github.com/catchplay/scaffold)  -脚手架会生成Starter Go项目布局. 让您专注于实现的业务逻辑.

### Strings

*用于处理字符串的库.*

* [go-formatter](https://gitlab.com/tymonx/go-formatter) -实现由“ {}”花括号包围的“替换字段”格式字符串.
...
* [captcha](https://github.com/steambap/captcha) -验证码程序包为生成验证码提供了易于使用且不受质疑的API.
...
* [go-openapi](https://github.com/go-openapi) -收集用于解析和利用开放式API模式的软件包.
...
* [gotoprom](https://github.com/cabify/gotoprom) -适用于Prometheus官方客户的类型安全指标构建器包装器库.
* [gountries](https://github.com/pariz/gountries) -包含国家和细分数据的软件包.
...
* [hostutils](https://github.com/Wing924/hostutils) -用于打包和解压缩FQDN列表的golang库.
...
* [shoutrrr](https://github.com/containrrr/shoutrrr) -通知库，提供对各种消息传递服务的轻松访问，例如松弛，最重要，gotify和smtp等.
...
* [go-i18n](https://github.com/nicksnyder/go-i18n/) -打包和随附的工具，用于处理本地化的文本.
...
* [iuliia-go](https://github.com/mehanizm/iuliia-go) -各种可能的音译西里尔字母→拉丁字母.
...
* [ethernet](https://github.com/mdlayher/ethernet) -包以太网实现IEEE 802.3以太网II帧和IEEE 802.1Q VLAN标记的封送处理.
...
* [gohooks](https://github.com/averageflow/gohooks)  -GoHooks使从Go应用程序发送和使用受保护的Web钩子变得容易. 受Spatie的Laravel Webhook客户端和服务器启发.
...
* [gopcap](https://github.com/akrennmair/gopcap) -为libpcap包装.
...
* [tcp_server](https://github.com/firstrow/tcp_server) -更快地建立tcp服务器的库.
...
* [go-glmatrix](https://github.com/technohippy/go-glmatrix) -前往 [glMatrix](http://glmatrix.net/) 图书馆.
...
* [go-queryset](https://github.com/jirfag/go-queryset) -具有代码生成功能并基于GORM的MySQL，PostgreSQL，Sqlite3和SQL Server支持的100％类型安全的ORM.
...
* [gws](https://github.com/Zaba505/gws) -Apollos的“基于Websocket的GraphQL”客户端和服务器实现.
...
* [rqp](https://github.com/timsolov/rest-query-parser)  -REST API查询解析器. 查询直接支持过滤，验证以及“与”，“或”运算.
...
* [mule](https://github.com/wlbr/mule)  -将图像，电影等外部资源嵌入Go源代码中，以便使用“ go generate”创建单个文件二进制文件. 专注于简单性.
...
* [sparse](https://github.com/james-bowman/sparse) -线性代数的Go稀疏矩阵格式，支持科学和机器学习应用程序，与gonum矩阵库兼容.
...
* [argon2-hashing](https://github.com/andskur/argon2-hashing) -Go的argon2程序包周围的轻包装，与Go的标准库Bcrypt和simple-scrypt程序包非常相似.
...
* [certificates](https://github.com/mvmaasakkers/certificates) -生成tls证书的自带工具.
...
* [gogoprotobuf](https://github.com/gogo/protobuf) -与小工具配套使用的协议缓冲区.
...
* [go-feature-flag](https://github.com/thomaspoignant/go-feature-flag) -一种功能标记解决方案，后端仅包含YAML文件（S3，GitHub，HTTP，本地文件...），无需安装服务器，只需在中央系统中添加文件并引用即可.
* [go-proxy-cache](https://github.com/fabiocicerchia/go-proxy-cache) -使用Redis使用Go编写的带有缓存的简单反向代理.
...
* [SFTPGo](https://github.com/drakkan/sftpgo)  -功能齐全且高度可配置的SFTP服务器，具有可选的FTP / S和WebDAV支持. 它可以为本地文件系统和Cloud Storage后端（例如S3和Google Cloud Storage）提供服务.
...
* [damsel](https://github.com/dskinner/damsel) -标记语言具有通过css-selectors进行html概述的功能，可通过pkg html / template等扩展.
* [ego](https://github.com/benbjohnson/ego)  -轻量级的模板语言，可让您在Go中编写模板. 模板被翻译成Go并编译.
...
*测试框架
    ...
    * [goc](https://github.com/qiniu/goc) -Goc是针对Go编程语言的全面覆盖测试系统.
    ...
    * [httpexpect](https://github.com/gavv/httpexpect) -简洁，声明式且易于使用的端到端HTTP和REST API测试.
    ...
    * [schema](https://github.com/jgroeneveld/schema) -快速简便的表达式匹配，可用于请求和响应中的JSON模式.
    * [stop-and-go](https://github.com/elgohr/stop-and-go) -测试帮助程序的并发性.
    * [testcase](https://github.com/adamluzsi/testcase) -行为驱动开发的惯用测试框架.
    * [testfixtures](https://github.com/go-testfixtures/testfixtures) -Rails的测试夹具，用于测试数据库应用程序.
...
*特定格式
    ...
    * [codetree](https://github.com/aerogo/codetree) -解析缩进的代码（python，pixy，猩红色等）并返回树结构.
    ...
    * [dataflowkit](https://github.com/slotix/dataflowkit) -Web抓取框架，可将网站转变为结构化数据.
    ...
    * [go-slugify](https://github.com/mozillazg/go-slugify) -多种语言支持，让你很笨拙.
    ...
    * [go-zero-width](https://github.com/trubitsyn/go-zero-width) -零宽度字符检测和Go删除.
    ...
    * [html-to-markdown](https://github.com/JohannesKaufmann/html-to-markdown)  -将HTML转换为Markdown. 甚至适用于整个网站，并且可以通过规则进行扩展.
    ...
    * [omniparser](https://github.com/jf-tech/omniparser) -通用的ETL库，以流方式解析文本输入（CSV / txt / JSON / XML / EDI / X12 / EDIFACT / etc），并使用数据驱动的模式将数据转换为JSON输出.
...
* 公用事业
    ...
    * [regroup](https://github.com/oriser/regroup) -使用struct标记和自动解析功能将名为groups的正则表达式表达式匹配到go struct中.
...
* [coinpaprika-go](https://github.com/coinpaprika/coinpaprika-api-go-client) -用于与Coinpaprika的API交互的客户端库.
...
* [facebook](https://github.com/huandu/facebook) -转到支持Facebook Graph API的库.
...
* [go-postman-collection](https://github.com/rbretecher/go-postman-collection) -可以使用的模块 [Postman Collections](https://learning.getpostman.com/docs/postman/collections/creating-collections/) （与失眠兼容）.
...
* [gostorm](https://github.com/jsgilmore/gostorm) -GoStorm是一个Go库，该库实现了在Go中编写与Storm外壳进行通信的Storm喷口和螺栓所需的通信协议.
...
* [dlog](https://github.com/kirillDanshin/dlog) -编译时控制的记录器，使您的发行版更小，而无需删除调试调用.
...
* [git-time-metric](https://github.com/git-time-metric/gtm) -简单，无缝，轻量级的Git时间跟踪.
...
* [go-clip](https://github.com/prashantgupta24/go-clip) -Mac的简约剪贴板管理器.
...
* [go-dry](https://github.com/ungerik/go-dry) -Go的DRY（不要重复自己）软件包.
...
* [go-lock](https://github.com/viney-shih/go-lock) -go-lock是一个锁库，实现了没有饥饿的读写互斥和读写trylock.
...
* [jump](https://github.com/gsamokovarov/jump) -跳转可帮助您通过学习习惯来更快地导航.
* [koazee](https://github.com/wesovilabs/koazee) -受到懒惰评估和函数式编程启发的库，消除了使用数组的麻烦.
* [lets-go](https://github.com/aplescia-chwy/lets-go)  -Go模块，为Cloud Native REST API开发提供通用实用程序. 还包含AWS特定实用程序.
...
* [minquery](https://github.com/icza/minquery) -支持高效分页的MongoDB / mgo.v2查询（光标继续列出我们停下来的文档）.
* [moldova](https://github.com/StabbyCutyou/moldova) -用于基于输入模板生成随机数据的实用程序.
* [mole](https://github.com/davrodpin/mole) -cli应用程序可轻松创建ssh隧道.
* [mongo-go-pagination](https://github.com/gobeam/mongo-go-pagination) -适用于官方mongodb / mongo-go-driver程序包的Mongodb分页，它支持正常查询和聚合管道.
...
* [ptr](https://github.com/gotidy/ptr) -提供简化基本类型常量的指针创建功能的软件包.
...
* [spinner](https://github.com/briandowns/spinner) -打包即可轻松地为终端旋转器提供选项.
...
* [goid](https://github.com/jakehl/goid) -生成和解析符合RFC4122的V4 UUID.
...
* [mango](https://github.com/paulbellamy/mango) -芒果是受Rack和PEP333启发的Go的模块化Web应用程序框架.
...
* [rex](https://github.com/goanywhere/rex) -Rex是用于基于大猩猩/ mux进行模块化开发的库，与`net / http`完全兼容.
...
* [vox](https://github.com/aisk/vox) -受Koa大力启发的golang网络框架.
...
* [lars](https://github.com/go-playground/lars) -是用于Go的轻量级，快速且可扩展的零分配HTTP路由器，用于创建可自定义的框架.
...
* [apicompat](https://github.com/bradleyfalzon/apicompat) -检查最近对Go项目所做的更改是否存在向后不兼容的更改.
...
* [go-mod-outdated](https://github.com/psampaz/go-mod-outdated) -一种轻松的方法来查找Go项目的过时依赖项.
...
* [go-plus](https://github.com/joefitzgerald/go-plus) -Go（Golang）原子软件包，增加了自动完成，格式化，语法检查，整理和审核.
...
* [genny](https://github.com/cheekybits/genny) -Go的优雅仿制药.
...
* [gothanks](https://github.com/psampaz/gothanks) -GoThanks自动为您的go.mod github依赖项加注星标，从而向其维护者发送一些爱意.
...
* [abbreviate](https://github.com/dnnrly/abbreviate) -abbreviate是一种工具，可将长字符串转换为带有可配置分隔符的较短字符串，例如将分支名称嵌入部署堆栈ID中.
...
* [gobrew](https://github.com/cryptojuice/gobrew) -gobrew可让您轻松地在多个版本的go之间切换.
* [godbg](https://github.com/sirnewton01/godbg) -基于Web的gdb前端应用程序.
* [Gogs](https://gogs.io/) -使用Go编程语言的自托管Git服务.
* [gonative](https://github.com/inconshreveable/gonative) -创建Go的构建的工具，该工具可以交叉编译到所有平台，同时仍使用启用了Cgo的stdlib软件包版本.
* [govvv](https://github.com/ahmetalpbalkan/govvv) -“ go build”包装器，可轻松将版本信息添加到Go二进制文件中.
* [gox](https://github.com/mitchellh/gox) -死简单，没有多余的东西去交叉编译工具.
* [goxc](https://github.com/laher/goxc) -构建Go的工具，重点是交叉编译和打包.
...
* [Rodent](https://github.com/alouche/rodent) -啮齿动物可帮助您管理Go版本，项目并跟踪依赖关系.
...
* [Gor](https://github.com/buger/gor) -Http流量复制工具，用于实时回放从生产到阶段/开发环境的流量.
...
* [limetext](https://limetext.github.io) -Lime Text是一款功能强大且优雅的文本编辑器，主要是在Go中开发的，旨在成为Sublime Text的免费开源软件继任者.
...
* [wellington](https://github.com/wellington/wellington) -Sass项目管理工具，通过Sprite函数（例如Compass）扩展了语言.
...
* [Awesome Go @LibHunt](https://go.libhunt.com) -您转到的工具箱.
* [Awesome Remote Job](https://github.com/lukasz-madon/awesome-remote-job)  -精选的很棒的远程工作列表. 他们中的许多人正在寻找Go黑客.
* [awesome-awesomeness](https://github.com/bayandin/awesome-awesomeness) -其他超赞列表的列表.
...
* [Gopher Community Chat](https://invite.slack.golangbridge.org) -加入我们的新松弛鼠社区（[Understand how it came](https://blog.gopheracademy.com/gophers-slack-community/)).
...
* [Go Playground for iOS](https://codeplayground.app) -在移动设备上交互式地编辑和播放Go片段.
* [Go WebAssembly Tutorial - Building a Simple Calculator](https://tutorialedge.net/golang/go-webassembly-tutorial/)
* [go-patterns](https://github.com/tmrts/go-patterns) -Go设计模式，配方和习惯用法的精选清单.
...
* [GopherSnippets](https://gophersnippets.com/) -带有Go编程语言的测试和可测试示例的代码段.
...
* [Learn Go with 1000+ Exercises](https://github.com/inancgumus/learngo) -通过数千个示例，练习和测验学习Go.
