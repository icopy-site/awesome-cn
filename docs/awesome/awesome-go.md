 ...
 **我们没有每月费用** _，但我们有员工**努力工作**以维持Awesome Go，筹集的资金可以偿还每个参与人员的工作！ 所有的帐单和分配方式将向整个社区开放._
...
* [gosamplerate](https://github.com/dh1tw/gosamplerate) -libsamplerate绑定进行.
* [id3v2](https://github.com/bogem/id3v2) -用于Go的ID3解码和编码库.
...
* [go-guardian](https://github.com/shaj13/go-guardian) -Go-Guardian是一个golang库，它提供了一种简单，干净且惯用的方式来创建功能强大的现代API和Web身份验证，该功能支持LDAP，Basic，Bearer令牌和基于证书的身份验证.
...
* [jeff](https://github.com/abraithwaite/jeff) -具有可插拔后端的简单，灵活，安全和惯用的Web会话管理.
* [jwt](https://github.com/robbert229/jwt) -JSON Web令牌（JWT）的清洁易用实现.
* [jwt](https://github.com/pascaldekloe/jwt) -轻量级JSON Web令牌（JWT）库.
* [jwt](https://github.com/cristalhq/jwt) -用于Go的安全，简单和快速的JSON Web令牌.
...
* [sessions](https://github.com/adam-hanna/sessions) -用于go http服务器的简单，高性能，可高度自定义的会话服务.
...
* [echotron](https://github.com/NicoNex/echotron) -用纯Go语言编写的电报机器人库.
...
* [govkbot](https://github.com/nikepan/govkbot) -简单的去 [VK](https://vk.com) 机器人程序库.
* [hanu](https://github.com/sbstjn/hanu) -编写Slack机器人的框架.
* [Kelp](https://github.com/stellar/kelp) -官方交易和做市机器人 [Stellar](https://www.stellar.org/) 敏捷开箱即用，用Golang编写，可与集中交易和自定义交易策略兼容.
...
* [slack-bot](https://github.com/innogames/slack-bot) -可供懒惰的开发人员使用Slack Bot：自定义命令，Jenkins，Jira，Bitbucket，Github ...
...
* [tbot](https://github.com/yanzay/tbot) -具有类似于net / http的API的Telegram僵尸服务器.
...
* [argv](https://github.com/cosiner/argv) -转到库，使用bash语法将命令行字符串拆分为参数数组.
...
* [cmd](https://github.com/posener/cmd) -扩展了标准的flag软件包，以支持子命令，并以idomatic方式提供更多支持.
...
* [env](https://github.com/codingconcepts/env) -基于标记的环境配置.
* [flag](https://github.com/cosiner/flag) -用于Go支持子命令的简单但功能强大的命令行选项解析库.
...
* [gocmd](https://github.com/devfacet/gocmd) -转到用于构建命令行应用程序的库.
* [hiboot cli](https://github.com/hidevopsio/hiboot/tree/master/pkg/app/cli) -具有自动配置和依赖项注入的cli应用程序框架.
* [job](https://github.com/liujianping/job) -求职，将您的短期命令作为一项长期工作.
...
* [mow.cli](https://github.com/jawher/mow.cli) -用于通过复杂的标志和参数解析和验证来构建CLI应用程序的库.
...
* [asciigraph](https://github.com/guptarohit/asciigraph) -转到软件包，以在命令行应用程序中制作轻量级的ASCII线图╭┈╯，而没有其他依赖项.
* [aurora](https://github.com/logrusorgru/aurora) -支持fmt.Printf / Sprintf的ANSI终端颜色.
* [box-cli-maker](https://github.com/Delta456/box-cli-maker) -为CLI制作高度自定义的框.
* [cfmt](https://github.com/mingrammer/cfmt) -受引导颜色类启发的上下文fmt.
...
* [pterm](https://github.com/pterm/pterm) -一个通过许多可组合组件美化每个平台上的控制台输出的库.
* [simpletable](https://github.com/alexeyco/simpletable) -使用Go在终端中创建简单表.
* [tabby](https://github.com/cheynewallace/tabby) -一个用于超级简单的Golang表的小型库.
...
* [uiprogress](https://github.com/gosuri/uiprogress) -灵活的库，可在终端应用程序中显示进度条.
...
* [aconfig](https://github.com/cristalhq/aconfig) -简单，有用且自以为是的配置加载程序.
* [cleanenv](https://github.com/ilyakaznacheev/cleanenv) -简约的配置读取器（来自文件，ENV，以及您想要的任何位置）.
* [config](https://github.com/golobby/config) -用于Go项目的轻量级功能强大的配置包.
* [config](https://github.com/JeremyLoy/config)  -云原生应用程序配置. 仅在两行中将ENV绑定到结构.
* [config](https://github.com/olebedev/config) -带有环境变量和标志解析的JSON或YAML配置包装器.
* [configuration](https://github.com/BoRuDar/configuration) -用于从env变量，文件，标志和&#39;default&#39;标记初始化配置结构的库.
...
* [fig](https://github.com/kkyr/fig) -微小的库，用于从文件和环境变量中读取配置（带有验证和默认值）.
...
* [go-aws-ssm](https://github.com/PaddleHQ/go-aws-ssm) -Go软件包，可从AWS System Manager-参数存储中获取参数.
* [go-ini](https://github.com/subpop/go-ini) -封送和解封INI文件的Go软件包.
...
* [goConfig](https://github.com/crgimenes/goConfig) -解析一个结构作为输入，并使用命令行，环境变量和配置文件中的参数填充该结构的字段.
...
* [gonfig](https://github.com/milad-abbasi/gonfig) -基于标记的配置解析器，可将来自不同提供程序的值加载到类型安全的结构中.
...
* [mini](https://github.com/sasbury/mini) -用于解析ini样式的配置文件的Golang软件包.
...
* [drone](https://github.com/drone/drone) -Drone是一个用Go语言编写的基于Docker的持续集成平台.
* [duci](https://github.com/duck8823/duci) -简单的CI服务器不需要域特定的语言.
...
* [bloomfilter](https://github.com/OldPanda/bloomfilter) -Go中的另一个Bloomfilter实现，与Java的Guava库兼容.
* [boomfilters](https://github.com/tylertreat/BoomFilters) -用于处理连续无界流的概率数据结构.
* [cmap](https://github.com/lrita/cmap) -用于go的线程安全并发映射，支持使用“ interface {}”作为键并自动扩展分片.
* [concurrent-writer](https://github.com/free/concurrent-writer) -高度并行地替换“ bufio.Writer”.
* [conjungo](https://github.com/InVisionApp/conjungo) -小型，强大而灵活的合并库.
* [count-min-log](https://github.com/seiflotfy/count-min-log) -实施Count-Min-Log草图：使用近似计数器进行近似计数（类似于Count-Min草图，但使用较少的内存）.
* [crunch](https://github.com/superwhiskers/crunch) -Go包实现了用于轻松处理各种数据类型的缓冲区.
* [cuckoo-filter](https://github.com/linvon/cuckoo-filter) -布谷鸟过滤器：一个全面的布谷鸟过滤器，与其他工具相比，它是可配置的并且在空间上进行了优化，并且提供了原论文中提到的所有功能.
* [cuckoofilter](https://github.com/seiflotfy/cuckoofilter) -布谷鸟过滤器：Go中实现的计数布隆过滤器的一个很好的选择.
...
* [dsu](https://github.com/ihebu/dsu) -Go中的Disjoint Set数据结构实现.
...
* [go-edlib](https://github.com/hbollon/go-edlib) -与Unicode兼容的字符串比较和编辑距离算法库（Levenshtein，LCS，Hamming，Damerau levenshtein，Jaro-Winkler等）.
...
* [goterator](https://github.com/yaa110/goterator) -迭代器实现，以提供地图并减少功能.
* [hide](https://github.com/emvi/hide) -ID类型与进/出哈希进行编组，以防止将ID发送给客户端.
* [hilbert](https://github.com/google/hilbert) -Go软件包，用于在空间填充曲线（例如希尔伯特曲线和皮亚诺曲线）之间来回映射值.
...
* [levenshtein](https://github.com/agext/levenshtein) -Levenshtein距离和相似性度量标准，具有可自定义的编辑费用和通用前缀类似Winkler的奖金.
...
* [nan](https://github.com/kak-tus/nan) -零分配具有方便转换功能，编组器和解组器的一个库中的可空结构.
* [null](https://github.com/emvi/null) -可与JSON编组/解组的Nullable Go类型.
* [ordered-concurrently](https://github.com/tejzpr/ordered-concurrently) -处理并发工作的Go模块，并按输入顺序在通道中返回输出.
* [parapipe](https://github.com/nazar256/parapipe) -FIFO管道，在保持消息和结果顺序的同时，并行执行每个阶段的执行.
...
* [skiplist](https://github.com/gansidui/skiplist) -Go中的Skiplist实施.
* [slices](https://github.com/srfrog/slices)  -在分片上运行的功能； 类似于“包装字符串”，但适用于切片.
...
* [ttlcache](https://github.com/ReneKroon/ttlcache) -具有各种基于时间的到期选项和回调的内存中字符串接口{}缓存.
...
* [Bitcask](https://github.com/prologic/bitcask) -Bitcask是使用纯Go语言编写的可嵌入，持久且快速的键值（KV）数据库，由于使用了bitcask磁盘布局（LSM + WAL），因此具有可预测的读/写性能，低延迟和高吞吐量.
...
* [clusteredBigCache](https://github.com/oaStuff/clusteredBigCache) -BigCache具有群集支持和单个项到期.
...
* [hare](https://github.com/jameycribbs/hare) -一个简单的数据库管理系统，将每个表存储为以行分隔的JSON文本文件.
* [immudb](https://github.com/codenotary/immudb) -immudb是用于Go编写的系统和应用程序的轻量级高速不可变数据库.
...
* [pogreb](https://github.com/akrylysov/pogreb) -嵌入式键值存储，用于读取繁重的工作负载.
...
* [rosedb](https://github.com/roseduan/rosedb) -基于LSM + WAL的嵌入式kv数据库，支持字符串，列表，哈希，集合，zset.
* [rqlite](https://github.com/rqlite/rqlite) -建立在SQLite上的轻量级，分布式，关系数据库.
...
* [ttlcache](https://github.com/cheshir/ttlcache) -内存中的键值存储，每个记录带有TTL.
* [unitdb](https://github.com/unit-io/unitdb)  -用于物联网，实时消息传递应用程序的快速时间序列数据库. 使用github.com/unit-io/unitd应用程序通过tcp或websocket通过pubsub访问unitdb.
* [Vasto](https://github.com/chrislusf/vasto)  -分布式高性能键值存储. 在磁盘上. 最终一致. 哈. 能够增长或缩小而不会中断服务.
...
* [go-pg-migrate](https://github.com/lawzava/go-pg-migrate) -支持CLI的软件包，用于go-pg迁移管理.
* [go-pg-migrations](https://github.com/robinjoseph08/go-pg-migrations) -Go软件包，可帮助编写go-pg / pg的迁移文件.
* [goose](https://github.com/pressly/goose)  -数据库迁移工具. 您可以通过创建增量SQL或Go脚本来管理数据库的演变.
...
* [migrator](https://github.com/larapulse/migrator) -MySQL数据库迁移器，旨在通过直观的go代码运行向您的功能的迁移并管理数据库架构更新.
* [pravasan](https://github.com/pravasan/pravasan) -简单的迁移工具-目前适用于MySQL，但计划很快支持Postgres，SQLite，MongoDB等.
* [schema](https://github.com/adlio/schema) -在Go二进制文件中嵌入用于数据库/ sql兼容数据库的架构迁移的库.
* [skeema](https://github.com/skeema/skeema) -适用于MySQL的Pure-SQL模式管理系统，支持分片和外部在线模式更改工具.
...
* [pg_timetable](https://github.com/cybertec-postgresql/pg_timetable) -PostgreSQL的高级计划.
...
* [ormlite](https://github.com/pupizoid/ormlite) -轻量级软件包，其中包含一些类似于ORM的功能和sqlite数据库的帮助程序.
* [ozzo-dbx](https://github.com/go-ozzo/ozzo-dbx) -强大的数据检索方法以及与数据库无关的查询构建功能.
* [qry](https://github.com/HnH/qry) -使用原始SQL查询从文件生成常量的工具.
* [sq](https://github.com/bokwoon95/go-structured-query) -Go的类型安全的SQL构建器和结构映射器.
...
*关系数据库
    ...
    * [bgc](https://github.com/viant/bgc) -适用于go的BigQuery的数据存储连接.
    * [firebirdsql](https://github.com/nakagami/firebirdsql) -用于Go的Firebird RDBMS SQL驱动程序.
    * [go-adodb](https://github.com/mattn/go-adodb) -用于使用数据库/ sql的go的Microsoft ActiveX Object DataBase驱动程序.
    ...
    * [godror](https://github.com/godror/godror) -使用ODPI-C驱动程序的Go的Oracle驱动程序.
    ...
    * [pig](https://github.com/alexeyco/pig) - Simple [pgx](https://github.com/jackc/pgx) 包装器执行并 [scan](https://github.com/georgysavva/scany) 轻松查询结果.
    * [pq](https://github.com/lib/pq) -用于数据库/ sql的Pure Go Postgres驱动程序.
    * [Sqinn-Go](https://github.com/cvilsmeier/sqinn-go) -带有纯Go的SQLite.

* NoSQL数据库
    * [aerospike-client-go](https://github.com/aerospike/aerospike-client-go) -Go语言的Aerospike客户.
    * [arangolite](https://github.com/solher/arangolite) -用于ArangoDB的轻量级golang驱动程序.
    ...
    * [gocosmos](https://github.com/btnguyen2k/gocosmos) -用于Azure Cosmos DB的REST客户端和标准`database / sql`驱动程序.
    * [gocql](http://gocql.github.io) -用于Apache Cassandra的Go语言驱动程序.
    * [godis](https://github.com/piaohao/godis) -受jedis启发，由golang编写的redis客户工具.
    ...
    * [qmgo](https://github.com/qiniu/qmgo)  -Go的MongoDB驱动程序. 它基于官方的MongoDB驱动程序，但像Mgo一样易于使用.
...
* [feiertage](https://github.com/wlbr/feiertage)  -一组计算德国公共假期的功能，包括德国（Bundesländer）州的专业. 像复活节，五旬节，感恩节之类的东西.
* [go-persian-calendar](https://github.com/yaa110/go-persian-calendar) -在Go（golang）中实施了波斯（Solar Hijri）日历.
...
* [gostradamus](https://github.com/bykof/gostradamus) -用于处理日期的Go软件包.
...
* [consistenthash](https://github.com/mbrostami/consistenthash) -与可配置副本一致的哈希.
...
* [dynatomic](https://github.com/tylfin/dynatomic) -用于将DynamoDB用作原子计数器的库.
...
* [gleam](https://github.com/chrislusf/gleam) -以纯Go和Luajit编写的快速，可扩展的分布式地图/缩减系统，结合了Go的高并发性和Luajit的高性能，可独立运行或分布式.
...
* [go-sundheit](https://github.com/AppsFlyer/go-sundheit) -建立用于为golang服务定义异步服务运行状况检查提供支持的库.
* [go-zero](https://github.com/tal-tech/go-zero)  -网络和rpc框架. 它的诞生是为了通过弹性设计确保繁忙站点的稳定性. 内置的goctl大大提高了开发效率.
...
* [pjrpc](https://gitlab.com/pjrpc/pjrpc) -具有Protobuf规范的Golang JSON-RPC服务器-客户端.
...
* [Semaphore](https://github.com/jexia/semaphore) -一个简单的（微）服务协调器.
* [sleuth](https://github.com/ursiform/sleuth) -用于HTTP服务之间的无主p2p自动发现和RPC的库（使用 [ZeroMQ](https://github.com/zeromq/libzmq)).
...
* [DDNS](https://github.com/skibish/ddns) -以Digital Ocean Networking DNS为后端的个人DDNS客户端.
* [dyndns](https://gitlab.com/alcastle/dyndns) -后台执行流程可定期自动检查您的IP地址，并在您的地址发生更改时更新（一个或多个）Google域的动态DNS记录.
...
* [go-email-validator](https://github.com/go-email-validator/go-email-validator) -用于语法，一次性，SMTP等检查的模块化电子邮件验证器.
...
* [goja](https://github.com/dop251/goja) -Go中的ECMAScript 5.1（+）实现.
...
* [ngaro](https://github.com/db47h/ngaro) -可嵌入的Ngaro VM实现可在Retro中实现脚本编写.
* [purl](https://github.com/ian-kent/purl) -嵌入在Go中的Perl 5.18.2.
* [tengo](https://github.com/d5/tengo) -Go的字节码编译脚本语言.
...
* [errors](https://github.com/bnkamalesh/errors)  -内置替换，用于构建Go错误. 这是一个最小的错误处理程序包，具有自定义错误类型，用户友好消息，Unwrap＆Is. 具有非常易于使用和简单的帮助功能.
* [errorx](https://github.com/joomcode/errorx) -具有堆栈跟踪，错误组成等的功能丰富的错误包.
* [Falcon](https://github.com/SonicRoshan/falcon) -用于错误处理的简单但功能强大的软件包.
...
* [baraka](https://github.com/xis/baraka) -一个用于处理http文件上传的库.
...
* [higgs](https://github.com/dastoori/higgs) -一个微小的跨平台Go库，用于隐藏/取消隐藏文件和目录.
...
* [skywalker](https://github.com/dixonwille/skywalker) -允许一个人轻松地同时通过一个文件系统的软件包.
...
* [todotxt](https://github.com/1set/todotxt) -去吉娜·特拉帕尼（Gina Trapani）的图书馆 [*todo.txt*](http://todotxt.org/) 文件，支持解析和处理任务列表中 [*todo.txt* format](https://github.com/todotxt/todo.txt).
...
* [sleet](https://github.com/BoltApp/sleet) -一个统一的界面，供多个支付服务提供商（PsP）处理在线支付.
...
* [conform](https://github.com/leebenson/conform)  -使用户输入保持选中状态. 根据struct标记修剪，清理和清理数据.
...
* [qs](https://github.com/sonh/qs) -用于将结构编码为URL查询参数的Go模块.
...
* [gofp](https://github.com/rbrahul/gofp) -像Golang一样强大的实用工具库.
...
* [go-sdl2](https://github.com/veandco/go-sdl2) -进行绑定 [Simple DirectMedia Layer](https://www.libsdl.org/).
...
* [nano](https://github.com/lonng/nano) -轻巧，便利，基于高性能golang的游戏服务器框架.
* [Oak](https://github.com/oakmound/oak) -Pure Go游戏引擎.
* [Pitaya](https://github.com/topfreegames/pitaya) -可扩展的游戏服务器框架，具有群集支持，并通过C SDK为iOS，Android，Unity等提供了客户端库.
...
* [termloop](https://github.com/JoelOtter/termloop) -在Termbox之上构建的用于Go的基于终端的游戏引擎.
* [tile](https://github.com/kelindar/tile) -面向数据且易于缓存的2D网格库（TileMap），包括寻路，观察者和导入/导出.
...
* [go-xray](https://github.com/pieterclaerhout/go-xray) -帮助使反射更容易使用的辅助工具.
* [goderive](https://github.com/awalterschulze/goderive) -从输入类型派生功能.
* [gotype](https://github.com/wzshiming/gotype) -Golang源代码解析，用法像反射包.
...
* [typeregistry](https://github.com/xiaoxin01/typeregistry) -一个可以动态创建类型的库.
...
* [pbf](https://github.com/maguro/pbf) -OpenStreetMap PBF组编码器/解码器.
...
* [async](https://github.com/reugn/async) -Go的备用同步库（未来，承诺，锁）.
...
* [concurrency-limiter](https://github.com/vivek-ng/concurrency-limiter) -并发限制器，支持goroutine的超时，动态优先级和上下文取消.
* [conexec](https://github.com/ITcathyh/conexec)  -并发工具包，可帮助以高效，安全的方式并发执行功能. 它支持指定整体超时以避免阻塞，并使用goroutine池提高效率.
...
* [go-workers](https://github.com/catmullet/go-workers) -轻松安全地为大型数据处理管道运行工作者.
* [goccm](https://github.com/zenthangplus/goccm) -Go Concurrency Manager程序包限制了允许并发运行的goroutine的数量.
...
* [gowl](https://github.com/hamed-yousefi/gowl)  -Gowl同时是一个过程管理和过程监视工具. 无限的工作池使您能够控制池和进程并监视其状态.
...
* [Hunch](https://github.com/AaronJan/Hunch) -Hunch提供了诸如“全部”，“优先”，“重试”，“瀑布”等功能，使异步流控制更加直观.
* [kyoo](https://github.com/dirkaholic/kyoo) -提供无限的作业队列和并发工作池.
* [neilotoole/errgroup](https://github.com/neilotoole/errgroup) -可以替代`sync / errgroup`，仅限于N个工人goroutine池.
...
* [pond](https://github.com/alitto/pond) -用Go语言编写的简约高性能Goroutine工作池.
...
* [semaphore](https://github.com/marusama/semaphore) -基于CAS的可快速调整大小的信号灯实现（比基于通道的信号灯实现更快）.
...
* [go-astilectron](https://github.com/asticode/go-astilectron) -使用GO和HTML / JS / CSS（由Electron支持）构建跨平台的GUI应用程序.
* [go-gtk](http://mattn.github.io/go-gtk/) -绑定GTK.
* [go-sciter](https://github.com/sciter-sdk/go-sciter)  -Sciter的绑定：用于现代桌面UI开发的可嵌入HTML / CSS / script引擎. 跨平台.
...
* [walk](https://github.com/lxn/walk) -用于Go的Windows应用程序库工具包.
...
See [go-hardware](https://github.com/rakyll/go-hardware) 以获得完整的列表.
...
* [darkroom](https://github.com/gojek/darkroom) -具有可变存储后端的图像代理和注重速度和弹性的图像处理引擎.
...
* [govips](https://github.com/davidbyttow/govips) -Go的闪电般快速图像处理和调整大小的库.
* [gridder](https://github.com/shomali11/gridder) -基于网格的2D图形库.
...
* [imaginary](https://github.com/h2non/imaginary) -快速，简单的HTTP微服务，用于图像大小调整.
...
* [webp-server](https://github.com/mehdipourfar/webp-server) -简单而最小的图像服务器，能够存储，调整大小，转换和缓存图像.
...
* [gatt](https://github.com/paypal/gatt) -Gatt是用于构建低功耗蓝牙外围设备的Go软件包.
...
* [cronticker](https://github.com/krayzpipes/cronticker) -计价器实现以支持cron计划.
* [go-cron](https://github.com/rk/go-cron)  -适用于go的简单Cron库，可以在不同的时间间隔执行关闭或函数，从每秒一次到每年一次在特定日期和时间. 主要用于Web应用程序和长期运行的守护程序.
* [go-quartz](https://github.com/reugn/go-quartz) -简单，零依赖的Go调度库.
* [gocron](https://github.com/go-co-op/gocron)  -轻松流畅的Go作业调度. 这是一个积极维护 [jasonlvhit/gocron](https://github.com/jasonlvhit/gocron).
* [gron](https://github.com/roylee0704/gron) -使用简单的Go API定义基于时间的任务，Gron的调度程序将相应地运行它们.
* [JobRunner](https://github.com/bamzi/jobrunner) -具有内置的作业队列和实时监控功能的智能功能强大的Cron作业调度程序.
...
* [tasks](https://github.com/madflojo/tasks) -易于使用的进程内调度程序，用于Go中的重复任务.
...
* [ask](https://github.com/simonnilsson/ask)  -轻松访问地图和切片中的嵌套值. 与encoding / json和其他用于将任意数据“解组”为Go数据类型的程序包结合使用.
* [dynjson](https://github.com/cocoonspace/dynjson) -动态API的客户可定制的JSON格式.
* [ej](https://github.com/lucassscaravelli/ej) -简洁地编写和读取来自不同来源的JSON.
* [epoch](https://github.com/vtopc/epoch) -包含用于将Unix时间戳/时期编组到内置时间或从内置时间编组/取消编组的原语.JSON中的时间类型.
* [fastjson](https://github.com/valyala/fastjson)  -Go的快速JSON解析器和验证器. 没有自定义结构，没有代码生成，没有反射.
...
* [jettison](https://github.com/wI2L/jettison) -用于Go的快速，灵活的JSON编码器.
* [JSON-to-Go](https://mholt.github.io/json-to-go/) -将JSON转换为Go结构.
* [JSON-to-Proto](https://json-to-proto.github.io/) -在线将JSON转换为Protobuf.
...
* [jsondiff](https://github.com/wI2L/jsondiff) -基于RFC6902（JSON补丁）的Go的JSON差异库.
* [jsonf](https://github.com/miolini/jsonf) -控制台工具，用于突出显示格式和获取JSON的结构查询.
...
* [glog](https://github.com/golang/glog) -Go的级别执行日志.
...
* [httpretty](https://github.com/henvic/httpretty) -在终端上漂亮地打印常规HTTP请求以进行调试（类似于http.DumpRequest）.
* [journald](https://github.com/ssgreg/journald) -实施systemd Journal的本机API进行日志记录.
* [kemba](https://github.com/clok/kemba) -受启发的微型调试记录工具 [debug](https://github.com/visionmedia/debug)，非常适合CLI工具和应用程序.
...
* [logger](https://github.com/azer/logger) -用于Go的简约日志记录库.
...
* [logrusiowriter](https://github.com/cabify/logrusiowriter) -使用的io.Writer实现 [logrus](https://github.com/sirupsen/logrus) 日志.
* [logrusly](https://github.com/sebest/logrusly) - [logrus](https://github.com/sirupsen/logrus) 插件将错误发​​送到 [Loggly](https://www.loggly.com/).
* [logur](https://github.com/logur/logur) -一个自以为是的记录器界面，并收集了带有适配器和知名库集成的最佳记录实践（[logrus](https://github.com/sirupsen/logrus), [go-kit log](https://github.com/go-kit/kit/tree/master/log), [zap](https://github.com/uber-go/zap), [zerolog](https://github.com/rs/zerolog)， 等等）.
...
* [onelog](https://github.com/francoispqt/onelog)  -Onelog是一个非常简单但非常有效的JSON记录器. 在所有情况下，它都是最快的JSON记录器. 而且，它是分配最低的记录器之一.
* [ozzo-log](https://github.com/go-ozzo/ozzo-log)  -高性能日志记录，支持日志严重性，分类和过滤. 可以将过滤后的日志消息发送到各种目标（例如，控制台，网络，邮件）.
* [phuslu/log](https://github.com/phuslu/log) -结构化记录变得容易.
* [rollingwriter](https://github.com/arthurkiller/rollingWriter) -RollingWriter是自动旋转的io.Writer实现，具有多种策略来提供日志文件的旋转.
* [seelog](https://github.com/cihub/seelog) -具有灵活的调度，过滤和格式化功能的日志记录功能.
* [spew](https://github.com/davecgh/go-spew) -为Go数据结构实现了一个深层漂亮的打印机，以帮助调试.
* [sqldb-logger](https://github.com/simukti/sqldb-logger) -Go SQL数据库驱动程序的记录器，无需修改现有的* sql.DB stdlib用法.
* [stdlog](https://github.com/alexcesaro/log)  -Stdlog是一个面向对象的库，提供了分层的日志记录. 这对于cron作业非常有用.
* [tail](https://github.com/hpcloud/tail) -努力模仿BSD尾部程序功能的Go软件包.
* [xlog](https://github.com/xfxdev/xlog) -Go的插件架构和灵活的日志系统，具有ctrl级，多个日志目标和自定义日志格式.
* [xlog](https://github.com/rs/xlog) -用于“ net / context”感知的HTTP处理程序的结构化记录器，具有灵活的调度功能.
* [yell](https://github.com/jfcg/yell) -另一个简约的日志记录库.
* [zap](https://github.com/uber-go/zap) -在Go中进行快速，结构化，分级的日志记录.
* [zerolog](https://github.com/rs/zerolog) -零分配JSON记录器.
* [zkits-logger](https://github.com/edoger/zkits-logger) -强大的零依赖JSON记录器.
...
* [ddt](https://github.com/sgrodriguez/ddt) -动态决策树，创建定义可自定义规则的树.
...
* [go-fann](https://github.com/white-pony/go-fann) -快速神经网络（FANN）库的绑定.
* [go-featureprocessing](https://github.com/nikolaydubina/go-featureprocessing) -快速，方便的功能处理，可在Go中实现低延迟的机器学习.
...
* [gosseract](https://github.com/otiai10/gosseract) -通过使用Tesseract C ++库，用于OCR（光学字符识别）的软件包.
...
* [neural-go](https://github.com/schuyler/neural-go) -在Go中实施的多层感知器网络，并通过反向传播进行训练.
...
* [Beaver](https://github.com/Clivern/Beaver) -实时消息传递服务器，用于在Web和移动应用程序中构建可扩展的应用程序内通知，多人游戏，聊天应用程序.
...
* [Confluent Kafka Golang Client](https://github.com/confluentinc/confluent-kafka-go) -confluent-kafka-go是Apache Kafka和Confluent平台的Confluent的Golang客户端.
...
* [emitter](https://github.com/olebedev/emitter) -使用Go方式发出事件，并使用通配符，谓词，取消可能性和许多其他不错的胜利.
* [event](https://github.com/agoalofalife/event) -模式观察器的实现.
* [EventBus](https://github.com/asaskevich/EventBus) -具有异步兼容性的轻量级事件总线.
* [gaurun-client](https://github.com/osamingo/gaurun-client) -用Go编写的Gaurun Client.
* [Glue](https://github.com/desertbit/glue) -健壮的Go和Javascript套接字库（替代Socket.io）.
* [go-mq](https://github.com/cheshir/go-mq) -具有声明性配置的RabbitMQ客户端.
...
* [Gollum](https://github.com/trivago/gollum) -一个n：m多路复用器，它从不同的来源收集消息并将其广播到一组目标.
* [golongpoll](https://github.com/jcuga/golongpoll) -HTTP longpoll服务器库，使Web pub-sub变得简单.
* [gopush-cluster](https://github.com/Terry-Mao/gopush-cluster) -gopush-cluster是go push服务器集群.
...
* [gosd](https://github.com/alexsniffin/gosd) -用于计划何时将消息发送到通道的库.
* [guble](https://github.com/smancke/guble) -使用推送通知的消息服务器（Google Firebase Cloud Messaging，Apple推送通知服务，SMS）以及websockets，一种REST API，具有分布式操作和消息持久性.
* [hare](https://github.com/leozz37/hare) -一个用户友好的库，用于发送消息和侦听TCP套接字.
...
* [mangos](https://github.com/nanomsg/mangos) -具有传输互操作性的Nanomsg（“可伸缩性协议”）的完全实施.
...
* [pubsub](https://github.com/tuxychandru/pubsub) -可以使用的简单pubsub软件包.
...
* [zmq4](https://github.com/pebbe/zmq4) -转到ZeroMQ版本4的接口.也可用于 [version 3](https://github.com/pebbe/zmq3) 和 [version 2](https://github.com/pebbe/zmq2).
...
* [xlsx](https://github.com/tealeg/xlsx) -简化阅读Go程序中最新版本的Microsoft Excel所用XML格式的库.
...
* [container](https://github.com/golobby/container) -强大的IoC容器，具有流畅且易于使用的界面.
...
* [goioc/di](https://github.com/goioc/di) -春天启发的依赖注入容器.
* [kinit](https://github.com/go-kata/kinit) -可定制的依赖项注入容器，具有全局模式，级联初始化和应急安全完成.
* [linker](https://github.com/logrange/linker) -基于反射的依赖项注入和具有组件生命周期支持的控制库反转.
* [nject/npoint](https://github.com/BlueOwlOpenSource/nject) -基于库，测试和端点的类型的类型安全的反射框架.
...
* [insidieux/inizio](https://github.com/insidieux/inizio) -带有插件的Golang项目布局生成器.
...
* [gobeam/Stringy](https://github.com/gobeam/Stringy) -字符串处理库，用于将字符串转换为骆驼箱，蛇箱，烤肉串箱/子弹箱等.
* [strutil](https://github.com/ozgio/strutil) -字符串实用程序.
* [xstrings](https://github.com/huandu/xstrings) -收集了从其他语言移植过来的有用的字符串函数.

### Uncategorized

*将这些库放在这里是因为其他类别似乎都不适合.

* [anagent](https://github.com/mudler/anagent) -带有依赖项注入的简约，可插入的Golang evloop / timer处理程序.
* [antch](https://github.com/antchfx/antch) -快速，强大和可扩展的Web爬网和抓取框架.
* [archiver](https://github.com/mholt/archiver) -用于制作和提取.zip和.tar.gz档案的库和命令.
* [autoflags](https://github.com/artyom/autoflags) -转到软件包，以从结构字段自动定义命令行标志.
...
* [datacounter](https://github.com/miolini/datacounter) -前往读者/作家/http.ResponseWriter的计数器.
* [faker](https://github.com/pioz/faker) -Go的随机虚假数据和结构生成器.
* [ffmt](https://github.com/go-ffmt/ffmt) -美化人类的数据显示.
* [gatus](https://github.com/TwinProduction/gatus) -自动化的服务运行状况仪表板.
...
* [gotoprom](https://github.com/cabify/gotoprom) -面向正式Prometheus客户端的类型安全指标构建器包装器库.
* [gountries](https://github.com/pariz/gountries) -包含国家/地区和细分数据的数据包.
...
* [lk](https://github.com/hyperboloide/lk) -用于golang的简单许可库.
* [llvm](https://github.com/llir/llvm) -用于在纯Go语言中与LLVM IR交互的库.
* [metrics](https://github.com/pascaldekloe/metrics) -度量工具和Prometheus展示库.
* [morse](https://github.com/alwindoss/morse) -可以在莫尔斯电码之间来回转换的库.
* [numa](https://github.com/lrita/numa)  -NUMA是一个实用程序库，它是用go编写的. 它有助于我们编写一些NUMA-AWARED代码.
...
* [address](https://github.com/bojanz/address) -处理地址表示，验证和格式设置.
...
* [gojieba](https://github.com/yanyiwu/gojieba) -这是Go的实现 [jieba](https://github.com/fxsjy/jieba) which a Chinese word splitting algorithm.
* [golibstemmer](https://github.com/rjohnsondev/golibstemmer) -绑定雪球libstemmer库，包括搬运工2.
* [gosentiwordnet](https://github.com/dinopuguh/gosentiwordnet) -在Go中使用sentiwordnet词典的情感分析器.
...
* [iuliia-go](https://github.com/mehanizm/iuliia-go) -音译西里尔字母→拉丁字母
...
* [MMSEGO](https://github.com/awsong/MMSEGO) -这是GO的实现 [MMSEG](http://technology.chtsai.org/mmseg/) which a Chinese word splitting algorithm.
...
* [RAKE.go](https://github.com/afjoseph/RAKE.Go) -快速自动关键字提取算法（RAKE）的端口.
* [segment](https://github.com/blevesearch/segment) -执行如下所述的用于执行Unicode文本分段的库 [Unicode Standard Annex #29](http://www.unicode.org/reports/tr29/)
...
* [spaGO](https://github.com/nlpodyssey/spago) -Go中独立的机器学习和自然语言处理库.
...
* [whatlanggo](https://github.com/abadojack/whatlanggo)  -用于Go的自然语言检测程序包. 支持84种语言和24种脚本（书写系统，例如拉丁语，西里尔字母等）.
...
* [ethernet](https://github.com/mdlayher/ethernet) -包以太网实现IEEE 802.3以太网II帧和IEEE 802.1Q VLAN标签的封送和封送.
...
* [ftpserverlib](https://github.com/fclairamb/ftpserverlib) -功能齐全的FTP服务器库.
...
* [gnet](https://github.com/panjf2000/gnet) -`gnet`是用纯Go编写的高性能，轻量级，无阻塞，事件驱动的网络框架.
...
* [gohooks](https://github.com/averageflow/gohooks)  -GoHooks使您可以轻松地从Go应用程序发送和使用受保护的Web钩子. 受Spatie的Laravel Webhook客户端和服务器启发.
* [golibwireshark](https://github.com/sunwxg/golibwireshark) -软件包golibwireshark使用libwireshark库来解码pcap文件并分析解剖数据.
* [gopacket](https://github.com/google/gopacket) -带有libpcap绑定的数据包处理库.
...
* [lhttp](https://github.com/fanux/lhttp) -强大的websocket框架，可以更轻松地构建IM服务器.
* [linkio](https://github.com/ian-kent/linkio) -用于读取器/写入器接口的网络链接速度仿真.
* [llb](https://github.com/kirillDanshin/llb)  -对于代理服务器来说，这是一个非常简单但快速的后端. 对于零内存分配和快速响应的快速重定向到预定义域很有用.
...
* [nbio](https://github.com/lesismal/nbio) -用Go编写的高性能，非阻塞，事件驱动，易于使用，最低依赖性的网络框架.
...
* [panoptes-stream](https://github.com/yahoo/panoptes-stream) -云原生分布式流网络遥测（gNMI，Juniper JTI和Cisco MDT）.
* [peerdiscovery](https://github.com/schollz/peerdiscovery) -用于使用UDP多播的跨平台本地对等点发现的Pure Go库.
* [portproxy](https://github.com/aybabtme/portproxy) -简单的TCP代理，它向不支持的API添加了CORS支持.
...
* [sftp](https://github.com/pkg/sftp) -软件包sftp实施了SSH文件传输协议，如https://filezilla-project.org/specs/draft-ietf-secsh-filexfer-02.txt中所述.
...
* [tcp_server](https://github.com/firstrow/tcp_server) -可以更快地构建tcp服务器的库.
* [tspool](https://github.com/two/tspool) -TCP库使用工作池来提高性能并保护服务器.
* [utp](https://github.com/anacrolix/utp) -实施uTP微传输协议.
* [vssh](https://github.com/yahoo/vssh) -Go库，用于通过SSH协议构建网络和服务器自动化.
* [water](https://github.com/songgao/water) -简单的TUN / TAP库.
* [webrtc](https://github.com/pions/webrtc) -WebRTC API的纯Go实现.
* [winrm](https://github.com/masterzen/winrm) -转到WinRM客户端以在Windows计算机上远程执行命令.
* [xtcp](https://github.com/xfxdev/xtcp) -具有同时全双工通信，正常关闭和自定义协议的TCP Server Framework.
...
* [request](https://github.com/monaco-io/request)  -用于golang的HTTP客户端. 如果您有关于axios或请求的经验，那么您会喜欢的. 没有第三个依赖项.
...
* [gl](https://github.com/go-gl/gl) -用于OpenGL的绑定（通过光晕生成）.
* [glfw](https://github.com/go-gl/glfw) -为GLFW 3进行绑定.
* [go-glmatrix](https://github.com/technohippy/go-glmatrix) -前往的港口 [glMatrix](http://glmatrix.net/) 图书馆.
...
* [ent](https://github.com/facebook/ent)  -Go的实体框架. 简单而强大的ORM，用于建模和查询数据.
* [go-firestorm](https://github.com/jschoedt/go-firestorm) -适用于Google / Firebase Cloud Firestore的简单ORM.
* [go-pg](https://github.com/go-pg/pg) -PostgreSQL ORM，专注于PostgreSQL的特定功能和性能.
...
* [GORM](https://github.com/go-gorm/gorm) -出色的Golang ORM库，旨在对开发人员友好.
* [gormt](https://github.com/xxjwxc/gormt) -将Mysql数据库转换为golang gorm结构.
...
* [lore](https://github.com/abrahambotros/lore) -用于Go的简单轻量的伪ORM /伪结构映射环境.
* [marlow](https://github.com/marlow/marlow) -从项目结构生成的ORM，以确保编译时的安全性.
...
* [rel](https://github.com/go-rel/rel) -适用于Golang的现代数据库访问层-可测试，可扩展且精心制作成简洁美观的API.
...
* [XORM](https://gitea.com/xorm/xorm)  -简单而强大的Go专用ORM.  （支持：MySQL，MyMysql，PostgreSQL，Tidb，SQLite3，MsSql和Oracle）.
...
* [gom](https://github.com/mattn/gom) -Go Manager-go捆绑包.
...
* [modgv](https://github.com/lucasepe/modgv) -将“ go mod graph”输出转换为Graphviz的DOT语言.
* [mvn-golang](https://github.com/raydac/mvn-golang) -该插件提供了自动加载Golang SDK，依赖性管理以及在Maven项目基础架构中启动构建环境的方式.
...
* [pixie](https://github.com/pixie-labs/pixie) -没有通过eBPF进行Golang应用程序的仪器跟踪.
* [profile](https://github.com/pkg/profile) -Go的简单分析支持包.
* [statsviz](https://github.com/arl/statsviz) -您的Go应用程序运行时统计信息的实时可视化.
* [tracer](https://github.com/kamilsk/tracer) -简单，轻便的跟踪.

## Query Language

* [api-fu](https://github.com/ccbrown/api-fu) -全面的GraphQL实现.
* [dasel](https://github.com/tomwright/dasel)  -使用命令行中的选择器查询和更新数据结构. 与jq / yq相似，但支持JSON，YAML，TOML和XML，且运行时相关性为零.
...
* [graphql](https://github.com/neelance/graphql-go) -注重易用性的GraphQL服务器.
* [graphql-go](https://github.com/graphql-go/graphql) -实施GraphQL for Go.
* [gws](https://github.com/Zaba505/gws) -Apollos的“ Websocket上的GraphQL”客户端和服务器实现.
* [jsonpath](https://github.com/AsaiYusuke/jsonpath) -一个查询库，用于基于JSONPath语法检索JSON的一部分.
...
* [rqp](https://github.com/timsolov/rest-query-parser)  -REST API查询解析器. 查询中直接支持过滤，验证以及“与”，“或”运算.
* [straf](https://github.com/SonicRoshan/straf) -轻松将Golang结构转换为GraphQL对象.

## Resource Embedding

* [debme](https://github.com/leaanthony/debme) -从现有的“ embed.FS”子目录中创建“ embed.FS”.
...
* [mule](https://github.com/wlbr/mule)  -将图像，电影等外部资源嵌入到Go源代码中，以使用“ go generate”创建单个文件二进制文件. 专注于简单性.
* [packr](https://github.com/gobuffalo/packr) -将静态文件嵌入Go二进制文件的简单方法.
* [rebed](https://github.com/soypat/rebed) -从Go 1.16的`embed.FS`类型重新创建文件夹结构和文件
...
* [calendarheatmap](https://github.com/nikolaydubina/calendarheatmap) -受Github贡献活动启发的纯Go日历热图.
...
* [decimal](https://github.com/db47h/decimal) -程序包十进制实现了任意精度的十进制浮点运算.
...
* [go-estimate](https://github.com/milosgajdos/go-estimate) -Go中的状态估计和过滤算法.
* [go-gt](https://github.com/ThePaw/go-gt) -用“ Go”语言编写的图论算法.
* [godesim](https://github.com/soypat/godesim) -扩展/多变量ODE求解器框架，用于使用简单API进行基于事件的模拟.
...
* [acra](https://github.com/cossacklabs/acra) -网络加密代理，可保护基于数据库的应用程序免受数据泄漏：强大的选择性加密，SQL注入预防，入侵检测系统.
* [argon2-hashing](https://github.com/andskur/argon2-hashing) -Go的argon2程序包周围的轻包装，与Go的标准库Bcrypt和simple-scrypt程序包紧密对应.
...
* [BadActor](https://github.com/jaredfolkins/badactor) -基于fail2ban精神构建的内存中，应用程序驱动的监牢器.
...
* [firewalld-rest](https://github.com/prashantgupta24/firewalld-rest) -其余应用程序，用于在Linux服务器上动态更新防火墙规则.
...
* [lego](https://github.com/go-acme/lego) -Pure Go ACME客户端库和CLI工具（与Let&#39;s Encrypt一起使用）.
...
* [secure](https://github.com/unrolled/secure) -用于Go的HTTP中间件，有助于快速赢得安全性.
* [secureio](https://github.com/xaionaro-go/secureio) -基于XChaCha20-poly1305，ECDH和ED25519的io.ReadWriteCloser的密钥交换，认证，加密包装器和多路复用器.
* [simple-scrypt](https://github.com/elithrar/simple-scrypt) -具有简单，显而易见的API和内置自动成本校准功能的Scrypt软件包.
...
* [themis](https://github.com/cossacklabs/themis) -用于解决典型数据安全任务（安全数据存储，安全消息传递，零知识证明）的高级密码库，适用于14种语言，最适合多平台应用程序.
...
* [asn1](https://github.com/PromonLogicalis/asn1) -Gon的Asn.1 BER和DER编码库.
...
* [go-codec](https://github.com/ugorji/go) -高性能，功能丰富，惯用的编码，解码和rpc库，用于msgpack，cbor和json，具有基于运行时的OR代码生成支持.
* [go-lctree](https://github.com/sbourlon/go-lctree) -提供CLI和原语以进行序列化和反序列化 [LeetCode binary trees](https://support.leetcode.com/hc/en-us/articles/360011883654-What-does-1-null-2-3-mean-in-binary-tree-representation).
* [gogoprotobuf](https://github.com/gogo/protobuf) -Go与小工具一起使用的协议缓冲区.
* [goprotobuf](https://github.com/golang/protobuf) -以库和协议编译器插件的形式提供对Google协议缓冲区的支持.
* [jsoniter](https://github.com/json-iterator/go) -高性能100％兼容的“ encoding / json”替代品.
* [mapstructure](https://github.com/mitchellh/mapstructure) -Go库，用于将通用地图值解码为本地Go结构.
...
* [unitpacking](https://github.com/recolude/unitpacking) -将单位向量压缩到尽可能少的字节的库.

## Server Applications

* [algernon](https://github.com/xyproto/algernon) -HTTP / 2 Web服务器，内置对Lua，Markdown，GCSS和Amber的支持.
* [Caddy](https://github.com/caddyserver/caddy) -Caddy是一种易于配置和使用的备用HTTP / 2 Web服务器.
* [consul](https://www.consul.io/) -Consul是用于服务发现，监视和配置的工具.
* [cortex-tenant](https://github.com/blind-oracle/cortex-tenant) -Prometheus远程写代理，该代理基于度量标签添加了添加Cortex租户ID标头.
...
* [go-feature-flag](https://github.com/thomaspoignant/go-feature-flag) -一种功能标记解决方案，后端仅包含YAML文件（S3，GitHub，HTTP，本地文件...），无需安装服务器，只需在中央系统中添加文件并引用它即可.
* [go-proxy-cache](https://github.com/fabiocicerchia/go-proxy-cache) -使用Redis用Go编写的带有缓存的简单反向代理.
* [jackal](https://github.com/ortuman/jackal) -用Go语言编写的XMPP服务器.
* [lets-proxy2](https://github.com/rekby/lets-proxy2) -使用let-encrypt即时处理带有签发证书的https的反向代理.
...
* [protoxy](https://github.com/camgraff/protoxy) -将JSON请求主体转换为协议缓冲区的代理服务器.
...
* [SFTPGo](https://github.com/drakkan/sftpgo)  -功能齐全且可高度​​配置的SFTP服务器，具有可选的FTP / S和WebDAV支持. 它可以为本地文件系统和Cloud Storage后端（例如S3和Google Cloud Storage）提供服务.
* [simple-jwt-provider](https://github.com/leberKleber/simple-jwt-provider) -展示JWT的简单轻量级提供程序，支持登录，密码重置（通过邮件）和用户管理.
...
* [machine](https://github.com/whitaker-io/machine) -Go库用于编写和生成具有内置指标和可追溯性的流工作者.
* [stream](https://github.com/youthlin/stream) -像Java 8 Stream一样运行Stream：Filter / Map / FlatMap / Peek / Sorted / ForEach / Reduce ...
...
* [amber](https://github.com/eknkc/amber) -Amber是Go编程语言的优雅模板引擎.它受HAML和Jade的启发.
* [damsel](https://github.com/dskinner/damsel) -标记语言，具有通过css-selectors进行html概述的功能，可通过pkg html / template等扩展.
...
* [gofpdf](https://github.com/jung-kurt/gofpdf) -具有高级支持的文本，图形和图像的PDF文档生成器.
* [gospin](https://github.com/m1/gospin) -文章旋转和spintax / spinning语法引擎，可用于A / B，测试文本/文章并创建更自然的对话.
...
* [maroto](https://github.com/johnfercher/maroto)  -创建PDF的一种maroto方法.  Maroto受Bootstrap启发，并使用gofpdf. 快速而简单.
...
* [sprig](https://github.com/Masterminds/sprig) -Go模板的有用模板功能.
...
*测试框架
    ...
    * [biff](https://github.com/fulldump/biff) -分叉测试框架，与BDD兼容.
    ...
    * [gnomock](https://github.com/orlangure/gnomock) -在Docker中运行具有真实依赖项（数据库，缓存，甚至Kubernetes或AWS）的集成测试，而无需模拟.
    ...
    * [go-vcr](https://github.com/dnaeon/go-vcr) -记录并重放您的HTTP交互，以进行快速，确定性和准确的测试.
    * [goblin](https://github.com/franela/goblin) -Mocha像Go一样的测试框架.
    * [goc](https://github.com/qiniu/goc) -Goc是用于Go编程语言的全面覆盖测试系统.
    * [gocheck](http://labix.org/gocheck) -比getest更高级的测试框架.
    * [GoConvey](https://github.com/smartystreets/goconvey/) -具有Web UI和实时重载的BDD样式的框架.
    * [gocrest](https://github.com/corbym/gocrest) -Go断言可组合的类似于hamcrest的匹配器.
    * [godog](https://github.com/DATA-DOG/godog) -类似于Go的BDD框架的Cucumber或Behat.
    ...
    * [GoSpec](https://github.com/orfjackal/gospec) -用于Go编程语言的BDD风格的测试框架.
    ...
    * [Hamcrest](https://github.com/rdrdr/hamcrest) -声明性Matcher对象的流利框架，当将其应用于输入值时，会产生自描述结果.
    ...
    * [stop-and-go](https://github.com/elgohr/stop-and-go) -测试帮助程序的并发性.
    * [testcase](https://github.com/adamluzsi/testcase) -行为驱动开发的惯用测试框架.
    * [testfixtures](https://github.com/go-testfixtures/testfixtures) -Rails的帮助工具，例如测试装置，以测试数据库应用程序.
    ...
    * [trial](https://github.com/jgroeneveld/trial) -快速简单的可扩展断言，而无需引入很多样板.
...
* 嘲笑
    ...
    * [govcr](https://github.com/seborama/govcr) -用于Golang的HTTP模拟：记录和重播HTTP交互以进行离线测试.
    * [hoverfly](https://github.com/SpectoLabs/hoverfly) -HTTP（S）代理，用于通过可扩展的中间件和易于使用的CLI记录和模拟REST / SOAP API.
    ...
    * [mockit](https://github.com/pasdam/mockit)  -允许函数和方法轻松模拟，而无需定义新类型； 它类似于Java的Mockito.
...
*硒和浏览器控制工具.
    ...
    * [ggr](https://github.com/aerokube/ggr) -一种轻量级服务器，可将Selenium WebDriver请求路由和代理到多个Selenium集线器.
    * [playwright-go](https://github.com/mxschmitt/playwright-go) -浏览器自动化库，可通过单个API控制Chromium，Firefox和WebKit.
    * [rod](https://github.com/go-rod/rod) -一个Devtools驱动程序，可轻松实现Web自动化和抓取.
    * [selenoid](https://github.com/aerokube/selenoid) -备用Selenium集线器服务器，可在容器内启动浏览器.
...
*特定格式
    ...
    * [go-output-format](https://github.com/drewstinnett/go-output-format) -在您的命令行应用程序中，将go结构输出为多种格式（YAML / JSON / etc）.
    ...
    * [go-toml](https://github.com/pelletier/go-toml) -具有查询支持和方便的cli工具的TOML格式的库.
    * [go-vcard](https://github.com/emersion/go-vcard) -解析并格式化vCard.
    * [go-wildcard](https://github.com/IGLOU-EU/go-wildcard) -简单轻巧的通配符模式匹配.
    ...
    * [goq](https://github.com/andrewstuart/goq) -使用带有jQuery语法的struct标记对HTML进行声明式解封（使用GoQuery）.
    * [GoQuery](https://github.com/PuerkitoBio/goquery) -GoQuery为Go语言带来了一种类似于jQuery的语法和一组功能.
    ...
    * [html-to-markdown](https://github.com/JohannesKaufmann/html-to-markdown)  -将HTML转换为Markdown. 甚至可以在整个网站上使用，并且可以通过规则进行扩展.
    * [htmlquery](https://github.com/antchfx/htmlquery) -用于HTML的XPath查询包，可让您通过XPath表达式从HTML文档中提取数据或评估.
    * [inject](https://github.com/facebookgo/inject) -包装注入可提供基于反射的注入器.
    * [ltsv](https://github.com/Wing924/ltsv) -高性能 [LTSV (Labeled Tab Separated Value)](http://ltsv.org/) Go的阅读器.
    * [mxj](https://github.com/clbanning/mxj)  -将XML编码/解码为JSON或map [string] interface {}； 提取带有点符号路径和通配符的值. 替换x2j和j2x软件包.
    * [normalize](https://github.com/avito-tech/normalize) -清理，归一化和比较模糊文本.
    * [omniparser](https://github.com/jf-tech/omniparser) -一个通用的ETL库，该库以流方式解析文本输入（CSV / txt / JSON / XML / EDI / X12 / EDIFACT / etc），并使用数据驱动的模式将数据转换为JSON输出.
    * [pagser](https://github.com/foolin/pagser) -Pagser是一个简单，可扩展，可配置的解析和反序列化html页面，以基于golang爬虫的goquery和struct标签的结构.
    ...
    * [slug](https://github.com/gosimple/slug) -支持URL的Slugify具有多种语言支持.
...
* 公用事业
    ...
    * [regroup](https://github.com/oriser/regroup) -使用struct标签和自动解析功能将名为groups的正则表达式表达式匹配到go struct中.
...
* [appstore-sdk-go](https://github.com/Kachit/appstore-sdk-go) -用于AppStore Connect API的非官方Golang SDK.
...
* [cachet](https://github.com/andygrunwald/cachet) -前往的客户库 [Cachet (open source status page system)](https://cachethq.io/).
* [circleci](https://github.com/jszwedko/go-circleci) -Go客户端库，用于与CircleCI的API进行交互.
...
* [coinpaprika-go](https://github.com/coinpaprika/coinpaprika-api-go-client) -用于与Coinpaprika的API进行交互的客户端库.
* [device-check-go](https://github.com/rinchsan/device-check-go) -用于与之交互的客户端库 [iOS DeviceCheck API](https://developer.apple.com/documentation/devicecheck) v1.
...
* [fcm](https://github.com/maddevsio/fcm) -转到Firebase Cloud Messaging的库.
* [gads](https://github.com/emiddleton/gads) -Google Adwords非官方API.
* [gami](https://github.com/bit4bit/gami) -用于Asterisk Manager界面的库.
...
* [go-aws-news](https://github.com/circa10a/go-aws-news) -转到应用程序和库以从AWS中获取新功能.
* [go-chronos](https://github.com/axelspringer/go-chronos) -Go库，用于与 [Chronos](https://mesos.github.io/chronos/) 作业调度器
...
* [go-imgur](https://github.com/koffeinsource/go-imgur) -前往的客户库 [imgur](https://imgur.com)
* [go-jira](https://github.com/andygrunwald/go-jira) -前往的客户库 [Atlassian JIRA](https://www.atlassian.com/software/jira)
* [go-marathon](https://github.com/gambol99/go-marathon) -用于与Mesosphere的Marathon PAAS进行交互的库.
* [go-myanimelist](https://github.com/nstratos/go-myanimelist) -访问客户端库以访问 [MyAnimeList API](https://myanimelist.net/apiconfig/references/api/v2).
* [go-openproject](https://github.com/manuelbcd/go-openproject) -用于与之交互的客户端库 [OpenProject](https://docs.openproject.org/api/) 火.
* [go-postman-collection](https://github.com/rbretecher/go-postman-collection) -可以使用的模块 [Postman Collections](https://learning.getpostman.com/docs/postman/collections/creating-collections/) （与失眠症相容）.
...
* [go-unsplash](https://github.com/hbagdi/go-unsplash) -转到客户端库 [Unsplash.com](https://unsplash.com) 火.
* [go-xkcd](https://github.com/nishanths/go-xkcd) - Go client for the xkcd API.
* [go-yapla](https://git.iglou.eu/Production/go-yapla) -使用Yapla v2.0 API的客户端库.
* [gogtrends](https://github.com/groovili/gogtrends) -Google趋势非官方API.
...
* [gomalshare](https://github.com/MonaxGT/gomalshare) -转到库MalShare API [malshare.com](http://www.malshare.com/)
* [GoMusicBrainz](https://github.com/michiwend/gomusicbrainz) -转到MusicBrainz WS2客户端库.
* [google](https://github.com/google/google-api-go-client) -自动生成的Google API for Go.
...
* [google-email-audit-api](https://github.com/ngs/go-google-email-audit-api) -前往的客户库 [Google G Suite Email Audit API](https://developers.google.com/admin-sdk/email-audit/).
...
* [gostorm](https://github.com/jsgilmore/gostorm) -GoStorm是一个Go库，该库实现了在Go中编写与Storm外壳进行通信的Storm喷口和Bolt所需的通信协议.
...
* [lastpass-go](https://github.com/ansd/lastpass-go) -转到客户端库 [LastPass](https://www.lastpass.com/) 火.
* [libgoffi](https://github.com/clevabit/libgoffi) -本机库适配器工具箱 [libffi](http://sourceware.org/libffi/) 一体化
...
* [pushover](https://github.com/gregdel/pushover) -Pushover API的包装器.
...
* [simples3](https://github.com/rhnvrm/simples3) -使用REST和Go语言编写的V4签名，使AWS S3库变得简单而简洁.
* [slack](https://github.com/slack-go/slack) -Go中的Slack API.
...
* [TripAdvisor](https://github.com/mrbenosborne/tripadvisor-golang) -前往TripAdvisor API的包装器.
* [tumblr](https://github.com/mattcunningham/gumblr) -前往Tumblr v2 API的包装器.
* [twitter-scraper](https://github.com/n0madic/twitter-scraper) -删除没有身份验证和限制的Twitter Frontend API.
* [uptimerobot](https://github.com/bitfield/uptimerobot) -使用Uptime Robot v2 API的包装器和命令行客户端.
...
* [bleep](https://github.com/sinhashubham95/bleep) -在Go中对任何一组OS信号执行任何数量的操作.
...
* [command](https://github.com/txgruppi/command) -带有线程安全的串行和并行调度程序的Go的命令模式.
...
* [create-go-app](https://github.com/create-go-app/cli) -强大的CLI，可通过运行一个命令来创建具有后端（Golang），前端（JavaScript，TypeScript）和部署自动化（Ansible，Docker）的新产品就绪项目.
...
* [equalizer](https://github.com/reugn/equalizer) -Go的配额管理器和速率限制器集合.
...
* [git-time-metric](https://github.com/git-time-metric/gtm) -用于Git的简单，无缝，轻量级的时间跟踪.
...
* [go-clip](https://github.com/prashantgupta24/go-clip) -适用于Mac的简约剪贴板管理器.
* [go-convert](https://github.com/Eun/go-convert) -包go-convert使您可以将值转换为另一种类型.
* [go-countries](https://github.com/mikekonan/go-countries) -通过ISO-3166代码进行轻量级查找.
...
* [go-health](https://github.com/Talento90/go-health) -运行状况软件包简化了向服务添加运行状况检查的方式.
* [go-httpheader](https://github.com/mozillazg/go-httpheader) -Go库，用于将结构编码为Header字段.
* [go-lock](https://github.com/viney-shih/go-lock) -go-lock是一个锁库，实现了没有饥饿的读写互斥锁和读写trylock.
* [go-problemdetails](https://github.com/mvmaasakkers/go-problemdetails) -用于处理“问题详细信息”的软件包.
* [go-rate](https://github.com/beefsack/go-rate) -Go的限时限速器.
* [go-safe](https://github.com/kenkyu392/go-safe) -紧急安全沙箱.
...
* [go-type](https://github.com/mikekonan/go-types) -提供Go类型的库，用于存储/验证和传输ISO-4217，ISO-3166和其他类型.
* [goback](https://github.com/carlescere/goback) -使用简单的指数补偿程序包.
* [goctx](https://github.com/zerosnake0/goctx) -以高性能获得上下文价值.
...
* [gotenv](https://github.com/subosito/gotenv) -在Go中从`.env`或任何`io.Reader`加载环境变量.
...
* [immortal](https://github.com/immortal/immortal) -\ * nix跨平台（与OS无关）管理程序.
...
* [lets-go](https://github.com/aplescia-chwy/lets-go)  -Go模块，提供用于Cloud Native REST API开发的通用实用程序. 还包含特定于AWS的实用程序.
...
* [mergo](https://github.com/imdario/mergo)  -在Golang中合并结构和地图的助手. 对于配置默认值很有用，可避免混乱的if语句.
...
* [mongo-go-pagination](https://github.com/gobeam/mongo-go-pagination) -官方mongodb / mongo-go-driver程序包的Mongodb分页，它支持正常查询和聚合管道.
...
* [netbug](https://github.com/e-dard/netbug) -轻松地对您的服务进行远程配置.
...
* [ptr](https://github.com/gotidy/ptr) -提供用于简化从基本类型的常量创建指针的功能的程序包.
* [rclient](https://github.com/zpatrick/rclient) -REST API的可读，灵活，易于使用的客户端.
* [repeat](https://github.com/ssgreg/repeat) -实施不同的退避策略，对重试操作和心跳很有用.
* [request](https://github.com/mozillazg/request) -进行人类HTTP请求.
* [rerate](https://github.com/abo/rerate) -基于Redis的Go速率计数器和速率限制器.
...
* [retry](https://github.com/shafreeck/retry) -一个非常简单的库，以确保您可以完成工作.
...
* [scany](https://github.com/georgysavva/scany) -用于将数据从数据库扫描到Go结构等的库.
...
* [slicer](https://github.com/leaanthony/slicer) -使切片工作更加轻松.
...
* [statiks](https://github.com/janiltonmaciel/statiks) -快速，零配置的静态HTTP文件服务器.
* [Storm](https://github.com/asdine/storm) -用于BoltDB的简单而强大的工具包.
* [structs](https://github.com/PumpkinSeed/structs) -实现简单的功能来操纵结构.
* [tik](https://github.com/andy2046/tik) -Go的简单易用的计时轮包.
...
* [goflake](https://github.com/hart87/GoFlake)  -小型，可伸缩且无服务器的唯一ID生成器，用于分布式系统. 受Twitters Snowflake启发.
* [goid](https://github.com/jakehl/goid) -生成并解析符合RFC4122的V4 UUID.
* [gouid](https://github.com/twharmon/gouid) -仅分配一次即可生成加密安全的随机字符串ID.
...
* [uniq](https://gitlab.com/skilstak/code/go/uniq) -无需麻烦，安全，快速的命令唯一标识符.
...
* [wuid](https://github.com/edwingeng/wuid) -极其快速的唯一数字生成器，比UUID快10-135倍.
...
* [govalidator](https://github.com/thedevsaddam/govalidator)  -使用简单的规则验证Golang请求数据. 受到Laravel的请求验证的极大启发.
* [jio](https://github.com/faceair/jio) -jio是类似于json模式验证器 [joi](https://github.com/hapijs/joi).
* [ozzo-validation](https://github.com/go-ozzo/ozzo-validation) -支持使用普通代码结构中指定的可配置和可扩展验证规则（而不是struct标签）验证各种数据类型（结构，字符串，映射，切片等）.
...
* [go-git](https://github.com/go-git/go-git) -在纯Go中高度可扩展的Git实现.
...
* [libvlc-go](https://github.com/adrg/libvlc-go) -转到libvlc 2.X / 3.X / 4.X的绑定（由VLC媒体播放器使用）.
...
* [appy](https://github.com/appist/appy) -一个自以为是的高效Web框架，可帮助您更轻松地扩展业务.
* [Banjo](https://github.com/nsheremet/banjo) -Go的非常简单和快速的Web框架.
* [Beego](https://github.com/astaxie/beego) -beego是用于Go编程语言的开源，高性能Web框架.
* [Buffalo](http://gobuffalo.io) -提高Rails的生产力！
* [Confetti Framework](https://www.confetti-framework.com)  -Confetti是一种Go Web应用程序框架，具有表达力强，优雅的语法. 五彩纸屑结合了Laravel的优雅和Go的简单性.
* [Echo](https://github.com/labstack/echo) -高性能，简约的Go Web框架.
* [Fiber](https://github.com/gofiber/fiber) -在Fasthttp上构建Express.js启发的Web框架.
...
* [Gearbox](https://github.com/abahmed/gearbox) -用Go语言编写的Web框架，着重于高性能和内存优化.
* [Gin](https://github.com/gin-gonic/gin)  -Gin是用Go语言编写的Web框架！ 它具有类似于martini的API，性能更高，速度最高可提高40倍. 如果您需要性能和良好的生产率.
* [Ginrpc](https://github.com/xxjwxc/ginrpc) -杜松子酒参数自动绑定工具，杜松子酒rpc工具.
...
* [Golf](https://github.com/dinever/golf)  -Golf是Go的快速，简单和轻量级的微型Web框架. 它具有强大的功能，除了Go Standard Library之外没有其他依赖项.
* [Gondola](https://github.com/rainycape/gondola) -用于编写更快的网站，更快的Web框架.
* [gongular](https://github.com/mustafaakin/gongular) -具有输入映射/验证和（DI）依赖注入的Fast Go Web框架.
* [GoTuna](https://github.com/gotuna/gotuna) -Go的简约Web框架，带有多路复用器路由器，中间件，会话，模板，嵌入的视图和静态文件.
* [goweb](https://github.com/twharmon/goweb) -具有路由，websocket，日志记录，中间件，静态文件服务器（可选gzip）和自动TLS的Web框架.
* [Goyave](https://github.com/go-goyave/goyave) -功能齐全的REST API框架，旨在提供干净的代码和快速的开发，并具有强大的内置功能.
* [hiboot](https://github.com/hidevopsio/hiboot) -hiboot是具有自动配置和依赖项注入支持的高性能Web应用程序框架.
* [Macaron](https://github.com/go-macaron/macaron) -Macaron是Go中高效且模块化的设计Web框架.
...
* [patron](https://github.com/beatlabs/patron) -Patron是遵循最佳云实践并注重生产力的微服务框架.
...
* [vox](https://github.com/aisk/vox) -受Koa大力启发的golang网络人框架.
* [WebGo](https://github.com/bnkamalesh/webgo)  -用于构建Web应用程序的微框架； 与处理程序链接，中间件和上下文注入. 使用符合标准库的HTTP处理程序（即http.HandlerFunc）.
...
* [client-timing](https://github.com/posener/client-timing) -用于服务器计时标头的HTTP客户端.
* [CORS](https://github.com/rs/cors) -轻松将CORS功能添加到您的API.
* [formjson](https://github.com/rs/formjson) -透明地将JSON输入作为标准形式的POST处理.
...
* [ln-paywall](https://github.com/philippgille/ln-paywall) -使用中间件，通过Lightning Network（Bitcoin）根据每个请求从API获利.
...
* [chain](https://github.com/codemodus/chain) 处理程序包装器链与范围内的数据（基于net / context的“中间件”）.
...
* [mediary](https://github.com/HereMobilityDevelopers/mediary) -将拦截器添加到“ http.Client”中，以允许对请求/响应进行转储/整形/跟踪/....
...
* [alien](https://github.com/gernest/alien) -轻巧，快速的来自外部空间的http路由器.
...
* [Goji](https://github.com/goji/goji) -Goji是一种简单灵活的HTTP请求多路复用器，支持`net / context`.
* [goroute](https://github.com/goroute/route) -简单但功能强大的HTTP请求多路复用器.
* [GoRouter](https://github.com/vardius/gorouter) -GoRouter是一个服务器/ API微型框架，HTTP请求路由器，多路复用器和多路复用器，为请求路由器提供了支持“ net / context”的中间件.
* [gowww/router](https://github.com/gowww/router) -闪电般的快速HTTP路由器，与net / http.Handler接口完全兼容.
* [httprouter](https://github.com/julienschmidt/httprouter)  -高性能路由器. 使用此代码和标准的http处理程序可以形成一个非常高性能的Web框架.
* [httptreemux](https://github.com/dimfeld/httptreemux)  -用于Go的高速，灵活的基于树的HTTP路由器. 来自httprouter的启发.
* [lars](https://github.com/go-playground/lars) -是Go的轻量级，快速和可扩展的零分配HTTP路由器，用于创建可自定义的框架.
...
* [vestigo](https://github.com/husobee/vestigo) -用于go Web应用程序的高性能，独立的，HTTP兼容的URL路由器.
* [violetear](https://github.com/nbari/violetear) -转到HTTP路由器.
* [xmux](https://github.com/rs/xmux) -基于`httprouter`并支持`net / context`的高性能多路复用器.
...
* [tinygo](https://github.com/tinygo-org/tinygo)  -去小地方编译. 微控制器，WebAssembly和命令行工具. 基于LLVM.
...
* [xml2map](https://github.com/sbabiv/xml2map) -XML到MAP转换器编写的Golang转换器.
...
* [golines](https://github.com/segmentio/golines) -格式化程序，可自动缩短Go代码中的长行.
...
* [unused](https://github.com/dominikh/go-tools/tree/master/cmd/unused) -未使用的检查转到代码以查找未使用的常量，变量，函数和类型.
...
* [go-language-server](https://github.com/theia-ide/go-language-server) -将VSCode go扩展名转换为支持language-server-protocol的语言服务器的包装器.
* [go-mode](https://github.com/dominikh/go-mode.el) -GNU / Emacs的进入模式.
* [go-plus](https://github.com/joefitzgerald/go-plus) -Go（Golang）用于Atom的程序包，可添加自动完成，格式设置，语法检查，整理和审核.
...
* [vim-go](https://github.com/fatih/vim-go) -去为Vim开发插件.
* [vscode-go](https://github.com/golang/vscode-go) -Visual Studio Code（VS Code）扩展，提供对Go语言的支持.
* [Watch](https://github.com/eaburns/Watch) -在文件更改中以极致优势运行命令.
...
* [colorgo](https://github.com/songgao/colorgo) -用于`go build`输出的彩色`go`命令周围的包装器.
* [depth](https://github.com/KyleBanks/depth) - Visualize dependency trees of any package by analyzing imports.
* [docs](https://github.com/go-oas/docs) -自动为GO项目生成RESTful API文档-与Open API Specification标准保持一致.
* [generator-go-lang](https://github.com/axelspringer/generator-go-lang) - 一种 [Yeoman](http://yeoman.io) 生成器以启动新的Go项目.
...
* [go-pkg-complete](https://github.com/skelterjohn/go-pkg-complete) -Go和WGO的Bash完成.
...
* [gomodrun](https://github.com/dustinblackman/gomodrun/) -执行和缓存go.mod文件中包含的二进制文件的Go工具.
* [gothanks](https://github.com/psampaz/gothanks) -GoThanks自动为您的go.mod github依赖项加注星标，以此方式向其维护者发送一些爱意.
* [igo](https://github.com/rocketlaunchr/igo) -可以进行转译的igo（Go语言的新语言功能！）
* [OctoLinker](https://github.com/OctoLinker/browser-extension) -使用适用于GitHub的OctoLinker浏览器扩展有效地浏览go文件.
* [richgo](https://github.com/kyoh86/richgo) -丰富带有文本装饰的“ go test”输出.
* [roumon](https://github.com/becheran/roumon) -通过命令行界面监视所有活动goroutine的当前状态.
* [rts](https://github.com/galeone/rts)  -RTS：对结构的响应. 从服务器响应生成Go结构.
* [typex](https://github.com/dtgorski/typex) -检查Go类型及其传递依存关系，或者将结果导出为TypeScript值对象（或类型）声明.
...
* [abbreviate](https://github.com/dnnrly/abbreviate) -abbreviate是一种工具，可将长字符串转换为带有可配置分隔符的较短字符串，例如将分支名称嵌入到部署堆栈ID中.
* [aptly](https://github.com/smira/aptly) -恰如其分地是Debian仓库管理工具.
...
* [cassowary](https://github.com/rogerwelin/cassowary) -用Go语言编写的现代跨平台HTTP负载测试工具.
* [DepCharge](https://github.com/centerorbit/depcharge) -帮助协调跨大型项目中许多依赖项的命令执行.
* [docker-go-mingw](https://github.com/x1unix/docker-go-mingw) -用于使用MinGW工具链为Windows构建Go二进制文件的Docker映像.
* [Dockerfile-Generator](https://github.com/ozankasikci/dockerfile-generator) -一个go库和一个可执行文件，它使用各种输入通道生成有效的Dockerfile.
* [dogo](https://github.com/liudng/dogo) -监视源文件中的更改，并自动进行编译和运行（重新启动）.
...
* [gaia](https://github.com/gaia-pipeline/gaia) -以任何编程语言构建功能强大的管道.
* [ghorg](https://github.com/gabrie30/ghorg) -快速将整个组织/用户存储库克隆到一个目录中-支持GitHub，GitLab，Gitea和Bitbucket.
...
* [s3gof3r](https://github.com/rlmcpherson/s3gof3r) -小型实用程序/库经过优化，可用于将大型对象高速传输到Amazon S3和从Amazon S3传输出来.
...
* [terraform-provider-openapi](https://github.com/dikhan/terraform-provider-openapi) -Terraform provider插件，它基于包含公开API定义的OpenAPI文档（以前称为swagger文件）在运行时动态地进行自我配置.
* [traefik](https://github.com/containous/traefik) -反向代理和负载均衡器，支持多个后端.
* [trubka](https://github.com/xitonix/trubka) -一个CLI工具，用于管理Apache Kafka集群并对其进行故障排除，具有通常向/从Kafka发布/使用协议缓冲区和纯文本事件的功能.
...
* [Wide](https://wide.b3log.org/login) -使用Golang的团队的基于Web的IDE.
...
* [Circuit](https://github.com/gocircuit/circuit) -Circuit是可编程的平台即服务（PaaS）和/或基础架构即服务（IaaS），用于管理，发现，同步和编排组成云应用程序的服务和主机.
...
* [Go Package Store](https://github.com/shurcooL/Go-Package-Store) -在您的GOPATH中显示Go软件包更新的应用程序.
...
* [Guora](https://github.com/meloalright/guora) -用Go编写的自托管Quora之类的Web应用程序.
...
* [limetext](https://limetext.github.io) -Lime Text是一款功能强大且优雅的文本编辑器，主要是在Go中开发的，旨在成为Sublime Text的免费开源软件后继产品.
* [LiteIDE](https://github.com/visualfc/liteide) -LiteIDE是一个简单的，开源的，跨平台的Go IDE.
* [mockingjay](https://github.com/quii/mockingjay-server)  -伪造一个配置文件中的HTTP服务器和消费者驱动的合同. 您还可以使服务器随机出现异常行为，以帮助进行更实际的性能测试.
* [myLG](https://github.com/mehrdadrad/mylg) -用Go语言编写的命令行网络诊断工具.
...
* [Orbit](https://github.com/gulien/orbit) -一个简单的工具，用于运行命令并从模板生成文件.
...
* [scc](https://github.com/boyter/scc) -Sloc Cloc and Code，一个非常快速，准确的代码计数器，具有复杂度计算和COCOMO估计.
...
* [Snitch](https://github.com/lucasgomide/snitch) -当有人通过Tsuru部署了任何应用程序时，通知您的团队和许多工具的简单方法.
...
* [tcpdog](https://github.com/mehrdadrad/tcpdog) -基于eBPF的TCP可观察性.
* [tcpprobe](https://github.com/mehrdadrad/tcpprobe) -用于网络性能和路径监视（包括套接字统计信息）的TCP工具.
...
* [vaku](https://github.com/lingrino/vaku) -Vault中基于文件夹的功能（例如复制，移动和搜索）的CLI和API.
...
* [woke](https://github.com/get-woke/woke) -在源代码中检测非包容性语言.
...
* [go-benchmark-app](https://github.com/mrLSD/go-benchmark-app)  -强大的HTTP基准工具与Аb，Wrk，Siege工具混合在一起. 收集统计数据和各种参数以用于基准测试和比较结果.
* [go-benchmarks](https://github.com/tylertreat/go-benchmarks)  -很少有其他Go微基准测试. 将某些语言功能与替代方法进行比较.
* [go-http-routing-benchmark](https://github.com/julienschmidt/go-http-routing-benchmark) -转到HTTP请求路由器基准测试和比较.
* [go-json-benchmark](https://github.com/zerosnake0/go-json-benchmark) -进行JSON基准测试.
* [go-ml-benchmarks](https://github.com/nikolaydubina/go-ml-benchmarks) -Go中机器学习推理的基准.
...
* [For the Love of Go](https://bitfieldconsulting.com/books) -面向Go初学者的一系列入门书籍.
...
* [How To Code in Go eBook](https://www.digitalocean.com/community/books/how-to-code-in-go-ebook) -面向初学者的600页Go简介.
...
* [Free Gophers Pack](https://github.com/MariaLetta/free-gophers-pack) -玛丽亚·莱塔（Maria Letta）的Gopher图形包，包括插图和矢量和栅格中的情感人物.
...
* [awesome-awesomeness](https://github.com/bayandin/awesome-awesomeness) -其他令人敬畏的清单.
...
* [Go Code Club](https://www.youtube.com/watch?v=nvoIPQYdx9g&list=PLEcwzBXTPUE_YQR7R0BRtHBYJ0LN3Y0i3) -一群Gophers每周阅读和讨论一个不同的Go项目.
...
* [Go Report Card](https://goreportcard.com) -Go套票的成绩单.
* [go.dev](https://go.dev/) -Go开发人员的中心.
* [gocryforhelp](https://github.com/ninedraft/gocryforhelp)  -需要帮助的Go项目的集合. 在Go中开始您的开源方式的好地方.
...
* [Golang Resources](https://golangresources.com) -精选最好的文章，练习，讲座和视频来学习Go.
...
* [json2go](https://m-zajac.github.io/json2go) -先进的JSON到Go结构转换-在线工具.
...
* [Learn Go Programming](https://blog.learngoprogramming.com) -学习带有插图的围棋概念.
* [Lille Gophers](https://lille-gophers.loscrackitos.codes/) -法国里尔的Golang会谈社区（[@LilleGophers](https://twitter.com/LilleGophers)).
...
* [Debugged.it Go patterns](https://github.com/haveyoudebuggedit/go-patterns) -带有立即运行示例的高级Go模式.
...
* [Go Playground for iOS](https://codeplayground.app) -在您的移动设备上交互式地编辑和播放Go片段.
* [Go WebAssembly Tutorial - Building a Simple Calculator](https://tutorialedge.net/golang/go-webassembly-tutorial/)
* [go-patterns](https://github.com/tmrts/go-patterns) -Go设计模式，配方和惯用语的精选清单.
* [goapp](https://github.com/bnkamalesh/goapp) -构建和开发Go Web应用程序/服务的自以为是的准则.
...
* [GopherSnippets](https://gophersnippets.com/) -带有Go编程语言的测试和可测试示例的代码片段.
* [Hackr.io](https://hackr.io/tutorials/learn-golang) -从golang编程社区提交和投票的最佳在线golang教程中学习Go.
* [How to Benchmark: dbq vs sqlx vs GORM](https://medium.com/@rocketlaunchr.cloud/how-to-benchmark-dbq-vs-sqlx-vs-gorm-e814caacecb5)  -了解如何在Go中进行基准测试. 作为案例研究，我们将对dbq，sqlx和GORM进行基准测试.
...
* [Learn Go with 1000+ Exercises](https://github.com/inancgumus/learngo) -通过数以千计的示例，练习和测验学习Go.
* [Learn Go with TDD](https://github.com/quii/learn-go-with-tests) -通过测试驱动的开发来学习Go.
