...
* [jeff](https://github.com/abraithwaite/jeff) - 简单，灵活，安全和惯用的Web会话管理，带有可插拔的后端.
...
* [sessionup](https://github.com/swithek/sessionup) - 简单而有效的HTTP会话管理和识别包.
...
* [Kelp](https://github.com/stellar/kelp) - 官方交易和市场制造机器人 [Stellar](https://www.stellar.org/) DEX. Works out-of-the-box, written in Golang, compatible with centralized exchanges and custom trading strategies.
...
* [slackscot](https://github.com/alexandre-normand/slackscot) - 构建Slack机器人的另一个框架.
...
* [cmdr](https://github.com/hedzr/cmdr) -  POSIX / GNU样式，类似getopt的命令行UI Go库.
...
* [Dnote](https://github.com/dnote/dnote) - 面向开发人员的简单端到端加密笔记本.
...
* [harvester](https://github.com/beatlabs/harvester) -  Harvester，易于使用的静态和动态配置包支持播种，环境变量和Consul集成.
...
* [koanf](https://github.com/knadh/koanf)   - 轻量级，可扩展的库，用于在Go应用程序中读取配置.  内置支持JSON，TOML，YAML，env，命令行.
* [konfig](https://github.com/lalamove/konfig) - 针对分布式处理时代的Go的可组合，可观察和高性能配置处理.
* [mini](https://github.com/sasbury/mini) -  Golang包解析这种风格的配置文件.
* [nasermirzaei89/env](https://github.com/nasermirzaei89/env) - 读取环境变量的简单有用包.
...
* [gormigrate](https://github.com/go-gormigrate/gormigrate) - 数据库模式迁移有助于Gorm ORM.
* [migrate](https://github.com/golang-migrate/migrate)   - 数据库迁移.  CLI和Golang库.
* [migrator](https://github.com/lopezator/migrator) - 死简单Go数据库迁移库.
* [pravasan](https://github.com/pravasan/pravasan) - 简单迁移工具 - 目前用于MySQL，但计划很快支持Postgres，SQLite，MongoDB等.
* [skeema](https://github.com/skeema/skeema) - 用于MySQL的Pure-SQL模式管理系统，支持分片和外部在线模式更改工具.
...
* NoSQL数据库
    ...
    * [godis](https://github.com/piaohao/godis) - 由jedis启发的golang的redis客户端实现.
...
* [go-sundheit](https://github.com/AppsFlyer/go-sundheit) - 构建的库，用于为golang服务定义异步服务运行状况检查提供支持.
...
* [mailgun-go](https://github.com/mailgun/mailgun-go) - 使用Mailgun API发送邮件库.
...
* [go-exiftool](https://github.com/barasher/go-exiftool) - 用于ExifTool的绑定，ExifTool是用于从文件（图片，PDF，办公室......）中提取尽可能多的元数据（EXIF，IPTC，...）的着名库.
...
* [gonet](https://github.com/xtaci/gonet) - 用golang实现的游戏服务器骨架.
...
* [breaker](https://github.com/kamilsk/breaker) - 灵活的机制，使执行流程可中断.
...
* [Hunch](https://github.com/AaronJan/Hunch) -  Hunch提供的功能包括：`All`，`First`，`Retry`，`Waterfall`等，使异步流控制更加直观.
...
* [routine](https://github.com/x-mod/routine) - 使用上下文，支持进行例行控制：Main，Go，Pool和一些有用的Executors.
...
* [gltf](https://github.com/qmuntal/gltf) - 高效，强大的glTF 2.0读写器，编写器和验证器.
...
* [go-jsonerror](https://github.com/ddymko/go-jsonerror) -  Go-JsonError允许我们轻松创建遵循JsonApi规范的json响应错误.
...
* [Goptuna](https://github.com/c-bata/goptuna)   - 用Go编写的黑盒函数的贝叶斯优化框架.  一切都会得到优化.
...
* [Commander](https://github.com/jeroenrinzema/commander) - 支持各种“方言”的高级事件驱动的消费者/制作者，例如Apache Kafka.
...
* [linker](https://github.com/logrange/linker) - 基于反射的依赖注入和控制库的反转以及组件生命周期支持.
...
* [modern-go-application](https://github.com/sagikazarmark/modern-go-application) - 应用现代实践的应用程序样板和示例.
...
* [datacounter](https://github.com/miolini/datacounter) -  Go for reader / writer / http.ResponseWriter.
* [ffmt](https://github.com/go-ffmt/ffmt) - 为人类提供美化数据显示.
* [ghorg](https://github.com/gabrie30/ghorg) - 将整个组织/用户存储库快速克隆到一个目录中 - 支持GitHub，GitLab和Bitbucket.
...
* [gotoprom](https://github.com/cabify/gotoprom) - 适用于Prometheus官方客户端的类型安全度量标准构建器包装库.
...
* [stats](https://github.com/go-playground/stats) - 监控Go MemStats +系统统计数据，如内存，交换和CPU，并通过UDP发送到任何你想要的日志记录等...
...
* [porter2](https://github.com/zhenjl/porter2) - 波特2票真的很快.
...
* [ethernet](https://github.com/mdlayher/ethernet) - 包以太网实现IEEE 802.3以太网II帧和IEEE 802.1Q VLAN标记的编组和解组.
...
* [certificates](https://github.com/mvmaasakkers/certificates) - 用于生成tls证书的固定工具.
...
* [pletter](https://github.com/vimeda/pletter) - 为消息代理程序包装原型消息的标准方法.
...
* [SFTPGo](https://github.com/drakkan/sftpgo) - 全功能和高度可配置的SFTP服务器软件.
...
*测试框架
    * [apitest](https://apitest.dev) - 用于基于REST的服务或HTTP处理程序的简单且可扩展的行为测试库，支持模拟外部http调用和呈现序列图.
    ...
    * [gosuite](https://github.com/pavlo/gosuite) - 通过利用Go1.7的Subtests为轻量级测试套件提供设置/拆卸设施来“测试”.
...
* Selenium和浏览器控制工具.
    ...
    * [ggr](https://github.com/aerokube/ggr) - 轻量级服务器，用于将Selenium WebDriver请求路由和代理到多个Selenium集线器.
...
*特定格式
    ...
    * [encdec](https://github.com/mickep76/encdec) -  Package为编码器和解码器提供通用接口.
...
* [go-here](https://github.com/abdullahselek/go-here) - 围绕基于HERE位置的API转到客户端库.
...
* [nanoid](https://github.com/aidarkhanov/nanoid) - 一个小巧高效的Go唯一字符串ID生成器.
...
* [gmf](https://github.com/3d0c/gmf) - 为FFmpeg的\ *库提供绑定.
...
* [patron](https://github.com/beatlabs/patron) -  Patron是一个遵循最佳云实践的微服务框架，专注于生产力.
...
* [goroute](https://github.com/goroute/route) - 简单而强大的HTTP请求多路复用器.
...
* [go-checkstyle](https://github.com/qiniu/checkstyle)   -  checkstyle是一种样式检查工具，如java checkstyle.  这个工具的灵感来自java checkstyle，golint.  该风格提到了Go Code Review Comments中的一些要点.
...
* [go-plus](https://github.com/joefitzgerald/go-plus) - 用于添加自动完成，格式化，语法检查，Linting和Vetting的Atom的Go（Golang）包.
...
* [Dockerfile-Generator](https://github.com/ozankasikci/dockerfile-generator) -  go库和可执行文件，使用各种输入通道生成有效的Dockerfiles.
...
* [limetext](http://limetext.org/) -  Lime Text是一个强大而优雅的文本编辑器，主要在Go中开发，旨在成为Sublime Text的免费开源软件继承者.
...
* [gophers](https://github.com/ashleymcnamara/gophers) -  Ashley McNamara的Gopher作品.
...
* [Learning Golang - From zero to hero](https://milapneupane.com.np/2019/07/06/learning-golang-from-zero-to-hero/) - 初学者入门golang.
...
* [Your basic Go](http://yourbasic.org/golang) - 大量的教程和如何使用.
