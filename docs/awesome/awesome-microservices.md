...
- [OpenWhisk](http://openwhisk.org/) -无服务器的开源云平台，可执行各种规模的事件响应功能.
...
- [GraalVM](https://www.graalvm.org/) -高性能运行时，可显着改善应用程序的性能和效率，是微服务的理想选择.
...
- [Helidon](https://helidon.io/) -收集用于编写在由Netty支持的快速Web核心上运行的微服务的Java库.
...
- [Orbit](https://github.com/orbit/orbit) -JVM语言的现代框架，可以更轻松地构建和维护分布式可伸缩的在线服务.
...
- [Sogou Workflow](https://github.com/sogou/workflow) -企业级编程引擎，旨在满足大多数后端开发要求.
- [ULib](https://github.com/stefanocasazza/ULib) -用于编写C ++应用程序的高度优化的类框架.

### CSharp

- [Awesome Microservices .NET Core](https://github.com/mjebrahimi/Awesome-Microservices-NetCore) ：star：-一系列很棒的培训系列，文章，视频，书籍，课程，示例项目以及.NET Core中微服务工具的​​集合.
...
- [Tye](https://github.com/dotnet/tye)  -Tye是使开发，测试和部署微服务和分布式应用程序更加容易的工具.  Project Tye包括一个本地协调器，以使开发微服务变得更加容易，并且能够以最少的配置将微服务部署到Kubernetes.
...
- [Echo](https://echo.labstack.com/)  -用于Go的快速而荒唐的HTTP服务器框架. 速度比其他产品快10倍.
- [Enduro/X ASG](https://github.com/endurox-dev/endurox-go)  -Go的Enduro / X绑定允许使用Go语言有效地编写基于XATMI的微服务. 使用Unix内核IPC（队列）进行快速的进程通信.
- [Fiber](https://github.com/gofiber/fiber)  -基于Fasthttp（Go最快的HTTP引擎）构建的Express启发式Web框架. 旨在简化零内存分配和性能的情况，以便快速开发.
- [Gin](https://github.com/gin-gonic/gin)  -Gin是用Go（Golang）编写的HTTP Web框架. 它具有类似于Martini的API，具有更好的性能，速度可提高40倍.
...
- [Goji](https://goji.io/) -Go的最小和灵活的请求多路复用器.
- [Go Chassis](https://github.com/go-chassis/go-chassis) -Go中微服务快速开发的框架，该框架易于与某些云生态系统集成.
...
- [ActiveRPC](https://rpc.activej.io) -轻量级且快速的库，用于复杂的高负载分布式应用程序和类似Memcached的解决方案.
...
- [Cro](http://cro.services/) -用于使用Perl 6创建反应式分布式系统的库.
- [Mojolicious](https://mojolicious.org/) -Perl的下一代Web框架.
...
- [FastAPI](https://fastapi.tiangolo.com/) -基于标准Python类型提示，使用Python 3.6+构建API的现代，快速（高性能）Web框架.
- [Flask](http://flask.pocoo.org/) -基于Werkzeug和Jinja 2的微服务Python框架.
- [Nameko](https://github.com/onefinestay/nameko) -用于构建微服务的Python框架.
- [Sanic](https://github.com/channelcat/sanic) -Sanic是一款类似于Flask的Python 3.5+ Web服务器，其编写速度很快.
...
- [Grape](https://github.com/ruby-grape/grape) -用于创建类似REST的API的自以为是的框架
...
- [Actix](https://actix.rs/) -用于Rust的强大，实用，极其快速的Web框架.
...
- [Bunkerized-nginx](https://github.com/bunkerity/bunkerized-nginx) -默认情况下，Web应用程序托管和反向代理安全.
...
- [HAProxy](https://github.com/haproxy/haproxy) -可靠的高性能TCP / HTTP负载平衡器.
...
- [Kuma](https://kuma.io/) -适用于服务网格和微服务的与平台无关的开源控制平面.
...
- [Zuul](https://github.com/Netflix/zuul) -边缘服务，可提供动态路由，监视，弹性，安全性等.
...
-[AWS Step Functions！[c]]（https://aws.amazon.com/step-functions/）-使用可视化工作流程来协调分布式应用程序和微服务的组件.
...
- [Temporal](https://github.com/temporalio/temporal) -开源微服务编排平台，用于以任意规模运行关键任务代码.
- [Zeebe](https://zeebe.io/) -定义，协调和监视跨微服务的业务流程.
...
- [Crossbar](https://github.com/crossbario/crossbar)  -用于分布式和微服务应用程序的开源网络平台. 它实现了开放的Web应用程序消息传递协议（WAMP）.
...
- [Reactor.io](http://projectreactor.io) -第二代Reactive库，用于根据Reactive Streams规范在JVM上构建非阻塞应用程序.
...
- [Raft Consensus](http://raftconsensus.github.io/)  -易于理解的共识算法. 在容错性和性能上与Paxos等效.
- [Resilience4j](https://github.com/resilience4j/resilience4j) -为Java8和功能编程设计的容错库.
- [Resilient HTTP](http://resilient-http.github.io/) -为分布式系统设计的智能HTTP客户端，具有超强的功能，例如容错，动态服务器发现，自动平衡和反应式恢复.

### Security

- [Databunker](https://paranoidguy.com/) -建立符合GDPR的个人身份信息（PII）存储服务.
...
- [Avro](https://avro.apache.org/) -Apache数据序列化系统，以紧凑，快速的二进制数据格式提供丰富的数据结构.
...
- [Hikaku](https://github.com/codecentric/hikaku) -测试REST-API的实现是否符合其规范的库.
- [Mitmproxy](https://mitmproxy.org/) -交互式控制台程序，允许拦截，检查，修改和重播流量.
...
- [QUIC-Transport](https://tools.ietf.org/html/draft-ietf-quic-transport-27) -基于UDP的多路传输和安全传输.
...
- [How Do Committees Invent?](http://www.melconway.com/Home/pdf/committees.pdf) ：small_orange_diamond： <sup>PDF</sup> -Melvin E. Conway，《 Datamation》杂志1968年.定义Conway定律的原始文章.
- [Service per Team](https://microservices.io/patterns/decomposition/service-per-team.html)  -每个团队负责一个或多个业务职能（例如业务职能）. 一个团队拥有一个由一个或多个模块组成的代码库. 其代码库的大小应确保不超过团队的认知能力. 团队将其代码部署为一项或多项服务. 除非确实需要多种服务，否则团队应该只提供一项服务.
- [Start with Team Cognitive Load - Team Topologies](https://www.youtube.com/watch?v=haejb5rzKsM)  ：small_red_triangle： <sup>YT</sup> -DOES19伦敦.  “整体与微服务”的争论通常集中在技术方面，而忽略了战略和团队动力. 聪明的组织开始以团队的认知负荷作为现代软件的指导原则，而不是技术. 在本次演讲中，我们将通过实际案例说明如何以及为什么进行解释.
...
- [Microservices Resource Guide](http://martinfowler.com/microservices/) -Martin Fowler的文章，视频，书籍和播客的选择可以教您更多有关微服务体系结构样式的知识.
- [Microservice Patterns](http://microservices.io/) -微服务架构模式和最佳实践.
- [Microservice Antipatterns and Pitfalls](https://www.oreilly.com/ideas/microservices-antipatterns-and-pitfalls) -微服务通常是已知的反模式和陷阱.
