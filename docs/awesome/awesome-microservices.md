...
- [Light-4j](https://github.com/networknt/light-4j) -高吞吐率，低延迟，较小的内存占用以及更高效的微服务平台.
- [Micronaut](http://micronaut.io/) -基于JVM的现代全栈框架，用于构建模块化，易于测试的微服务应用程序.
- [Microserver](https://github.com/aol/micro-server) -Java 8本机，零配置，基于标准，经过战斗加固的库，可以运行Java REST微服务.
...
- [CoAP](http://coap.technology/impls.html) -受限的应用协议实施.
...
- [Sogou Workflow](https://github.com/sogou/workflow/blob/master/README_en.md) -企业级编程引擎，旨在满足大多数后端开发要求.
...
- [Fiber](https://github.com/gofiber/fiber)  -建立在Fasthttp（Go最快的HTTP引擎）之上的Express启发性Web框架. 旨在简化零内存分配和性能的情况，以便快速开发.
- [Gin](https://github.com/gin-gonic/gin)  -Gin是用Go（Golang）编写的HTTP Web框架. 它具有类似于Martini的API，具有更好的性能，快40倍.
...
- [Http4s](http://http4s.org/) -用于HTTP的最小，惯用的Scala接口
...
- [Kuma](https://kuma.io/) -服务网格和微服务的平台不可知开源控制平面.
...
- [Schedulix](http://www.schedulix.org/en) 开源企业工作计划系统为高级系统环境中的IT流程的专业自动化制定了突破性的标准.
...
- [RabbitMQ](https://www.rabbitmq.com/) -开源的基于Erlang的消息代理，可以正常工作.
...
- [Reactive Kafka](https://github.com/softwaremill/reactive-kafka) -适用于Apache Kafka的反应式API.
...
- [QUIC-WG](https://quicwg.org/) -被授权为互联网提供下一个传输协议的IETF工作组.
...
- [HAL](https://tools.ietf.org/html/draft-kelly-json-hal-07)  -JSON超文本应用程序语言（HAL）是一种标准，用于建立用于表达超媒体控件（如链接）和JSON的约定. <sup>草案</sup>
...
- [Service per Team](https://microservices.io/patterns/decomposition/service-per-team.html)  -每个团队负责一个或多个业务职能（例如业务职能）. 团队拥有由一个或多个模块组成的代码库. 其代码库的大小应确保不超过团队的认知能力. 团队将其代码部署为一项或多项服务. 除非确实需要多种服务，否则团队应该只提供一项服务.
