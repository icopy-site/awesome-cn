...
* [ASP.NET Boilerplate](https://github.com/aspnetboilerplate/aspnetboilerplate)  -ASP.NET Boilerplate是一个通用应用程序框架，专门为新的现代Web应用程序设计. 它使用已经熟悉的工具，并围绕它们实施最佳实践，以为您提供SOLID开发经验.
...
* [AsyncEx](https://github.com/StephenCleary/AsyncEx) -用于异步/等待的帮助程序库.
...
* [ASP.NET MVC](https://github.com/dotnet/aspnetcore/tree/master/src/Mvc) -模型视图控制器框架，用于构建具有清晰关注点的动态网站，包括合并的MVC，Web API和带有Razor的Web页面.
* [Butterfly Server .NET](https://github.com/firesharkstudios/butterfly-server-dotnet)  -允许以最少的精力构建实时Web应用程序和本机应用程序. 定义一个Web API和Subscription API，它们可以自动在连接的客户端之间同步数据集.
...
* [DNTFrameworkCore](https://github.com/rabbal/DNTFrameworkCore) -基于ASP.NET Core的轻量级和可扩展的基础结构，用于构建高质量的Web应用程序.
...
* [Finbuckle.MultiTenant](https://github.com/Finbuckle/Finbuckle.MultiTenant)  -Finbuckle.MultiTenant是一个用于ASP.NET 2.0+的用于支持多租户的.NET标准库. 它提供了用于租户解析，每个租户应用程序配置和每个租户数据隔离的功能.
...
* [RService.io](https://github.com/Stoom/RService.IO) -注重速度和易用性的ASP.Net Core RESTful微服务框架.
...
* [X.PagedList](https://github.com/dncuug/X.PagedList) -用于轻松分页ASP.NET/ASP.NET Core中的IEnumerable / IQueryable的库.

### Application Templates
* [.NET Boxed](https://github.com/Dotnet-Boxed/Templates)  -包含电池的项目模板，为您提供了所需的最少代码量. 包括ASP.NET Core API和GraphQL模板.
...
* [ASP.NET-MVC-Template](https://github.com/NikolayIT/ASP.NET-MVC-Template) -ASP.NET MVC 5和ASP.NET Core的现成模板，已修复存储库，服务，模型映射以及DI和StyleCop警告.
* [AddFeatureFolders](https://github.com/OdeToCode/AddFeatureFolders) -在ASP.NET Core中为MVC控制器和视图启用功能文件夹.
* [Angular Visual Studio Webpack Starter](https://github.com/damienbod/AngularWebpackVisualStudio)  -Webpack，Visual Studio，ASP.NET Core和Angular的模板. 该应用程序的客户端和服务器端都在一个ASP.NET Core项目中实现，这使部署更加容易.
* [CleanArchitecture](https://github.com/ardalis/CleanArchitecture)  -使用ASP.NET Core进行“清洁体系结构”的起点. 对于相同的松耦合，依赖关系反转的体系结构，Clean Architecture只是一系列名称中的最新名称. 您还会发现它名为六角形，端口和适配器或洋葱结构.
...
* [Casbin.NET](https://github.com/casbin-net/Casbin.NET) -支持C中的访问控制模型（如ACL，RBAC，ABAC）的授权库#
...
* [stormpath-sdk](https://github.com/stormpath/stormpath-sdk-dotnet) - 建造 [simple, secure web applications](https://github.com/stormpath/stormpath-aspnetcore) 使用Stormpath和ASP.NET Core. 
* [stormpath-sdk](https://github.com/stormpath/stormpath-sdk-dotnet) - 建造 [simple, secure web applications](https://github.com/stormpath/stormpath-aspnetcore) 使用Stormpath和ASP.NET Core.（不建议使用：自加入OKTA之后，它将在2017年3月之前得到更新） 
...
* [Piranha CMS](https://github.com/piranhacms/piranha.core) -适用于ASP.NET Core和Entity Framework Core的轻量级且无干扰的开源CMS.
...
* [multiformats](https://github.com/multiformats/cs-multihash) -通用哈希库，但是一个用于编码/解码多哈希的库，它是“容器”，用于描述使用摘要计算的哈希算法.
...
* [StringDB](https://github.com/SirJosh3917/StringDB) -StringDB是一个模块化的键/值对档案数据库，旨在消耗*少量*的ram并生成*微小*的数据库.
...
* [MongoDB.Entities](https://github.com/dj-nitehawk/MongoDB.Entities) -用于MongoDB的数据访问库，具有优雅的api，LINQ支持和内置的实体关系管理
* MySQL
...
* [AspNetCore.Diagnostics.HealthChecks](https://github.com/xabaril/AspNetCore.Diagnostics.HealthChecks) -ASP.NET Core诊断包的Enterprise HealthChecks
...
* [Obvs](https://github.com/christopherread/Obvs) -可观察到的微服务总线.NET库，将基础传输包装在基于简单Rx的接口中
...
* [nopCommerce](https://github.com/nopSolutions/nopCommerce) -免费的开源电子商务购物车（ASP.NET MVC / ASP.NET Core MVC），具有庞大的社区和一个充满新功能，主题和插件的市场.
...
* [Rx.NET](https://github.com/Reactive-Extensions/Rx.NET) - 这 [Reactive Extensions](http://reactivex.io) 适用于.NET.
* [Qactive](https://github.com/RxDave/Qactive)  -反应性可查询的可观察框架.  `4.xx或更高版本`
* [sodium](https://github.com/SodiumFRP/sodium/tree/master/)  -功能反应式编程（FRP）库.  `4.xx或更高版本`
...
* [LibVLCSharp](https://github.com/videolan/libvlcsharp)：.libvlc的.NET / Mono绑定，该多媒体框架为VideoLAN制作的VLC应用程序提供了动力.
...
* [veldrid](https://github.com/mellinoe/veldrid) -用于.NET的低级，硬件加速的3D图形库.
...
* [Visual Studio Code](https://github.com/Microsoft/vscode)  -新型工具，结合了代码编辑器的简便性和开发人员对其核心edit-build-debug周期的需求. 代码提供全面的编辑和调试支持，可扩展性模型以及与现有工具的轻量级集成.
...
* [NetCoreStack.Localization](https://github.com/NetCoreStack/Localization) -具有实体框架和内存缓存中的.NET Core的数据库资源本地化
* [Westwind.Globalization](https://github.com/RickStrahl/Westwind.Globalization) -.NET应用程序的数据库驱动的资源本地化.
...
### Misc
* [AdvanceDLSupport](https://github.com/Firwood-Software/AdvanceDLSupport)  -改进P /调用本机代码的库. 与本机对象进行交互，就好像它们是一流的对象一样.
...
* [DotNet.Glob](https://github.com/dazinator/DotNet.Glob)  -用于.NET / .NETStandard应用程序的快速glob库. 胜过正则表达式.
...
* [FastExpressionCompiler](https://github.com/dadhi/FastExpressionCompiler) -快速的ExpressionTree编译器进行委派.
* [FluentDocker](https://github.com/mariotoffia/FluentDocker) -适用于docker，docker-compose和docker-machine，适用于win / mac / linux和本机docker的命令，服务和Fluent API.
...
* [LibSass Host](https://github.com/Taritsyn/LibSassHost) -.NET包装器 [libSass](http://sass-lang.com/libsass) 支持虚拟文件系统的库.
...
* [NuGet Trends](https://github.com/NuGetTrends/nuget-trends) -带有NuGet软件包下载计数统计信息的网站.
...
* [npoi](https://github.com/tonyqus/npoi)  -.NET库，可以在不安装Microsoft Office的情况下读取/写入Office格式. 没有COM +，没有互操作.
...
* [Entity Framework Core](https://github.com/aspnet/EntityFramework) -熟悉EF以前版本的开发人员经验，包括LINQ，POCO和Code First支持.
  ...
  * [LINQKit](https://github.com/scottksmith95/LINQKit) -一组免费的LINQ扩展，可扩展到SQL和Entity Framework高级用户.
...
* [Dapper](https://github.com/StackExchange/Dapper) -用于.NET的简单对象映射器.
  * [Dapper-FluentMap](https://github.com/henkmollema/Dapper-FluentMap) -提供了一个简单的API，以便在使用Dapper时将POCO属性流畅地映射到数据库列.
...
* [NEventStore](https://github.com/NEventStore/NEventStore)  -使用事件源作为存储机制时，用于抽象化不同存储实现的持久性库. 该库专门针对DDD / CQRS应用程序而开发.
* [NPoco](https://github.com/schotime/NPoco)  -简单的microORM，可将查询结果映射到POCO对象上. 基于PetaPoco的Schotime分支的项目.
...
### Query Builders
* [SqlKata](https://github.com/sqlkata/querybuilder) -优雅的Sql Query Builder，支持复杂的查询，联接，子查询，嵌套条件，供应商引擎目标等
...
* [OpenCQRS](https://github.com/OpenCQRS/OpenCQRS)  -用于DDD，CQRS和具有Azure Service Bus集成的事件源的.NET Core库. 命令存储库和事件存储库支持的数据库提供程序包括：DocumentDB，MongoDB，SQL Server，MySQL，PostgreSQL和SQLite.
...
* [Tossit](https://github.com/turgayozgur/tossit)  -简单，易于使用的库，用于分布式作业/工作人员逻辑. 由内置RabbitMQ实现处理的分布式消息.
...
### Scheduler and Job
* [Chroniton.NetCore](https://github.com/leosperry/Chroniton) -轻量级的健壮库，可按计划运行任务（作业）.
...
* [DarkSkyCore](https://github.com/amweiss/dark-sky-core) -用于.NET的.NET标准包装 [Dark Sky API](https://darksky.net/dev/docs).
...
* [Testavior](https://github.com/geeklearningio/Testavior) -Testavior是一个轻量级的解决方案，可以帮助您开发ASP.NET Core的行为测试.
...
* WebAssembly
  * [Blazor](https://github.com/SteveSanderson/Blazor) -通过WebAssembly在浏览器中运行.NET的UI框架.
    * [Awesome Blazor](https://github.com/AdrienTorris/awesome-blazor) -收集有关Blazor的真棒资源（样本，组件，文章，视频和其他资源）.
...
* [Topshelf](https://github.com/Topshelf/Topshelf) -易于使用.NET构建Windows服务的服务托管框架.
...
* [serverlessDotNetStarter](https://github.com/pharindoko/serverlessDotNetStarter) 用于基于无服务器框架的AWS云中lambda函数的开发和部署的入门工具包.
...
*微服务和服务网格
  ...
  * [practical-dapr](https://github.com/thangchung/practical-dapr) -基于Dapr和Tye构建的全栈.NET微服务.
...
*巨石
  ...
  * [allReady](https://github.com/HTBox/allReady) -开源解决方案侧重于提高备灾运动的认识，效率和影响，因为这些运动是由当地社区的人道主义和灾难响应组织提供的. [http://www.htbox.org/projects/allready](http://www.htbox.org/projects/allready)
  ...
  * [Entropy](https://github.com/aspnet/Entropy) -混乱的实验场，以了解新功能和新想法-在此处查看单个特征的简单样本.
  ...
  * [PokeR](https://github.com/halomademeapc/pokeR)  -在ASP.NET Core的SPA托管中使用SignalR和Angular进行实时Scrum扑克. 包括Docker支持. [Demo](https://planning.halomademeapc.com)
  * [Practical ASP.NET Core](https://github.com/dodyg/practical-aspnetcore) -每日更新的ASP.NET Core功能和设施的微型示例.
  * [Sample .NET Core CQRS REST API](https://github.com/kgrzybek/sample-dotnet-core-cqrs-api) -.NET Core REST API CQRS实现，带有使用Clean Architecture的原始SQL和DDD.
...
- [InfoQ .NET articles](https://www.infoq.com/dotnet) -InfoQ网站上的最佳.NET文章集
