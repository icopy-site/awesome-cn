...
_visual模式_中的命令行，您可以访问_命令行模式_中的命令，依此类推.
乍一看这听起来很复杂，但有一个巨大的优势：您没有
大部分时间一次按住几个键来折断手指
...
之后，使用_motion_指定要操作的文本区域.
要更改括号之间的所有内容，请使用`ci（`（阅读_change inner
 括号_）.  要删除整个文本段，请使用`dap`（请阅读_delete
段落周围）.

如果看到高级Vim用户正在工作，您会注意到他们说的是
...
[vi](https://en.wikipedia.org/wiki/Vi) 克隆，并支持两种操作模式：
 ...
 用户vimrc或使用`vim -N`启动了Vim，假定为兼容模式！  别
 在兼容模式下使用Vim.  只是不要.
...
3.通读 [basics](#basics-1) 部分以了解什至可能.
...
各种炒作 [plugins](#managing-plugins) 只实现了
Vim已经本地支持.
...
```vim
“如果至少在启用了+ profile的情况下运行Vim 7.4.42，则应执行某些操作.
 ...
 ** Windows **是视口_onto_缓冲区.  如果要查看以下文件
...
The [global buffer list](#buffers-windows-tabs)  是Vim的东西.  在那之前
vi，以前只有参数列表，Vim中也有.
...
|---------------------|------------------------|------------|-----------|---------------------|
 ...
 |  编号  从0到9  vim |  [] |  寄存器0：最后一次拉动.  寄存器1：最后删除.  寄存器2：倒数第二次删除.  等等.  将寄存器`1`-`9`视为只读 [queue](https://en.wikipedia.org/wiki/Queue_(abstract_data_type) ）的9个元素.  |
 |  小删除|  `-` |  vim |  [] |  上次删除少于一行.  |
...
| Alternate buffer    | `#`                    | vim        | [ ]       | Most of the time the previously visited buffer of the current window. See `:h alternate-file` |
 ...
 |  下降|  `〜`|  vim |  [x] |  从上次拖放开始.  |
 |  黑洞|  `_` |  vim |  [] |  如果您不希望其他任何寄存器隐式受影响.  例如，“ _ dd”会删除当前行，而不影响寄存器“”，“ 1”，“ +”，“ *”.  |
...
| `A` - `Z` | User | Global, thus valid between files. Also called _file marks_. Jumping to a file mark may switch to another buffer. |
 |  `0`-`9` |  viminfo |  “ 0”是最后写入viminfo文件的位置.  实际上，这意味着最后一个Vim进程结束.  1是第二个Vim进程结束时的位置，依此类推.  |

将&#39;/ g&#39;或&#39;/ g&#39;标记在前面以形成运动.

 使用“ mm”记下带有“ m”标记的当前位置.  移动文件
然后通过`&#39;m`（第一个非空白）或``m&#39;（精确列）返回.
如果告诉viminfo，退出Vim后会记住小写标记
...
标记也可以用于 [range](#ranges) .  您可能之前已经看到过
...
完成的典型种类是标签，来自导入模块的功能或
库，文件名，字典或仅是当前缓冲区中的单词.
...
|---------|------|--------------|
 ...
 |  ` <c-x><c-o>  `|  omn​​i完成（在&#39;&#39;omnifunc&#39;`中指定）|  `：hi ^ x ^ o` |
 |  ` <c-x>  s` |  拼写建议|  `：hi ^ Xs` |
...
Vim还可以通过设置
 ...
 根据您的口味进行修剪.  您可以使用以下任一方法触发此完成 <c-n>  `
...
触发了“ FileType”事件.
 ...
 每个人添加到他们的vimrc的第一件事就是`filetype on`.  这个
...
## Folding
 ...
 每个文本（或源代码）都有特定的结构.  如果您有结构，它
 表示您有逻辑上分开的文本区域.  折叠允许“折叠”
 这样的区域变成一行并显示简短说明.  有
 作用于这些区域的许多命令称为_folds_.  折痕可以嵌套.
...
|--------------|-------|
 |  差异|  在差异窗口中用于折叠未更改的文本.  |
 |  expr |  使用`&#39;foldexpr&#39;`基本创建一个新的fold方法.  |
 |  缩进  基于缩进的折痕.  |
...
them.
 ...
 您忽略文件名，将使用`Session.vim`.  该文件将保存到
...
3.正则表达式项目始终以“ /”开头，因此`：h / \ +`将带您进入帮助项目
...
5.寄存器总是以“ quote”开头，因此使用`：h quote`可以找到关于
 ...
 7.在帮助页面`：h map.txt`中讨论了映射.  使用`：h mapmode-i`
   ...
   关于某些特定于映射的子主题（例如：：h：map-local`
    本地缓冲区映射或`：h map_bar`了解&#39;|&#39;的方式  在映射中处理.
...
[Freenode](https://freenode.net) 庞大，通常充满了乐于助人的人.
...
|-----------|------------|---------------|-------------------------|
 |  主要|  选择文字|  “中键”，“ Shift +插入” |  *注册
 |  剪贴板|  选择文本和`ctrl + c` |  `ctrl + v` |  `+`注册|

**注意**：选项（否，甚至不是CLIPBOARD选项）都不会保留
 X服务器！  因此，当应用程序运行时，您将丢失用ctrl + c复制的数据
closes.
...
当备份，交换和撤消文件都是关于文本状态的时，viminfo文件是
...
有最新版本，请参见`：helpgrep startjob`.）
 ...
 目录，使用`：！ls`.  照常使用`|`在外壳中进行管道输送，例如`：！ls -1 |
 排序  尾-n5`.
...
这将在当前工作中创建3个文件：`cscope {，.in，.po} .out`
 目录.  将它们视为您的数据库.
...
之后，您可以使用来在任何Python文件中循环浏览这3条语句
％（向前）或g％（向后）.
...
设置选项-ga终端覆盖&#39;，xterm-256color：Tc&#39;
```

-第一行对于大多数人应该是相同的，并表示$ TERM
  在_tmux中使用.
-第二行将tmux专用的“ Tc”（本色）功能添加到
...
```vim
 命令！  更大的：let＆guifont = Alternative（＆guifont，&#39;\ d \ + $&#39;，&#39;\ = submatch（0）+1&#39;，&#39;&#39;）
...
有趣的事实：您可能都知道旧的grep，这是Ken编写的过滤器程序
 汤普森  它有什么作用？  它打印与特定常规匹配的所有行
 表达！  现在猜想`：global / regexp / print`的缩写形式？  那就对了！
 是`：g / re / p`.  肯·汤普森（Ken Thompson）在写grep时受到vi的`：global`的启发.

尽管它的名字是`：global`默认情况下只作用于所有行，但是它也需要
...
--noplugin`，了解启动时加载的其他组合.）

 如果您现在仍然可以重现它，则很可能是Vim本身的错误！  报告
它到 [vim_dev](https://groups.google.com/forum/#!forum/vim_dev) 邮件
 清单.  在大多数情况下，目前暂时无法解决问题，
进一步调查.
...
Vim将分析信息保留在内存中，仅将其写到
 ...
 看一下`/ tmp / profile.log`.  分析期间执行的所有代码
 将显示.  每行，执行的频率和花费的时间.

大多数情况下，这将是用户不熟悉的插件代码，但是如果
 您正在调查某个问题，请跳至日志底部.  这是
两个不同的部分：“按时间总计的功能”和“按时间排序的功能”
 值得金的自我时间.  快速浏览，您可以看到
功能花费的时间太长.
...
|------------|-------------------------------------|------|
 ...
 |  配对|  让G：loaded_matchparen = 1  `：h pi_paren` |
 |  netrw |  让g：loaded_netrwPlugin = 1 |  `：h pi_netrw` |
 |  rrhelper |  让let g：loaded_rrhelper = 1  `：e $ VIMRUNTIME / plugin / rrhelper.vim` |
 ...
 |  拉链  让let g：loaded_zipPlugin = 1  `：h pi_zip` |
...
publishing.
 ...
 该命令创建一个新名称相同的缓冲区，并附加`.html`.  
 颜色与Vim中的颜色相同.  他们取决于
[colorscheme](#colorschemes).

插件知道几种用于微调输出的选项，例如用于设置
编码和字体.
...
有两件事会对性能产生巨大影响：
 ...
 1.复杂的**正则表达式**.  特别是Ruby语法文件引起
    人们过去会放慢脚步.  （另请参阅 [Debugging syntax files](#debugging-syntax-files).)
...
您是否尝试过将代码粘贴到Vim中，之后一切似乎都混乱了
up?

仅当通过“ cmd + v”，“ shift-insert”，“ middle-click”等粘贴时才会发生这种情况.
...
emulator_，例如xterm，gnome-terminal，iTerm2等（与实
