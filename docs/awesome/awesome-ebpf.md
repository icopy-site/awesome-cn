...
- [linux/Documentation/networking/filter.rst](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/Documentation/networking/filter.rst) -eBPF规范（有些过时；信息应仍然有效，但并不详尽）.
...
丹尼尔·博克曼（Daniel Borkmann）作了几篇介绍和论文，内容涉及eBPF的内部结构，尤其是与tc一起使用时.
  ...
  - [Advanced programmability and recent updates with tc's cls_bpf](http://netdevconf.org/1.2/session.html?daniel-borkmann) -有关eBPF，用于隧道和封装，直接数据包访问等的详细信息.
...
-技术论文中的所有XDP详细信息： [The eXpress Data Path: Fast Programmable Packet Processing in the Operating System Kernel](https://github.com/tohojo/xdp-paper)由TokeHøiland-Jørgensen，Jesper Dangaard Brouer，Daniel Borkmann，John Fastabend，Tom Herbert，David Ahern和David Miller撰写，它们都是eBPF和XDP的重要贡献者.
...
-Jesper Dangaard Brouer有几组幻灯片描述了XDP的内部结构：
  ...
  - [XDP – eXpress Data Path, Used for DDoS protection](http://people.netfilter.org/hawk/presentations/OpenSourceDays2017/XDP_DDoS_protecting_osd2017.pdf) -有关XDP的详细信息和用例，以及基准测试结果，用于基准测试的代码段以及使用eBPF / XDP（基于IP黑名单方案）的基本DDoS保护.
...
- [ply](https://wkz.github.io/ply/) -适用于Linux的小型但灵活的开源动态跟踪器，具有与bcc工具类似的功能，但具有受awk和DTrace启发的更简单的语言.
...
- [linux/kernel/trace/bpf_trace.c](https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/tree/kernel/trace/bpf_trace.c) -与跟踪和监视有关的功能和eBPF帮助器（kprobes，跟踪点等）.
