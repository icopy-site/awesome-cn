...
官方网站， [analysis-tools.dev](https://analysis-tools.dev/) 基于此存储库，并为每个工具添加了排名，用户评论和其他资源（例如视频）.
...
如果您还想支持该项目，请转到我们 [Github sponsors page](https://github.com/sponsors/analysis-tools-dev).
...
拉请求非常欢迎！  
还要检查姊妹项目， [awesome-dynamic-analysis](https://github.com/mre/awesome-dynamic-analysis).
...
- [gawk --lint](https://www.gnu.org/software/gawk/manual/html_node/Options.html) -警告有关可疑或不可移植到其他awk实现中的构造.
...
- [CPAchecker](https://cpachecker.sosy-lab.org)  -用于C程序的可配置软件验证的工具. 选择名称CPAchecker是为了反映该工具基于CPA概念，并且用于检查软件程序.
...
- [ESBMC](http://esbmc.org) -ESBMC是基于可满足性模理论的开放源代码，未经许可的，上下文受限的模型检查器，用于验证单线程和多线程C / C ++程序.
- [flawfinder](http://dwheeler.com/flawfinder/) -查找可能的安全漏洞.
...
- [PC-lint](https://www.gimpel.com)  ：copyright：-C / C ++的静态分析. 在Windows / Linux / MacOS下本机运行. 分析几乎所有支持C11 / C18和C ++ 17的平台的代码.
...
- [elm-analyse](https://stil4m.github.io/elm-analyse) ：warning：-一种工具，可让您分析Elm代码，识别不足并应用最佳实践.
- [elm-review](https://package.elm-lang.org/packages/jfmengels/elm-review/latest) -分析整个Elm项目，重点放在用Elm编写的可共享和自定义规则上，这些规则可以确保Elm编译器不会给您带来任何好处.
...
- [i-Code CNES for Fortran](https://github.com/lequal/i-CodeCNES) ：warning：-用于Fortran 77，Fortran 90和Shell的开源静态代码分析工具.
...
- [dingo-hunter](https://github.com/nickng/dingo-hunter) ：warning：-用于在Go中查找死锁的静态分析器.
...
- [errwrap](https://github.com/fatih/errwrap)  -使用新的％w动词指令包装并修复Go错误. 此工具分析fmt.Errorf（）调用并报告包含与Go v1.13中引入的新％w verb指令不同的verb指令的调用. 它还能够重写调用以使用新的％w wrap verb指令.
...
- [go vet](https://golang.org/cmd/vet) -检查Go源代码并报告可疑事件.
- [go-consistent](https://github.com/Quasilyte/go-consistent) ：warning：-分析器，可帮助您使Go程序更加一致.
- [go-critic](https://github.com/go-critic/go-critic) -使用源代码linter来维护当前在其他linter中未实现的检查.
...
- [goimports](https://pkg.go.dev/golang.org/x/tools/cmd/goimports) -检查缺少或未引用的程序包导入.
...
- [revive](https://revive.run)  -Go的快速，可配置，可扩展，灵活和美观的Linter. 即插即用替换的小垫布.
...
- [structcheck](https://gitlab.com/opennota/check) -查找未使用的struct字段.
...
- [Stan](https://kowainik.github.io/projects/stan) -Stan是一种命令行工具，用于分析Haskell项目并以有用的方式输出已发现的漏洞，并提供可能的解决问题的方法.
- [Weeder](https://github.com/ocharles/weeder) -使用Haskell代码检测无效出口或包裹进口的工具.
...
- [aether](http://aetherjs.com) ：warning：-在节点或浏览器中插入，分析，规范化，转换，沙箱，运行，逐步浏览和可视化用户JavaScript.
...
- [DeepScan](https://deepscan.io) ：copyright：-JavaScript的分析器，它针对运行时错误和质量问题，而不是编码约定.
- [es6-plato](https://github.com/the-simian/es6-plato) -可视化JavaScript（ES6）源复杂性.
- [escomplex](https://github.com/jared-stilwell/escomplex) ：warning：-JavaScript系列抽象语法树的软件复杂性分析.
- [Esprima](https://esprima.org) -ECMAScript解析基础结构，可进行多用途分析.
...
- [plato](https://github.com/es-analysis/plato) ：warning：-可视化JavaScript源的复杂性.
...
- [ktlint](https://ktlint.github.io) -带有内置格式化程序的防骑脚踏车科特林短绒.


<h2 id="lua">拿</h2>

- [luacheck](https://github.com/mpeterv/luacheck) ：warning：-用于Lua代码的整理和静态分析的工具.
- [lualint](https://github.com/philips/lualint) -lualint对Lua源代码中的全局变量使用情况进行基于luac的静态分析.
- [Luanalysis](https://plugins.jetbrains.com/plugin/14698-luanalysis) -用于静态类型Lua开发的IDE.
...
- [Mondrian](http://trismegiste.github.io/Mondrian) ：警告：-使用图论的一组静态分析和重构工具.
...
- [PHP-Token-Reflection](https://github.com/Andrewsville/PHP-Token-Reflection) ：warning：-库模拟PHP内部反射.
...
- [phpca](https://github.com/wapmorgan/PhpCodeAnalyzer) ：warning：-查找非内置扩展名的用法.
...
- [PhpDependencyAnalysis](https://mamuz.github.io/PhpDependencyAnalysis) ：warning：-为项目建立依赖关系图.
- [PhpDeprecationDetector](https://github.com/wapmorgan/PhpDeprecationDetector)  -PHP代码分析器，用于在较新的解释器版本中搜索不建议使用的功能的问题. 它查找已删除的对象（函数，变量，常量和ini指令），不赞成使用的函数功能以及禁止使用的名称或技巧（例如，较新版本中的保留标识符）.
...
- [phpsa](https://github.com/ovr/phpsa) ：warning：-用于PHP的静态分析工具.
...
- [ZPA](https://felipezorzo.com.br/zpa/)  -Z PL / SQL Analyzer（ZPA）是用于PL / SQL和Oracle SQL的可扩展代码分析器. 它可以与SonarQube集成.
...
- [bandit](https://bandit.readthedocs.io/en/latest) -一种在Python代码中查找常见安全问题的工具.
...
- [Bowler](https://pybowler.io/)  -用于现代Python的安全代码重构.  Bowler是用于在语法树级别处理Python的重构工具. 它可以安全，大规模地修改代码，同时保证生成的代码可以编译和运行. 它提供了简单的命令行界面和Python中的流畅API，用于在代码中生成复杂的代码修改.
- [cohesion](https://github.com/mschwager/cohesion) ：warning：-用于测量Python类内聚力的工具.
...
- [jedi](https://jedi.readthedocs.io/en/latest) -适用于Python的自动补全/静态分析库.
...
- [PyCodeQual](https://pycodequ.al)  ：copyright：-PyCodeQual为您提供有关复杂性和错误风险的见解. 它将自动评论添加到您的请求请求中.
...
- [PyT - Python Taint](https://github.com/python-security/pyt) ：warning：-用于检测Python Web应用程序中的安全漏洞的静态分析工具.
...
- [cyclocomp](https://github.com/MangoTheCat/cyclocomp) ：警告：-量化R函数/表达式的圈复杂度.
- [goodpractice](http://mangothecat.github.io/goodpractice) ：warning：-分析R软件包的源代码并提供最佳实践建议.
...
- [Querly](https://github.com/soutaro/querly) -用于Ruby的基于模式的检查工具.
- [Railroader](https://railroader.org) ：warning：-针对Ruby on Rails应用程序的开源静态分析安全漏洞扫描程序.
- [rails_best_practices](https://rails-bestpractices.com) -用于Rails项目的代码度量工具
...
- [dylint](https://www.trailofbits.com/post/write-rust-lints-without-forking-clippy)  -从动态库运行Rust皮棉的工具.  Dylint使开发人员可以轻松维护自己的个人皮棉收藏.
...
- [herbie](https://github.com/mcarton/rust-herbie-lint) ：warning：-在使用数值不稳定的浮点表达式时，在包装箱中添加警告或错误.
- [linter-rust](https://github.com/AtomLinter/linter-rust) ：warning：-使用Rustc和货物在Atom中清除您的Rust文件.
...
- [sqlcheck](https://github.com/jarulraj/sqlcheck) ：warning：-自动识别SQL查询中的反模式.
...
- [Visual Expert](https://www.visual-expert.com) ：copyright：-PowerBuilder，Oracle和SQL Server的代码分析探索，分析和记录代码 


<h2 id="scala">梯子</h2>

- [linter](https://github.com/HairyFotr/linter) ：warning：-Linter是一个Scala静态分析编译器插件，它为各种可能的错误，效率低下和样式问题添加了编译时检查.
...
- [i-Code CNES for Shell](https://github.com/lequal/i-CodeCNES) ：warning：-用于Shell和Fortran（77和90）的开源静态代码分析工具.
...
- [SwiftLint](https://realm.github.io/SwiftLint) -强制执行Swift样式和约定的工具.
- [Tailor](https://sleekbyte.github.io/tailor) ：warning：-用于以Apple的Swift编程语言编写的源代码的静态分析和皮棉工具.
...
- [TypeScript ESLint](https://github.com/typescript-eslint/typescript-eslint) -用于eslint的TypeScript语言扩展.
...
- [Application Inspector](https://www.ptsecurity.com/ww-en/products/ai) ：copyright：-商业静态代码分析，生成漏洞利用以验证漏洞.
...
- [goone](https://github.com/masibw/goone) -在Go代码中查找N + 1个查询（for循环中的SQL调用）
...
- [imhotep](https://github.com/justinabrahms/imhotep) ：warning：-评论进入您的存储库的提交，并检查语法错误和常规的掉毛警告.
...
- [lizard](https://github.com/terryyin/lizard)  -Lizard是可扩展的Cyclomatic Complexity Analyzer，适用于许多编程语言，包括C / C ++（不需要所有头文件或Java导入）. 它还执行复制粘贴检测（代码克隆检测/代码重复检测）和许多其他形式的静态代码分析. 计算没有注释的代码行，CCN（圈复杂度数），函数的令牌计数，函数的参数计数.
...
- [Rome](https://rome.tools/) -罗马是一个linter，编译器，捆绑器，以及 [more](https://rome.tools/#development-status) 适用于JavaScript，TypeScript，JSON，HTML，Markdown和CSS.
- [Roslyn Analyzers](https://github.com/dotnet/roslyn-analyzers) -基于Roslyn的FxCop分析仪实施.
- [Roslyn Security Guard](https://security-code-scan.github.io) -该项目侧重于识别潜在漏洞，例如SQL注入，跨站点脚本（XSS），CSRF，加密漏洞，硬编码密码等等.
...
- [Semgrep](https://semgrep.live)  -适用于多种语言的免费开放源代码轻量级静态分析. 使用看起来像源代码的模式查找和阻止错误变体.
- [Semmle QL and LGTM](https://semmle.com)  ：copyright：-使用对源代码的查询来查找安全漏洞，变体和关键代码质量问题. 自动PR代码审核； 免费供公共GitHub / Bitbucket回购： [LGTM.com](https://LGTM.com).
...
- [shipshape](https://github.com/google/shipshape) ：warning：-静态程序分析平台，允许自定义分析器通过公共接口插入.
...
- [Symfony Insight](https://insight.symfony.com/) ：copyright：-检测安全风险，查找错误并为PHP项目提供可操作的指标.
...
- [trivy](https://github.com/aquasecurity/trivy)  -适用于CI的用于容器和其他工件的简单而全面的漏洞扫描程序.  Trivy检测OS软件包（Alpine，RHEL，CentOS等）的漏洞和应用程序依赖项（捆绑程序，Composer，npm，yarn等）. 检查容器和文件系统.
...
<h2 id="arm">Azure资源管理器</h2>

- [AzSK](https://azsk.azurewebsites.net/)  -用于Azure（AzSK）的安全DevOps套件在您的基础结构即代码中提供了安全智能感知，安全验证测试（SVT），CICD扫描漏洞，合规性问题和基础结构错误配置. 通过ARM支持Azure.
...
- [Nauz File Detector](https://github.com/horsicq/Nauz-File-Detector) -适用于Windows，Linux和MacOS的静态链接器/编译器/工具检测器.
- [Twiggy](https://rustwasm.github.io/twiggy)  -分析二进制文件的调用图以分析代码大小. 目标是缩小wasm二进制文件大小.
- [VMware chap](https://github.com/vmware/chap)  -第一章分析了非仪器化的ELF核心文件的泄漏，内存增长和损坏. 它足够可靠，可以在自动化中用于在泄漏发生之前捕获泄漏. 作为一种交互式工具，它可以帮助解释内存的增长，可以识别某些形式的损坏，并通过给出各种内存位置的状态来补充调试器. 
...
- [Project Wallace CSS Analyzer](https://www.projectwallace.com) -CSS的分析工具，属于 [Project Wallace](https://www.projectwallace.com).
...
- [dotenv-linter (Rust)](https://dotenv-linter.github.io/#/)  -适用于.env文件的闪电般快速的林特. 写在锈
- [gixy](https://github.com/yandex/gixy)  ：warning：-分析Nginx配置的工具. 主要目标是防止配置错误并自动进行缺陷检测.


<h2 id="configmanagement">配置管理</h2>

- [ansible-lint](https://docs.ansible.com/ansible-lint) -检查剧本中可能会改善的做法和行为.
- [AWS CloudFormation Guard](https://github.com/aws-cloudformation/cloudformation-guard) -对照代码策略规则检查本地CloudFormation模板，并从现有模板生成规则.
- [AzSK](https://azsk.azurewebsites.net/)  -用于Azure（AzSK）的安全DevOps套件在您的基础结构即代码中提供了安全智能感知，安全验证测试（SVT），CICD扫描漏洞，合规性问题和基础结构错误配置. 通过ARM支持Azure.
...
- [AzSK](https://azsk.azurewebsites.net/)  -用于Azure（AzSK）的安全DevOps套件在您的基础结构即代码中提供了安全智能感知，安全验证测试（SVT），CICD扫描漏洞，合规性问题和基础结构错误配置. 通过ARM支持Azure.
- [Code Climate](https://codeclimate.com) -面向所有人的开放且可扩展的静态分析平台.
...
- [mdformat](https://mdformat.rtfd.io) -符合CommonMark的Markdown格式化程序
...
- [android-lint-summary](https://passy.github.io/android-lint-summary) ：warning：-将多个项目的棉绒错误合并为一个输出，立即检查多个子项目的棉绒结果.
...
- [lintian](https://lintian.debian.org) ：warning：-用于Debian软件包的静态分析工具.
- [rpmlint](https://github.com/rpm-software-management/rpmlint) -用于检查rpm包中常见错误的工具.


<h2 id="protobuf">协议缓冲区</h2>

- [protolint](https://github.com/yoheimuta/protolint) -可插入的linter和fixer，用于强制执行协议缓冲区样式和约定.
...
- [njsscan](https://opensecurity.in) -静态应用程序测试（SAST）工具，可以使用来自libsast的简单模式匹配器和可识别语法的语义代码模式搜索工具semgrep在node.js应用程序中找到不安全的代码模式.
- [Oversecured](https://oversecured.com)  ：copyright：-一个移动应用程序漏洞扫描程序，专为安全研究人员和漏洞赏金黑客使用. 它还允许将集成到企业的DevOps流程中.
- [PT Application Inspector](https://www.parasoft.com/)  ：copyright：-识别代码缺陷并检测漏洞以防止Web攻击. 通过展示可能的利用来演示远程代码执行.
- [Qualys Container Security](https://www.qualys.com/apps/container-security) ：copyright：-容器本机应用程序保护，以提供对容器化应用程序的可见性和控制.
- [SearchDiggity](https://resources.bishopfox.com/resources/tools/google-hacking-diggity/attack-tools/)  ：copyright：-识别托管在Github，Google Code，MS CodePlex，SourceForge等上的开放源代码项目中的漏洞. 该工具具有130多种默认搜索，可识别SQL注入，跨站点脚本（XSS），不安全的远程和本地文件包括，硬编码密码等. 
- [tfsec](https://github.com/tfsec/tfsec)  -Terraform静态分析工具，可通过在构建时检查云配置错误来防止潜在的安全问题，并直接与HCL解析器集成以获得更好的结果. 检查是否违反了AWS，Azure和GCP安全最佳实践建议.
...
- [MythX](https://mythx.io)  ：copyright：-MythX是一个易于使用的分析平台，集成了模糊分析，符号执行和静态分析等多种分析方法，可以高精度地查找漏洞. 它可以与Remix或VSCode之类的工具链集成，也可以从命令行调用.
- [slither](https://github.com/trailofbits/slither) -静态分析框架，运行一套漏洞检测器，打印有关合同详细信息的可视信息，并提供一个API，可轻松编写自定义分析.
- [solhint](https://protofire.github.io/solhint)  -Solhint是由https://protofire.io创建的开源项目. 它的目标是为Solidity代码提供一个整理工具.
...
- [dennis](https://github.com/willkg/dennis) ：warning：-一组用于处理PO文件的工具，以简化开发并提高质量.
...
- [After the Deadline](https://open.afterthedeadline.com) ：warning：-拼写，样式和语法检查器.
- [alex](https://alexjs.com) -捕捉不敏感，不体贴的文字
- [codespell](https://github.com/codespell-project/codespell) -检查代码中常见的拼写错误.
- [languagetool](https://languagetool.org)  -25种以上语言的样式和语法检查器. 它发现许多简单的拼写检查器无法检测到的错误.
- [misspell-fixer](https://github.com/vlajos/misspell-fixer) -用于修复源代码中常见的拼写错误，错别字的快速工具.
- [Misspelled Words In Context](http://jwilk.net/software/mwic) -拼写检查器，对可能的拼写错误进行分组，并在其上下文中显示它们.
- [proselint](http://proselint.com) -针对英语散文的短篇小说，着重于写作风格而不是语法.
- [vale](https://docs.errata.ai/vale/about) -为语法而设计的短文短句，考虑了速度和可扩展性.
...
- [OWASP Source Code Analysis Tools](https://owasp.org/www-community/Source_Code_Analysis_Tools) -由开放式Web应用程序安全项目维护的工具列表
