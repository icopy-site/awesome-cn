...
- [Jexer](https://gitlab.com/klamonte/jexer)  -先进的控制台（和Swing）文本用户界面（TUI）库，具有可拖动鼠标的窗口，内置的终端窗口管理器和sixel图像支持. 好像 [Turbo Vision](https://en.wikipedia.org/wiki/Turbo_Vision).
...
-[Bootify！[c]]（https://bootify.io）-使用JPA模型和REST API生成基于浏览器的Spring Boot应用程序.
...
_试图从图像和视频中获取高级信息的图书馆.

- [BoofCV](https://boofcv.org) -用于图像处理，相机校准，跟踪，SFM，MVS，3D视觉，QR代码等的库.
...
- [microconfig](https://microconfig.io)  -为微服务设计的配置系统，有助于将配置与代码分离. 不同服务的配置可以具有公共部分和特定部分，并且可以动态分布.
- [owner](https://github.com/lviggiano/owner) -减少特性的样板.
...
- [ThreeTen-Extra](https://github.com/ThreeTen/threeten-extra) -其他日期时间类，作为JDK 8中的类的补充.
...
- [fastexcel](https://github.com/dhatim/fastexcel) -高性能库，用于读写大型Excel（XLSX）工作表.
...
- [Google HTTP Client](https://github.com/googleapis/google-http-java-client) -支持java.net.HttpURLConnection，Apache HTTP Client，Android，Google App Engine，XML，Gson，Jackson和Protobuf的可插拔HTTP传输抽象.
...
- [Siren4J](https://github.com/eserating-chwy/siren4j) -警笛规范库.
...
- [Visual Studio Code](https://code.visualstudio.com/docs/languages/java) -通过使用内部市场的扩展，以简单，现代的工作流程为轻量级项目提供Java支持.
...
- [SAP Machine](https://sap.github.io/SapMachine/)  -SAP的免费，经过严格测试并经过JCK验证的OpenJDK友好fork.  （仅限GPL-2.0，带Classpath-exception-2.0）
...
- [OpenTracing Toolbox](https://github.com/zalando/opentracing-toolbox) -在OpenTracing之上构建的库的集合，并提供现有工具的扩展和插件.
...
- [DJL](https://djl.ai) -深度学习的高级且与引擎无关的框架.
...
- [Maven Wrapper](https://github.com/takari/maven-wrapper) -适用于Maven的Gradle Wrapper的类比，允许在不安装maven的情况下构建项目.
- [Membrane Service Proxy](https://github.com/membrane/service-proxy) -开源反向代理框架.
- [MinimalFTP](https://github.com/Guichaguri/MinimalFTP) -轻巧，小型且可定制的FTP服务器.
...
- [Smooks](https://github.com/smooks/smooks)  -基于片段的消息处理框架.  （Apache-2.0或LGPL-3.0或更高版本）
...
- [ObjectiveSql](https://github.com/braisdom/ObjectiveSql) -ActiveRecord ORM，用于快速开发和配置约定.
...
- [Morpheus](https://github.com/zavtech/morpheus-core) -提供了一种通用的二维内存高效表格式数据结构，称为DataFrame，以实现对JVM上的科学计算进行高效的内存中分析.
...
- [StringTemplate](https://github.com/antlr/stringtemplate4) -用于生成源代码，网页，电子邮件或任何其他格式的文本输出的模板引擎.
...
- [Temporal](https://temporal.io) -微服务编排平台，由Cadence派生，但基于gRPC.
