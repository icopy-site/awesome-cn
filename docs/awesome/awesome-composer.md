...
- [CheatSheet](http://composer.json.jolicode.com/) -CLI命令和`composer.json`模式的概述.
...
- [Composer-Composition](https://github.com/bamarni/composition) - 提供了一个API，以便在运行时检查您的环境.
...
- [Composer-Patches-Plugin](https://github.com/netresearch/composer-patches-plugin)   - 使您可以从任何包的任何包提供补丁.  当依赖被取出，将补丁应用之上.
...
- [Composer-Patchset](https://github.com/creativestyle/composer-plugin-patchset) -自动获取，更新补丁并将其应用于任何作曲家程序包-将补丁集存储为作曲家程序包本身.
- [Composer-Plugin-QA](https://github.com/Webysther/composer-plugin-qa) -全面的插件，可让作曲家执行PHP质量保证工具.
- [Composer-Cleanup-Plugin](https://github.com/barryvdh/composer-cleanup-plugin) - 移除来自供应商目录的测试和文档的文件夹.
- [Composer-Cleaner](https://github.com/dg/composer-cleaner) - 该工具从供应商目录中删除不必要的文件和目录.
- [Composer-Ignore-Plugin](https://github.com/lichunqiang/composer-ignore-plugin)  -使您能够从供应商文件夹中删除文件和文件夹（以进行更清洁，更小规模的生产部署）.  这是为`.gitattributes`替代.
...
- [Composer-Git-Hooks](https://github.com/BrainMaestro/composer-git-hooks) -一个用于轻松管理作曲家配置中的git挂钩的库.
...
- [Composer Registry Manager](https://github.com/slince/composer-registry-manager) - 允许不同作曲家的存储库之间进行切换.
...
- [Studio](https://github.com/franzliedke/studio)   - 一种用于开发作曲家包工作台.  其替代在供应商文件夹编辑依赖关系，或者使用 [PathRepositories](https://getcomposer.org/doc/05-repositories.md#path) 你依赖的本地副本加载到您的项目.
...
- [Composer-Normalize](https://github.com/localheinz/composer-normalize) -该插件通过重组和排序条目（规范化）来帮助保持您的`composer.json`文件一致.
- [Composer-Service](https://github.com/pborreli/composer-service) -使您可以在远程服务器上将Composer作为服务运行.
- [Composer PreferLowest Checker](https://github.com/dereuromark/composer-prefer-lowest) -严格比较您的composer.json的指定最低版本与首选最低的composer update命令选项实际使用的最低版本.
...
- [Dependabot](https://dependabot.com/)  -Dependabot是依赖项更新服务.  它通过发送请求请求来监视和更新您的依赖项.  该服务是免费的公共回购和个人账户回购.
...
- [Packagist Mirror](https://github.com/Webysther/packagist-mirror)   - 此脚本有助于建立一个packagist镜.  它是的维护和稳定版本 [Packagist Crawler](https://github.com/hirak/packagist-crawler).
...
关于元数据镜子：https://packagist.org/mirrors
...
- [Satisfy](https://github.com/ludofleury/satisfy) - 使用Web UI赛帝作曲家仓库管理器.
...
在法律允许的范围内， [Jens A. Koch](https://github.com/jakoch) 已放弃所有版权及相关或邻接权这项工作.
