...
> Useful resources for using [IPFS](https://ipfs.io) 并在其之上构建事物

 _此列表适用于项目，工具或几乎与IPFS相关的所有事物，它们都很棒**. 这是针对已经很棒的产品的-如果您计划使用IPFS做一些很棒的事情，则应该构建它，然后在此处链接它. 如果您有一个与IPFS有关的很棒的主意，可以在这里问一下它的好地方 [our IPFS Discuss Forums](https://discuss.ipfs.io)._
...
[Collaborative clusters](https://collab.ipfscluster.io/) 是公共IPFS群集，任何人都可以加入以帮助在IPFS网络上复制和重新分发内容. 通常会更新集群内容而不是数据集.

## Apps

- [2read](https://2read.net/) -将当前标签中的文章转换为可读形式，然后将其上传到可写节点. [Source](https://github.com/meehow/2read)
...
- [Agregore](https://github.com/AgregoreWeb/agregore-browser)  -用于分布式Web的最小Web浏览器. 支持使用浏览器的`fetch（）`API从IPFS下载/上传数据
- [AirSecure](https://github.com/airsecure/airsecure) -基于Textile＆IPFS构建的分散式两因素身份验证应用程序.
- [akasha](http://akasha.world/) -由以太坊（Ethereum）支持并嵌入IPFS的下一代社交媒体网络.
- [Alexandria](http://www.alexandria.io/) -分散式内容发布/营利平台.
- [Arbore](http://arbo.re) -建立在IPFS之上的朋友之间的文件共享应用程序.
...
- [Cohort](https://github.com/zignig/cohort) -一个golang应用程序，用于预设threejs界面并将其所有资产移出IPFS.
...
- [digx](https://www.dgx.io/) -Digix是建立在以太坊和IPFS之上的资产加密平台.
...
- [Peer Web Site](https://peerweb.site/)  -点对点托管的点对点网站！ 从您的浏览器发送功能齐全的HTML（包括CSS，JS）网站并附加文件，例如. 视频，图像等 [Source](https://github.com/Weedshaker/PeerWebSite)
...
- [Temporal](https://github.com/RTradeLtd/Temporal) -Temporal是易于使用的API和平台，用于将IPFS和其他分布式/分散式存储技术集成到企业应用程序中
- [Textile Photos](https://www.textile.photos) -纺织照片是您照片的安全移动数字钱包. [Source](https://github.com/textileio/textile-mobile)
...
- [Cooking Recipes](https://ipfs.io/ipfs/QmTXo6GforwsuGAd8behjgvKCsMMBdG1xMZJF4qa9W7CHB) -约50GB的格式化和未格式化烹饪食谱. [Source](https://old.reddit.com/r/opendirectories/comments/dsznin/50_gb_directory_of_cooking_recipes/)
...
- [Operating Systems Mirror & Archive (OSMA)](http://www.fleschutz.de/Service/OSMA.html) -包含192个发行版的29个已建立操作系统的镜像和存档. [Source](https://github.com/fleschutz/awesome-ipfs)
...
- [DWS](https://dws.datochain.com/) -使用DWS部署现代静态网站，并在IPFS上享受安全和分散的存储.
- [Eternum](https://www.eternum.io/) -使用易于使用的API的付费固定服务.
- [Fission](https://fission.codes) -由IPFS支持的应用和网站托管，包括DNS自动化，身份和端到端加密.
...
- [gomobile-ipfs](https://github.com/ipfs-shipyard/gomobile-ipfs) -使用Gomobile的Mobile上的IPFS和libp2p
- [http2ipfs](https://github.com/jbenet/http2ipfs-web) -这是将URL添加到IPFS节点的简单网络工具.
- [Image Sever](https://imageserver.link)  -通过JSON HTTP API上传图像；  URI，URL，文件和Base64 [Source](https://github.com/ekumahost/ImageServer)
- [ipcat](https://github.com/noffle/ipcat) -：cat2：检索IPFS对象数据并将其发送到stdout.
- [IPDR](https://github.com/miguelmota/ipdr) -IPFS支持的Docker注册表
- [IPFS Setup Action](https://github.com/marketplace/actions/ipfs-setup-action) -GitHub动作，用于安装和初始化go-ipfs，以在GitHub的CI平台上提供跨平台测试环境. [Source](https://github.com/ibnesayeed/setup-ipfs)
...
- [ipfs-common](https://github.com/arsyun) -一些常用的工具，1.CID到人类可读的cid，2.cid和块名称的转换，3.阅读leveldb工具. [Source](https://github.com/arsyun/ipfs-common)
...
- [ipfs-video-gateway](https://ipfs.video) -在云提供商上通过云启动您自己的IPFS网关，并通过简单的Web界面轻松固定内容. [Source](https://github.com/bneijt/ipfs-video-gateway)
...
- [iprfc](https://github.com/RTradeLtd/iprfc) -IETF RFC下载器，它在IPFS上存储RFC，并用RTradeLtd / Lens对它们进行索引.
...
- [Multiverse](https://github.com/multiverse-vcs/go-multiverse) -Multiverse是一个分散的版本控制系统，可实现对等软件开发.
...
- [Tellit](https://gitlab.com/terceranexus6/tellit) -在使用密钥对或密码上传文件之前先对其进行加密
