...
* [Bistro](https://github.com/asavinov/bistro)  -用于批处理和流分析的通用数据处理引擎.  它基于一种新颖的数据模型，与通过MapReduce或SQL等传统方法仅设置操作相反，该模型通过*函数*表示数据，并通过*列操作*处理数据.
...
* [Apache Crunch](http://crunch.apache.org/) -一个简单的Java API，用于执行诸如加入和数据聚合之类的任务，这些任务很难在纯MapReduce上实现.
...
* [DataTorrent StrAM](https://www.datatorrent.com/) -实时引擎旨在以尽可能不受阻碍的方式启用分布式，异步，实时内存中大数据计算，同时将开销降至最低，并且不会影响性能.
* [Facebook Corona](https://www.facebook.com/notes/facebook-engineering/under-the-hood-scheduling-mapreduce-jobs-more-efficiently-with-corona/10151142560538920) -Hadoop增强功能可消除单点故障.
...
* [Streamdrill](https://streamdrill.com/) -用于统计事件流在不同时间段内的活动并找到最活跃的事件流.
...
* [Alluxio](http://www.alluxio.org/) -在整个群集框架之间以内存速度可靠地共享文件.
...
* [RavenDB](https://ravendb.net/) -交易型开源文档数据库.
...
您可以在Daniel Abadi教授的博客上详细了解这种区别： [Distinguishing two major types of Column Stores](http://dbmsmusings.blogspot.com/2010/03/distinguishing-two-major-types-of_29.html).
...
* [HyperDex](https://github.com/rescrv/HyperDex) -具有多种功能的可扩展的下一代键值和文档存储，包括一致性，容错性和高性能.
...
* [GraphLab PowerGraph](https://turi.com/products/create/docs/) -核心C ++ GraphLab API以及在GraphLab API之上构建的高性能机器学习和数据挖掘工具箱的集合.
...
* [ClickHouse](https://clickhouse.yandex/) -面向列的开源数据库管理系统，允许实时生成分析数据报告.
...
* [LocustDB](https://github.com/cswinter/LocustDB) -实验性分析数据库，旨在为商品硬件的查询性能设置新标准. 
...
* [Bedrock](http://bedrockdb.com/) -在SQLite之上构建的简单，模块化，网络化和分布式事务层.
...
* [H-Store](http://hstore.cs.brown.edu/) -是一个实验性的主内存，并行数据库管理系统，针对在线事务处理（OLTP）应用进行了优化.
...
* [Apache Drill](http://drill.apache.org/) -互动分析的框架，受到Dremel的启发.
...
* [PipelineDB](https://www.pipelinedb.com/) -开源关系数据库，可在流上连续运行SQL查询，并将结果增量存储在表中.
...
* [Trafodion](https://wiki.trafodion.org/wiki/index.php/Main_Page) -针对大数据事务或操作工作负载的企业级HBase on SQLBase解决方案.
...
* [Cloudera Morphlines](https://github.com/cloudera/cdk/tree/master/cdk-morphlines) -帮助ETL转换为Solr，HBase和HDFS的框架.
...
* [Mara](https://github.com/mara/data-integration) -一个自以为是的轻量级ETL框架，介于纯脚本和Apache Airflow之间
...
* [Hunk](https://www.splunk.com/en_us/download/hunk.html) -Hadoop的Splunk分析.
...
* [Kapacitor](https://github.com/influxdata/kapacitor) -用于处理，监视和预警时间序列数据的开源框架.
...
* [Enigma.io](https://www.enigma.com/) – Freemium健壮的Web应用程序，用于浏览，过滤，分析，搜索和导出从Web上抓取的海量数据集.
...
* [TimescaleDB](http://www.timescale.com/) -针对快速提取和复杂查询而优化的开源时间序列数据库
...
* [chartd](http://chartd.co/) -仅包含img标签的自适应视网膜兼容图表.
...
* [2013](http://db.disi.unitn.eu/pages/VLDBProgram/pdf/industry/p767-wiener.pdf) -** Facebook **-Scuba：通过Facebook进入数据.
...
* [2012](http://vldb.org/pvldb/vol5/p1771_georgelee_vldb2012.pdf) -** Twitter **-统一日志记录基础架构
Twitter上的数据分析.
...
* [2012](http://research.microsoft.com/pubs/178045/ppaoxs-paper29.pdf) -** Microsoft **-Paxos并行.
...
* [2010](https://www.usenix.org/legacy/event/osdi10/tech/full_papers/Beaver.pdf) -** Facebook **-在干草堆中找针头：Facebook的照片存储.
...
* [Unified Log Processing](https://www.manning.com/books/event-streams-in-action) -统一日志处理是在您的企业中实施事件流（Kafka或Kinesis）的统一日志的实用指南
* [Kafka Streams in Action](https://www.manning.com/books/kafka-streams-in-action) -Kafka Streams in Action教您在流向Kafka平台的数据上实现流处理所需的一切知识，使您可以专注于从数据中获取更多信息，而无需花费时间或精力.
