...
* [JSON Viewer Awesome](https://chrome.google.com/webstore/detail/json-viewer-awesome/iemadiahhbebdklepanmkjenfdebfpfe/related?hl=en) -一个开放源代码的Chrome扩展程序，用于通过语法突出显示和折叠或以可视图形的形式浏览JSON.
...
* [jsoncat](https://github.com/pantuza/jsoncat) -在终端上用颜色和调整标签大小漂亮地打印Json.
* [jq](http://stedolan.github.io/jq/) -轻巧灵活的命令行JSON处理器.
* [json](http://trentm.com/json/) -用于在Unix命令行上按摩JSON的“ json”命令.
* [jshon](http://kmkeen.com/jshon/) -解析器旨在在Shell中提供最大的便利.
* [jarg](http://jdp.github.io/jarg/) -Shell中的简写JSON和表单编码语法.
* [jsawk](https://github.com/micha/jsawk) -与awk类似，但适用于JSON.
...
* [jid](https://github.com/simeji/jid)  -增量挖掘机. 通过使用过滤查询（如jq）以交互式方式向下钻取JSON.
* [jiq](https://github.com/fiatjaf/jiq)  -是`jq`的`jid`. 您可以使用`jq`过滤查询以交互方式向下钻取.
...
## Data generation
* [jsonymize](https://github.com/cameronhunter/jsonymize) -从标准输入中读取数据，进行匿名处理，然后写入标准输出中.
* [dyson](https://github.com/webpro/dyson)  -用于动态伪造JSON的服务器.  （node.js）

## Differencing
* [JSONPatch](http://jsonpatch.com/) -描述文档更改的格式.
* [JSON-Patch](https://github.com/Starcounter-Jack/JSON-Patch)  -JSON-Patch标准（RFC 6902）的精简和平均Javascript实现.  （JavaScript）
...
* [JSON Activity Streams](http://activitystrea.ms/) -在网络上联合社交活动的格式.
...
* [jsonapi-consumer](https://github.com/OKTAYKIR/jsonapi-consumer) -用于在上使用基于JSONAPI的API的客户端框架 [JSON API standard](http://jsonapi.org/).
...
* [SassyJSON](https://github.com/HugoGiraudel/SassyJSON)  -Sass驱动的API.  （S）
...
## Queries
* [dasel](https://github.com/tomwright/dasel)  -使用命令行中的选择器查询和更新数据结构. 可比 [jq](https://github.com/stedlan/jq) / [yq](https://github.com/kislyuk/yq) 但支持零运行时相关性的JSON，YAML，TOML和XML.
...
* [JSON Schema View](https://github.com/mohsen1/json-schema-view) -用于以HTML呈现JSON模式的AngularJS指令（AngularJS）
