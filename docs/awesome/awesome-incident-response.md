...
* [Caldera](https://github.com/mitre/caldera)  -自动化的对手仿真系统，可在Windows Enterprise网络中执行折衷后的对抗行为.  它在运行过程中使用规划系统和基于对抗策略，技术和常识（ATT＆CK™）项目的预先配置的对抗模型来生成计划.
...
* [Cyber Triage](http://www.cybertriage.com)  -Cyber​​ Triage可以远程收集和分析端点数据，以帮助确定是否受到威胁.  它采用无代理方法，并专注于易用性和自动化，因此公司无需大型基础架构变更和法医专家团队即可做出响应.  其结果用于确定系统是否应删除或进一步调查.
...
* [GRR Rapid Response](https://github.com/google/grr)  -针对远程现场取证的事件响应框架.  它由安装在目标系统上的python代理（客户端）和可以管理代理并与之通信的python服务器基础结构组成.
...
* [Mozilla Investigator (MIG)](http://mig.mozilla.org/)  -在远程端点上执行调查性手术的平台.  它使研究人员可以并行地从大量系统中获取信息，从而加快事件调查和日常运营安全性.
...
* [Cyphon](https://www.cyphon.io/)  -Cyphon通过一个平台简化了许多相关任务，从而消除了事件管理的麻烦.  它接收，处理和分类事件，以为您的分析工作流提供全方位的解决方案-汇总数据，对警报进行捆绑和优先排序，并赋予分析人员调查和记录事件的能力.
* [Demisto](https://www.demisto.com/product/) -Demisto社区版（免费）提供完整的事件生命周期管理，事件关闭报告，团队分配和协作以及许多集成来增强自动化（例如Active Directory，PagerDuty，Jira等）.
* [Fast Incident Response (FIR)](https://github.com/certsocietegenerale/FIR/)  -设计敏捷性和速度的网络安全事件管理平台.  它可以轻松创建，跟踪和报告网络安全事件，对于CSIRT，CERT和SOC都非常有用.
...
* [RTIR](https://www.bestpractical.com/rtir/)  -事件响应请求跟踪器（RTIR）是针对计算机安全团队的首要开源事件处理系统.  我们与全球十多个CERT和CSIRT团队合作，帮助您处理事件报告不断增加的情况.  RTIR建立在Request Tracker的所有功能之上.
...
* [Digital Evidence & Forensics Toolkit (DEFT)](http://www.deftlinux.net/)  -用于计算机取证证据的Linux发行版.  它与Windows数字高级响应工具包（DART）捆绑在一起.  还提供了DEFT的轻型版本，称为DEFT Zero，其主要侧重于法医证据收集.
...
* [inVtero.net](https://github.com/ShaneK2/inVtero.net) -具有嵌套管理程序支持的Windows x64的高级内存分析.
...
* [Magnet RAM Capture](https://www.magnetforensics.com/free-tool-magnet-ram-capture/)  -旨在捕获犯罪嫌疑人计算机物理内存的免费映像工具.  支持最新版本的Windows.
...
### Other Lists
* [Eric Zimmerman Tools](https://ericzimmerman.github.io/) -由SANS研究所的讲师Eric Zimmerman创建的法医工具的更新列表.
...
* [AMAaaS](https://amaaas.com/index.php/AMAaaS/dashboard) -在原生Android环境中执行的Android恶意软件分析即服务.
...
* [Hybrid-Analysis](https://www.hybrid-analysis.com/) -CrowdStrike提供的免费强大在线沙箱.
...
* [Reverse.IT](https://www.reverse.it/) -CrowdStrike提供的Hybrid-Analysis工具的替代域.
...
* [Visualize_Logs](https://github.com/keithjjones/visualize_logs) -用于日志的开源可视化库和命令行工具（Cuckoo，Procmon等）.
...
* [AChoir](https://github.com/OMENScan/AChoir) -框架/脚本工具，用于标准化和简化Windows实时采集实用程序的脚本编写过程.
...
* [DFIR ORC](https://dfir-orc.github.io/)  -DFIR ORC是专用工具的集合，这些工具专用于可靠地解析和收集关键文物，例如MFT，注册表配置单元或事件日志.  DFIR ORC收集数据，但不对其进行分析：它不打算对机器进行分类.  它提供了运行Microsoft Windows的计算机的法医相关快照.  该代码可以在找到 [GitHub](https://github.com/DFIR-ORC/dfir-orc).
* [FastIR Collector](https://github.com/SekoiaLab/Fastir_Collector)  -可以在实时Windows系统上收集不同工件的工具，并将结果记录在csv文件中.  通过对这些伪像的分析，可以发现早期的妥协.
* [Fast Evidence Collector Toolkit (FECT)](https://github.com/jipegit/FECT)  -光线事件响应工具包，用于在可疑Windows计算机上收集证据.  基本上，它是由与熟练操作者“事件处理程序”一起工作的非技术精通人士使用的.
