 ...
 **fresh** 是一种工具，可将其他人的 shell 配置（别名、函数等）获取到您自己的配置文件中. 我们还支持 ackrc 和 gitconfig 等文件. 把它想象成 [Bundler](https://bundler.io) 为您的点文件.
 ...
 **zit** 是 ZSH 的插件管理器. 它是最小的，因为它实现了成为插件管理器的最低要求：它允许用户从 `git` 存储库（和仅 `git` 存储库，这就是名称的原因）安装插件，源插件并更新它们. 它没有实现诸如清除已删除插件、自动编译已安装插件、oh-my-zsh/prezto/其他 ZSH 框架的别名、构建二进制文件、`$PATH` 操作等花哨的功能.
...
小巧的 ZSH 包管理器. 没有多余的装饰，没有臃肿，只有 2 kB 的 100% ZSH 代码，为您的 ZSH 环境提供完整的包管理.
 ...
 ZSH 包管理器很丰富，但大多数都臃肿、缓慢或要求过高. 最重要的是，有不少已被遗弃多年.  Zpico 并不寻求成为最好中的最好，而是在功能与小巧、快速的占用空间之间取得平衡.
...
**zpm** ( ZSH Plugin Manager ) 是一个插件管理器 [ZSH](http://www.zsh.org/) 它结合了命令式和声明式方法. 第一次运行时zpm会做一些复杂的逻辑并生成缓存，之后就只使用缓存，所以使得这个框架非常快.
...
专注于本地系统的安全性和强化. 要求 [fzf](https://github.com/junegunn/fzf)
...
**zshPlug** 是一个极简主义的插件管理器，主要基于 [zap](https://github.com/zap-zsh/zap).
...
如果您使用的是 Mac，它附带的 `zsh` 通常很陈旧. 您可以使用 `brew install zsh` 来更新它.

这里的许多主题都使用特殊字形来显示分支图标. 您需要在终端程序中使用 Nerd 字体或与电力线兼容的字体，否则您会在符号所在的位置看到丑陋的破损框.

以下是 Nerd Fonts 和 Powerline 兼容字体的一些不错来源：

* [Awesome Terminal Fonts](https://github.com/gabrielelana/awesome-terminal-fonts) - 一系列字体，包括一些漂亮的等宽图标.
* [Cascadia Code](https://github.com/microsoft/cascadia-code) - 微软的 Cascadia 代码
...
* [Hack](http://sourcefoundry.org/hack/) - 另一种为源代码和终端使用而设计的兼容电力线的字体.
* [Input Mono](https://input.djr.com/)  - 专为代码设计的字体系列. 它提供等宽字体和比例字体，并包括电力线字形.
* [Iosevka](https://be5invis.github.io/Iosevka/) - Iosevka 是一种开源的细长等宽无衬线和平板衬线字体，灵感来自 [Pragmata Pro](http://www.fsd.it/fonts/pragmatapro.htm), [M+](http://mplus-fonts.osdn.jp/) 和 [PF DIN Mono](https://www.myfonts.com/fonts/parachute/pf-din-mono/), 旨在成为编程的理想字体.
* [Monoid](http://larsenwork.com/monoid/) - Monoid 是可定制和优化的，即使在低分辨率显示器上也可以在 15 像素行高处使用类似位图的清晰度进行编码.
* [Mononoki](https://madmalik.github.io/mononoki/) - Mononoki 是 Matthias Tellen 设计的字体，旨在增强代码格式.
* [More Nerd Fonts](https://www.nerdfonts.com/font-downloads) - 另一个下载书呆子字体的网站.
* [Nerd fonts](https://github.com/ryanoasis/nerd-fonts)  - 超过 20 种修补字体（超过 1,700 种变体）和用于 Powerline、devicons 和 vim-devicons 的 fontforge 字体修补程序 python 脚本的集合：包括 Droid Sans、Meslo、AnonymousPro、ProFont、Inconsolta 等等. 这些可以用 `brew` 安装 - 做 `brew tap homebrew/cask-fonts &amp;&amp; brew install --cask fontname`
* [Powerline patched font collection](https://github.com/powerline/fonts) - 十几种字体的集合，已修补以包含电力线字形.
* [Victor Mono](https://rubjo.github.io/victor-mono/) - Victor Mono 是一种免费的编程字体，具有半连接草书斜体、符号连字（!=、&gt;&gt;、=&gt;、===、&lt;=、&gt;=、++）以及拉丁文、西里尔文和希腊文字符.
* [spacemono](https://github.com/googlefonts/spacemono) - Google 全新的原创等宽显示字体系列.
...
- [alias-finder](https://github.com/akash329d/zsh-alias-finder)  - 当您使用您之前使用别名的命令时显示别名. 有助于记住您过去定义的别名. 为了速度而编写为纯 ZSH 脚本.
...
- [archlinux (fourdim)](https://github.com/fourdim/zsh-archlinux) - 在 Arch Linux 上为 `pacman` 定义辅助函数.
...
- [asdf](https://github.com/zimfw/asdf) - 初始化 [asdf](https://github.com/asdf-vm/asdf) ，如果尚未安装，请使用 `git` 安装它. 此外，如果您使用的是绕过垫片 [direnv](https://github.com/asdf-community/asdf-direnv) 插件，按照插件的建议 [pro-tips](https://github.com/asdf-community/asdf-direnv/#pro-tips).
...
- [autoenv](https://github.com/Tarrasch/zsh-autoenv) - 如果一个目录包含 `.env` 文件，当您 `cd` 进入它时，它会自动执行.
...
- [autoupdate-oh-my-zsh-plugins](https://github.com/TamCore/autoupdate-oh-my-zsh-plugins) - [oh-my-zsh](https://ohmyz.sh/) 不会自动更新非核心插件，此插件会自动更新 `$ZSH_CUSTOM` 目录中的 `git` 存储库.
- [autovenv](https://github.com/linnnus/autovenv) - 进入父目录时自动激活 Python 虚拟环境.
...
- [base16](https://github.com/tinted-theming/base16-shell)  - 添加一个脚本以允许您更改 shell 的默认 ANSI 颜色，但最重要的是，更改 shell 的 256 色空间中的第 17 到 21 种颜色（如果您的终端支持）. 这个脚本可以在为应用程序提供额外的 base16 颜色的同时，尊重你的 shell 的原始明亮颜色（例如，亮绿色仍然是绿色等等） [Vim](https://www.vim.org).
...
- [bitwarden (casonadams)](https://github.com/casonadams/bitwarden-cli) - A [Bitwarden](https://bitwarden.com/download/) CLI模糊查找器使用 [fzf](https://github.com/junegunn/fzf) . 要求 [jq](https://stedolan.github.io/jq/).
...
- [cmd-time](https://github.com/TomfromBerlin/zsh-cmd-time)  - 收集命令的执行时间并将结果导出到可以在其他地方使用的变量. 它类似于内置 [REPORTTIME](http://zsh.sourceforge.net/Doc/Release/Parameters.html) 功能，但也略有不同. 与设置 REPORTTIME 不同，它考虑用户和系统时间，而不仅仅是 CPU 时间.
...
- [coffee-time](https://github.com/gakimball/zsh-coffee-time)  - 添加运行 `caffeinate -dims` 的 `caf` 别名. 额外的标志让一切保持清醒：系统、驱动器和显示器.
...
- [confer](https://github.com/SleepyBag/zsh-confer) - 尝试自动查找程序配置文件，以便您可以执行类似 `conf vim` 的操作来编辑您的 `vim` 配置文件.
...
- [deno (tricked-dev)](https://github.com/Tricked-dev/deno-zsh-plugin) - 自动安装 [deno](https://deno.land/) 如果尚未安装 deno，则在启动时添加到 `$HOME/.deno`.
...
- [dietpi](https://github.com/unixorn/dietpi.plugin.zsh) - 添加实用程序 [dietpi](https://dietpi.com) 当您登录到正在运行的机器时，到您的 `$PATH`（并包括别名以使用 `sudo` 自动运行它们）  [dietpi](https://dietpi.com).
...
- [ez-cmd](https://github.com/akgarhwal/ez-cmd) - 通过提供易于使用的快捷方式和别名来简化常见的命令行任务.
...
- [fast-alias-tips](https://github.com/decayofmind/zsh-fast-alias-tips)  - 帮助记住您定义和忘记的别名. 移植自 [djui/alias-tips](https://github.com/djui/alias-tips) . 主动分叉 [sei40kr/zsh-fast-alias-tips](https://github.com/sei40kr/zsh-fast-alias-tips).
...
- [firebase (rmrs)](https://github.com/rmrs/firebase-zsh) - 在提示中添加一个指示器，表明您位于带有 `firebase.json` 文件（又名“firebase 项目”）的目录中.
...
- [fnm (dominik-schwabe)](https://github.com/dominik-schwabe/zsh-fnm) - 安装并加载 [Fast Node Manager (fnm)](https://github.com/Schniz/fnm) 如果它丢失了.
- [fnm (wintermi)](https://github.com/wintermi/zsh-fnm) - 快速简单的 Node.js 版本管理器的帮助插件 [fnm](https://github.com/Schniz/fnm).
...
- [fzf-plugin](https://github.com/Atlas34/fzf-plugin) - [oh-my-zsh](https://github.com/ohmyzsh/ohmyzsh)的 `fzf` 插件已提取，因此可以轻松地与其他插件管理器一起使用.
...
- [git-check](https://github.com/git-girl/git-check) - 添加 zsh 钩子以检查当前分支的来源是否在后台更改为本地并发送通知.
...
- [gitsync](https://github.com/washtubs/gitsync) - ZSH 插件，用于改进一个人在多台机器上的同一存储库上开发的工作流程.
...
- [gpt](https://github.com/antonjs/zsh-gpt) - 从命令行启用查询 ChatGPT.
...
- [haiku](https://github.com/alesr/oh-my-zsh-haiku-plugin) - 在终端打开时每 24 小时打印一次促进工作与生活平衡和压力管理的俳句.
...
- [history-search-multi-word](https://github.com/zdharma-continuum/history-search-multi-word) - 用于 ZSH 的语法突出显示的多词历史搜索器，绑定到 Ctrl-R，具有高级功能（例如历史条目到历史顶部的碰撞）.
...
- [iwd](https://github.com/zshzoo/iwd) - 在概念上与 `$PWD` 类似，此 ZSH 插件将您的初始工作目录保存在 `$IWD` 中，以便轻松返回到会话的起点.
...
- [laravel (baliestri)](https://github.com/baliestri/laravel.plugin.zsh) - 用于在运行 `artisan` 命令时跳过 `php` 命令和在运行 `sail` 命令时跳过 `./sail` 或 `./vendor/bin/sail` 的插件.
...
- [lazy-load](https://github.com/goarano/zsh-lazy-load) - 仅在您实际需要时延迟加载选项卡完成.
...
- [lux](https://github.com/pndurette/zsh-lux)  - ZSH 插件，用于通过 `lux` 命令切换 macOS、iTerm 2、Visual Studio Code 和其他项目和应用程序的明暗模式. 高度可定制：包含的项目可以通过定义变量来配置. 高度可扩展：可以通过定义功能来添加项目. 包括一个 `macos_is_dark` 辅助函数来确定 macOS 黑暗模式是否处于活动状态以用于主题.
...
- [macos-theme](https://github.com/gakimball/zsh-macos-theme)  - 添加主题命令，在 macOS 中切换浅色和深色模式. 要求 [lux](https://github.com/pndurette/zsh-lux) 插入.
...
- [multi-evalcache](https://github.com/rwwiv/multi-evalcache) - 缓存多个 eval 加载以缩短启动时间，灵感来自 [mroth/evalcache](https://github.com/mroth/evalcache).
...
- [nodo](https://github.com/nicolodiamante/nodo)  - 此插件可帮助您通过取消同步目录来防止 `node_modules` 目录填满您的 iCloud 存储空间，或者通过删除所选根目录中的所有 `node_modules` 目录来节省更多空间. 这对于清理具有多个 node_modules 树的项目特别有用
...
- [oh-my-tmux-manager](https://github.com/unixorn/awesome-zsh-plugins/blob/master/omt-manager) - 让您轻松管理您的 tmux 配置.
...
- [pack](https://github.com/fourdim/zsh-pack/) - 用 ZSH 打包你的源代码.
...
- [penmux](https://github.com/mfulz/zsh-penmux) - 一个会话管理器插件，用于渗透测试会话和跟踪报告中使用的终端会话.
...
- [pins](https://github.com/mehalter/zsh-pins)  - 用于固定目录的 ZSH 插件. 就像带有制表符补全的 CLI 文件夹书签管理器.
...
- [pseudo-projectile](https://github.com/git-girl/pseudo-projectile-plugin/)  - 就像 emacs 中的 `projectile`？ 这通过在 `nvim` 中打开 `fzf`-selected 路径来为您的 ZSH 环境添加类似的功能. 作者弃用.
...
- [ranger](https://github.com/rc2dev/ranger-zshz) 集成 [zsh-z](https://github.com/agkozak/zsh-z) 进入 [ranger](https://github.com/ranger/ranger).
...
- [rust (cowboyd)](https://github.com/cowboyd/zsh-rust) - 配置你 [rust](https://www.rust-lang.org/) 工具链，安装 [rustup](https://rustup.rs) 如果当前尚未安装.
...
- [saneopt](https://github.com/willghatch/zsh-saneopt) - ZSH 选项的 Sane 默认值，本着 [vim-sensible](https://github.com/tpope/vim-sensible).
...
- [setpath](https://github.com/mys721tx/set_path) - 添加一些本地路径到你的 `fpath` 和 `$PATH`.
...
- [show-git-user](https://github.com/luisprgr/zsh-show-git-user) - 当您需要在同一台机器上与多个 `git` 用户一起工作时，此插件将在您的提示中显示哪个 `git` 用户名处于活动状态.
...
- [ssh-quickconnect](https://github.com/breda/zsh-ssh-quickconnect) - 简单的实用程序，可从您的 `ssh` 配置和 `known_hosts` 文件快速连接到主机.
...
- [virtualz](https://github.com/aperezdc/virtualz) - Python [virtualenv](https://virtualenv.pypa.io/en/latest/) manager inspired by Adam Brenecki's [Virtualfish](https://github.com/adambrenecki/virtualfish) 为了 [Fish shell](http://fishshell.com/), 替换了 virtualenvwrapper.
...
- [vivid](https://github.com/ryanccn/vivid-zsh) - 更容易使用 `LSCOLORS` [vivid](https://github.com/sharkdp/vivid).
...
- [vtex](https://github.com/xdigu/zsh-vtex) - 添加辅助别名 [vtex](https://developers.vtex.com/vtex-developer-docs/docs/vtex-io-documentation-vtex-io-cli-command-reference#default-commands) cli 命令.
...
- [wsl](https://github.com/florentinl/omz-wsl) - 添加辅助函数，以便在 WSL 中运行时更容易在 ZSH 中工作.
...
- [zinit-annex-meta-plugins](https://github.com/zdharma-continuum/zinit-annex-meta-plugins) - 安装带有单个标签的插件组（[zinit](https://github.com/zdharma-continuum/zinit) 仅有的）.
...
- [zman](https://github.com/mattmc3/zman) - 使用 [fzf](https://github.com/junegunn/fzf) 快速浏览 ZSH 手册.
- [znotify](https://github.com/rudeigerc/znotify) - 一个用于向其他服务发送通知的简单插件.
- [znvm](https://github.com/Ajnasz/znvm) - A [nodejs](https://nodejs.org) ZSH 的版本管理器类似于 [nvm.sh](https://github.com/nvm-sh/nvm) 但更快.
...
- [zshrpg](https://github.com/aliervo/zshrpg) - 完全集成的包装器 [rpg-cli](https://github.com/facundoolano/rpg-cli/) 用 zsh！
...
- [zypper-short](https://github.com/justanotherinternetguy/zypper-short) - OpenSuse Tumbleweed 包管理器的插件，`zypper`.
...
- [argc-completions](https://github.com/sigoden/argc-completions) - 用途 [argc](https://github.com/sigoden/argc) 和 [jq](https://github.com/stedolan/jq) 添加 ZSH 选项卡完成.
- [autopkg-zsh-completion](https://github.com/fuzzylogiq/autopkg-zsh-completion) - 完成 autopkg.
- [autorestic](https://github.com/naegling/zsh-autorestic) - 自动安装 [Restic](https://github.com/cupcakearmy/autorestic/)为您完成，并随着您的自动版本更改而使它们保持最新.
...
- [bosh (thomasmitchell)](https://github.com/thomasmitchell/bosh-complete) - 选项卡完成 [BOSH](https://github.com/cloudfoundry/bosh)].
...
- [buidler](https://github.com/gonzalobellino/buidler-zsh) - 为 NomicLabs Buidler 工具添加完成和有用的别名.
...
- [cross-compiler](https://github.com/Freed-Wu/zsh-completions-for-cross-compilers) - 在交叉编译中，有很多工具，比如x86_64-w64-mingw32-gcc、x86_64-linux-android32-clang、arm-none-eabi-gcc等.这个插件为它们提供了zsh补全.
...
- [fluxcd](https://github.com/l-umaca/omz-fluxcd-plugin) - 为 [FluxCD command line](https://fluxcd.io/flux/cmd/) 工具，以及一些最常见的通量命令的别名.
...
- [fvm](https://github.com/olrtg/zsh-fvm) - 为 [Flutter Version Manager (FVM)](https://fvm.app/).
...
- [gcloud (littleq0903)](https://github.com/littleq0903/gcloud-zsh-completion) - 为 Google Cloud SDK 添加补全.
- [gcloud (wintermi)](https://github.com/wintermi/zsh-gcloud) - 添加谷歌云命令行界面（[gcloud](https://cloud.google.com/cli) CLI) 完成.
...
- [kubeadm](https://github.com/gantsign/zsh-plugins/tree/master/kubeadm) - 添加选项卡完成 [kubeadm](https://kubernetes.io/docs/reference/setup-tools/kubeadm/).
...
- [poetry](https://github.com/fourdim/zsh-poetry) - 选项卡完成 [poetry](https://python-poetry.org/).
...
- [xcode](https://github.com/keith/zsh-xcode-completions) - 完成一些 Xcode 命令行工具 - `genstrings`、`nm`、`plutil`、`xcode-select`、`xcodebuild`、`xcrun`、`simctl`、`strings`、`swift-demangle`、` swift` 和 `lipo`.
...
- [abhiyan](https://github.com/abhiyandhakal/abhiyan.zsh/)  - 分段提示. 包括 `git` 分支、暂存文件数、未暂存文件数和未跟踪文件数、用户名、当前工作目录和时间的装饰器. 需要与电力线兼容的字体.
...
- [ai-hayasaka](https://github.com/aeghost/ai-hayasaka-zsh-theme) - 带有 `git` 状态、ruby env 和 python virtualenv 装饰器的极简主义主题.
...
- [astral (xwmx)](https://github.com/xwmx/astral)  - 带有禅宗模式的深色背景主题. 适用于 zsh 用户 [zsh-syntax-highlighting](https://github.com/zsh-users/zsh-syntax-highlighting) 插入. 包括最后一条命令的执行时间、运行时间、退出状态、机器名称、当前路径、`ssh` 状态和 `git` 状态的装饰器.
...
- [australis](https://github.com/Kimitzuni/australis-theme)  - 带有 `git` 信息和当前目录装饰器的轻量级主题. 需要 `git` 插件来自 [oh-my-zsh](https://github.com/ohmyzsh).
...
- [avoleo](https://github.com/flameleo11/avoleo-zsh-theme)  - 为每个命令提供日期和时间提示，以及历史记录中的命令编号. 此外，如果适用于当前路径，它会使用特殊符号 &#39;⠾&#39; 和 &#39;‡&#39; 来显示 `git` 信息. 它还支持基于 Gnome-Terminal 默认调色板的自定义颜色.
...
- [backbone](https://github.com/lmllrjr/backbone-zsh-prompt)  - 最低限度的单个文件提示，像走鹃一样快. 包括当前目录、`git`、`hg`、`mq` 和 `quilt` 信息的装饰器.
...
- [bashlover](https://github.com/Vu0811/bashlover)  - 专为那些喜欢 ZSH shell 的强大功能但仍然喜欢类似于 `bash` shell 的简单经典界面的人而设计. 包括 `git` 信息、user@host 和当前工作目录的装饰器
...
- [brisa](https://github.com/ambrisolla/oh-my-zsh-brisa-theme) - 基于多行主题 [fino-time](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/fino-time.zsh-theme) . 包括用户名、主机、当前目录和 `git` 状态的装饰.
...
- [calma](https://github.com/luislve17/calma)  - 极简主义主题，适用于深色背景. 包括用于截断当前目录、`git` 信息、时间和最后一个命令的退出状态的装饰器.
...
- [cheeky](https://github.com/kampanosg/zsh-cheeky-prompt) - 包括鸡表情符号、当前目录的装饰器、`git` 信息以及当前的 GCP 集群和项目.
...
- [cmder](https://github.com/potasiyam/cmder-zsh-theme)  - 与 Cmder 主题匹配的 ZSH 主题，Cmder 是一种流行的 Windows 终端仿真器. 包括 `node` 和 `git` 状态装饰.
...
- [cybensis](https://github.com/cybensis/cybensis-zsh-theme) - 基于 [af-magic](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/af-magic.zsh-theme) . 包括 `git` 信息、`hg` 信息和 python virtualenv 的装饰器.
...
- [dfrx](https://github.com/Dofoerix/Dfrx-Prompt-Theme)  - Oh-My-Posh 主题. 包括当前目录的装饰器、最后一条命令的执行时间、root 状态和时间.
...
- [doom](https://github.com/CMOISDEAD/doom-zsh)  - 厄运灵感. 看起来类似于电力线. 具有可定制的段、`git` status、`rust`、`node.js`、`python` 和 `ruby` 版本的装饰器.
...
- [elagoht](https://github.com/Elagoht/Elagoht.zsh-theme) - 包括 user@hostname、当前目录、虚拟环境、`git` 状态、是否在 `ssh` 会话中运行以及最后一条命令的执行时间的装饰器.
...
- [emojeer](https://github.com/lxynox/emojeer-ohmyzsh) - 表情符号味 [oh-my-zsh](https://ohmyz.sh/) 主题.
...
- [fe80](https://github.com/fe80/fe80.zsh-theme) - 包括 `git` 信息、当前目录、user@hostname、时间和非零时最后一条命令的返回码的装饰器.
...
- [gallifrey-war](https://github.com/cdubos-fr/gallifrey-war)  - 灵感来自 [gallifrey](). 包括 `git` 信息、user@host 和当前目录的装饰器.
...
- [gianu-alternative](https://github.com/zbentzinger/gianu-alternative-theme) - 替代品 [OMZ Gianu](https://github.com/ohmyzsh/ohmyzsh/blob/61dd3682e69aa990a8a3589c5c61ea2e1edf8312/themes/gianu.zsh-theme) 根据特权更改提示. 包括 `git` 状态和当前目录装饰器.
...
- [git-prompt (woefe)](https://github.com/woefe/git-prompt.zsh) - 一个快速的、可定制的、纯 shell 的、异步的 `git`-aware 提示，用于 ZSH，深受 Olivier Verdier 的启发 [zsh-git-prompt](https://github.com/olivierverdier/zsh-git-prompt) 和fish shell的“Informative VCS”提示非常相似.
...
- [goprompt](https://github.com/NonLogicalDev/shell.async-goprompt)  - 快如闪电. 包括用于截断当前目录的装饰器、上次命令持续时间和退出状态、vim 模式指示器、`git` 信息、日期时间和父进程名称.
...
- [hex](https://github.com/hectorBrown/hex-zsh) - 很大程度上基于 [bira](https://github.com/ohmyzsh/ohmyzsh/wiki/Themes#bira)， 和 [gruvbox](https://github.com/sbugzu/gruvbox-zsh), 这又是基于 [agnoster](https://gist.github.com/agnoster/3712874) . 包括当前目录的装饰器、`git` 状态信息、活动的 python virtualenv、最后一个命令运行的退出状态. 需要与电力线兼容的字体.
...
- [hoffish](https://github.com/emilHof/hoffish-zsh-theme) - 如果 [agnoster](https://github.com/agnoster/agnoster-zsh-theme) 主题和 [fish](https://fishshell.com/)  shell 有一个儿童 ZSH 主题. 包括 `git` 状态的装饰器、当前目录的修剪路径、root 状态、最后一个命令运行的退出状态和活动的 python virtualenv. 需要电力线字体和 [zsh-autosuggestions](https://github.com/zsh-users/zsh-autosuggestions) 和 [shrink-path](https://github.com/ohmyzsh/ohmyzsh/blob/master/plugins/shrink-path/shrink-path.plugin.zsh) 插件.
...
- [htb](https://github.com/Idan1993/zsh-htb-theme)  - 灵感来自 HackTheBox 机器上的 pwnbox. 包括 user@host、IP 地址和当前目录的装饰器.
...
- [kali-like](https://github.com/clamy54/kali-like-zsh-theme)  - 受 Kali Linux 默认 ZSH 主题的启发. 包括 user@host、当前目录和 `git` 信息的装饰器.
...
- [lukerandall-extended](https://github.com/mpyw/oh-my-zsh-lukerandall-extended) - Extended version of the [lukerandall](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/lukerandall.zsh-theme) 主题. 包括 `git` 状态的装饰和最后一个命令运行的状态.
...
- [magento](https://github.com/cmuench/zsh-magento-cloud/blob/main/zsh-magento-cloud.plugin.zsh) - Add Magento Cloud Command Line Interface ([magento-cloud CLI](https://experienceleague.adobe.com/docs/commerce-cloud-service/user-guide/dev-tools/cloud-cli.html?lang=en)) 完成.
...
- [materialshell](https://github.com/carloscuesta/materialshell) - A [material design](https://material.io/guidelines/style/color.html) 外壳的主题具有良好的对比度和重要部分的颜色. 专为保护眼睛而设计.
...
- [msys2](https://github.com/water-logger/MSYS2-Theme/)  - 受 MSYS2 启发. 包括 user@host、`git` 状态和当前目录的装饰器.
...
- [parrot](https://github.com/trabdlkarim/parrot-zsh-theme)  - 基于 Parrot OS bash 主题. 包括 user@host 的装饰器、`git` 信息、上一个命令的退出状态、时间和当前目录.
- [passion](https://github.com/ChesterYue/ohmyzsh-theme-passion)  - 包括当前时间、`git` 状态、最后一个命令运行时间（以毫秒为单位）以及最后一个命令的退出状态的装饰. 在 macOS 上需要 coreutils.
...
- [platypus](https://github.com/fdv/platypus) - Platypus 是一个简单方便的 oh-my-zsh 主题，由 Frédéric de Villamil 使用.
...
- [powerline-hs](https://github.com/rdnetto/powerline-hs) - A [Powerline](https://github.com/powerline/powerline) 用 Haskell 编写的克隆. 它比原来的实现要快得多，并且使 shell 的响应速度明显提高.
...
- [powerline-shell (banga)](https://github.com/b-ryan/powerline-shell) - A [powerline](https://github.com/Lokaltog/vim-powerline) - 类似 Bash、ZSH 和 Fish 的提示符. 包括 `git`/`svn`/`hg`/`fossil` 分支的装饰器、最后一个命令退出状态、当前目录的缩短路径和当前 python virtualenv，并且易于定制/扩展.
- [powerline-train](https://github.com/sherubthakur/powerline-train) - 电力线变体.
- [powerline](https://github.com/carlcarl/powerline-zsh) - A [Powerline](https://github.com/Lokaltog/vim-powerline)-like提示，基于 [powerline-bash](https://github.com/milkbikis/powerline-bash) . 显示 virtualenv、`git` 状态信息和最后一次命令运行的退出代码.
...
- [prowpt](https://github.com/alpaca-honke/prowpt) - 简单、轻量级和可自定义的类似电力线的提示，带有用于 `git` 信息、用户、主机名、当前目录、时间和最后一个命令的退出状态的装饰器.
...
- [renanborgez](https://github.com/renanborgez/ohmyzsh-theme-renanborgez)  - 适用于深色背景. 包括 `nvm` 和 `git` 信息的装饰器.
...
- [risbow](https://github.com/waddupp00/risbow) - A [risto](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/risto.zsh-theme) ZSH 主题的灵感来自 lolcat 般的彩虹效果.
- [river](https://github.com/revir/river-zsh-config) - 带有 `git` 信息的深色主题.
- [river_dreams](https://github.com/skippyr/river_dreams) - 包括 `git` 信息、机器的 IP 地址、时间、`/` 上的磁盘使用情况、当前 Python `venv`、当前目录、root 状态和最后一个命令的退出状态的装饰器.
...
- [roundy](https://github.com/nullxception/roundy)  - 快速、可爱、圆润的主题. 包括 `git` 状态、当前目录和上次命令执行时间的装饰器. 需要书呆子字体和支持 unicode 的终端应用程序.
...
- [sailormoon](https://github.com/Domanowska/zshSailorMoonThemes) - 美少女战士主题曲集.
...
- [searocket](https://github.com/dk949/searocket/) - 精简版 [spaceship](https://github.com/denysdovhan/spaceship-prompt) . 包括工作目录、最后命令退出代码、用户、后台作业、`bun`、`d`、elm、go、nodejs、python、zig 和 `git` 状态的装饰器. 需要`D`构建链.
...
- [shinkansen](https://github.com/MRZ07/shinkansen.zsh-theme)  - 快速、可定制且易于扩展的主题. 包括活动 virtualenv 中 rhw python 版本的装饰器，当前 ruby​​ 版本（如果您正在使用 `chruby`），当前 node.js 版本，当前 java 版本，当前 go 版本，当前 perl 版本（如果使用 `chperl`），当前 elixir 版本， `git` 状态、时间、当前目录、退出代码和最后一条命令的执行时间，以及可选的自定义消息. 需要电力线兼容的字体.
...
- [simply-perfect](https://github.com/SetOfAllSets/simply-perfect-zsh-theme/)  - 让人想起电力线和子弹头列车. 包括 `git` 状态、当前目录、上次命令退出状态、当前时间和用户名的装饰器.
...
- [starship (wintermi)](https://github.com/wintermi/zsh-starship) - 一个使用 Starship 提示符以及电力线主题的简单插件.
- [starship2k](https://github.com/2KAbhishek/starship2k) - 包括电力线支持、`git` 信息的装饰器、多种语言和多行提示.
...
- [thygod](https://github.com/Thy-GoD/thy-god-zsh-theme) - 基于关闭 [gnzh](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/gnzh.zsh-theme) 和 [bira](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/bira.zsh-theme). Includes a `git` status decorator 和 changes prompt to a red cross when a comm和 fails.
...
- [unicorn](https://github.com/juliuscaesar/unicorn)  - 包括 root 状态、virtualenv、nvm、rvm、当前目录、时间、当前目录和 emoji `git` 信息的装饰器. 灵感来自 [wild cherry](https://github.com/mashaal/wild-cherry).
...
- [xor](https://github.com/xor3n/xor-zsh-theme) - 自我描述为简约和“功能贫乏”，包括 `git` 装饰.
...
- [yazpt](https://github.com/jakshin/yazpt) - 一个干净、快速、美观的 ZSH 提示主题，精心整合了 Git/Subversion/TFVC 状态信息，与流行的插件管理器（如 Oh My Zsh）集成，并且易于定制和扩展.
...
- [zcraft](https://github.com/cpea2506/zcraft) - 带有 `git` 状态、最后一个命令退出状态和最后一个命令所用时间的装饰的极简主义主题.
...
- [zemm-blinks](https://github.com/aranasaurus/zemm-blinks.zsh-theme) - 自定义版本的oh-my-zsh [blinks](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/blinks.zsh-theme) 有了 Mercurial 支持和其他更​​改.
...
- [zinc](https://gitlab.com/robobenklein/zinc) - 一个超快、纯 ZSH、混合异步提示，灵感来自 [Powerlevel9k](https://github.com/bhilburn/powerlevel9k) 和 [Agnoster](https://github.com/agnoster/agnoster-zsh-theme) that's easily extensible 和 extremely configurable. It supports async segments using [zsh-async](https://github.com/mafredri/zsh-async).
...
这些插件中的大多数都可以通过将 zgenom load githubuser/reponame 添加到您的 .zshrc 文件中，在您正在执行其他 zgenom load 调用的同一函数中进行安装.
...
- [oh-plugin](https://github.com/mbergo/oh-plugin) - 帮助您安装插件 [oh-my-zsh](https://ohmyz.sh) 通过输入`oh-plugin install repository_address`.
