...
ZSH框架，插件，教程和主题的集合受到那里各种出色列表的启发.
 ...
 ** ansible-role-zsh **是零知识安装的ansible角色. 它使用抗原来管理束和oh-my-zsh. 可以有条件地加载捆绑包. 默认情况下，它包括powerlevel9k主题，自动建议，语法突出显示和 [fzf-widgets](https://github.com/ytet5uy4/fzf-widgets) 和 [fzf-marks](https://github.com/urbainvaes/fzf-marks) . 完全可定制.

### [ant-zsh](https://github.com/anthraxx/ant-zsh)

 ** Ant-zsh **是一个纤巧而轻巧的ZSH配置环境，可满足特殊的自定义需求. 它包括插件，主题和基本的便捷设置.
 ...
 ** Antigen **是一小组功能，可帮助您轻松管理称为捆绑软件的Shell（ZSH）插件. 这个概念与典型的vim + pathogen设置中的bundle几乎相同. 抗原是ZSH，Vundle是vim.  Antigen可以加载oh-my-zsh主题和插件，并会自动为您克隆它们.
 ...
 ** Dotzsh **致力于成为平台和版本无关的平台. 在较低版本的ZSH上运行时，某些功能可能会丢失，但是它应彻底降级，并允许您在不同操作系统的多台计算机上使用相同的设置，而不会出现问题.

### [fresh](https://github.com/freshshell/fresh)

 **新鲜**是一种工具，可以将其他人的外壳配置（别名，功能等）导出到您自己的配置文件中. 我们还支持ackrc和gitconfig等文件. 将其视为点文件的捆绑器.
...
### [PMS](https://github.com/JoshuaEstes/pms)
 ...
 PMS允许您以一种有助于减少设置时间并提高生产率的方式来管理外壳. 它支持主题（更改外壳的外观），插件（向外壳添加功能）和点文件管理.
...
### [pz](https://github.com/mattmc3/pz)
 ...
 ZSH的插件管理器不必复杂就可以强大. 当PZ可以很聪明时，它不会尝试变得聪明.  PZ是功能齐全，快速且易于理解的插件管理器，封装在一个文件中，其中包含约200行干净的ZSH.

PZ确实可以很好地管理您的ZSH插件，然后让您摆脱了困境.
...
** Zgem **是ZSH的插件管理器，支持从git，http和本地文件中加载和更新插件和主题.

### [zgen](https://github.com/tarjoilija/zgen)

 ** Zgen **是受Antigen启发的ZSH轻量级插件管理器. 目的是在启动外壳程序时将开销降至最低，因为没人喜欢等待. 目前尚未积极对其进行维护，建议您使用 [zgenom](https://github.com/jandamm/zgenom) 而是使用fork.

### [zgenom](https://github.com/jandamm/zgenom)

ZSH的轻量级插件管理器，它是扩展了辉煌的分支 [zgen](https://github.com/tarjoilija/zgen) 并提供了更多功能和错误修正，同时完全向后兼容.

为了在新的终端会话期间保持快速加载，`zgenom`会生成一个静态的`init.zsh`文件，该文件除了提供插件并将其附加到`fpath`之外什么也不做.

通过在启动过程中不必执行耗时的逻辑（插件检查，更新等），可以将启动时间最小化. 缺点是，每当更新.zshrc中的插件列表时，都必须使用zgenom reset手动刷新init脚本.

Zgenom可以加载 [oh-my-zsh](http://ohmyz.sh/)兼容和 [prezto](https://github.com/sorin-ionescu/prezto)兼容的插件和主题，当您将它们添加到插件列表中时，它们会自动为您克隆`git`克隆插件.
...
** Zinit **是一种创新产品，可能（由于Turbo） [fastest](https://github.com/zdharma/pm-perf-test) 插件管理器，支持：
 ...
 -Turbo模式– Zsh启动速度提高80％！ 例如：而不是200毫秒，而是40毫秒，
-完成管理（有选择地禁用和启用完成），
-片段（↔常规文件，通过URL下载，例如：脚本），并通过它们哦我的Zsh和Prezto插件支持（→低开销），
-附件（↔Zinit扩展名），
-报告（来自插件加载-插件不再是黑匣子），
-插件卸载（例如：动态主题切换），
-`bindkey`捕获和重新映射，
...
- [services](https://github.com/zservices) single单实例，后台插件，
-同样，通常：Zsh插件标准中的所有机制– Zinit是该标准的参考实现.

奖励：您可以使用 [zinit-console](https://github.com/zinit-zsh/zinit-console) 查看和更改ZSH会话的状态（例如：列出并卸载插件），并从磁盘上删除插件和代码片段.
 ...
 **：zap：Znap **是ZSH的轻量级插件管理器和Git存储库管理器，易于理解. 尽管Znap是专门为ZSH插件量身定制的，但它也可以用作管理Git仓库的通用工具.
...
** zpm **（ZSH插件管理器）是用于 [ZSH](http://www.zsh.org/) 谁结合了命令式和声明式方法. 第一次运行时，zpm将执行复杂的逻辑并生成高速缓存，此后将仅使用高速缓存，因此它使该框架变得非常快.
...
** Zulu **是ZSH 5+的环境管理器，旨在使您无需编写任何代码即可轻松管理您的Shell.

*轻松管理您的Shell环境，而无需编辑文件.
*创建别名，函数和环境变量，并在下次启动Shell时将它们提供给您.
...
* [zsh-quickstart-kit](https://github.com/unixorn/zsh-quickstart-kit) -在以下情况下使用ZSH的简单快速入门 [zgenom](https://github.com/jandamm/zgenom) . 其中包括精选的插件集，可自动配置ZSH以使用 [zgenom](https://github.com/jandamm/zgenom) 加载它们并定期自动更新自身，插件和快速入门工具包本身.
...
* [How To Make an Awesome Custom Shell with ZSH](https://linuxstans.com/how-to-make-an-awesome-custom-shell-with-zsh/) -关于如何安装和配置ZSH Shell的入门指南.
...
* [rs-example](https://github.com/al-jshen/zshplug-rs-example) -一个示例插件，显示Rust程序如何侦听和处理来自ZSH的命令.
...
* [ZSH for Humans](https://github.com/romkatv/zsh4humans)  -ZSH的成套配置，旨在开箱即用. 它将一组精选的ZSH插件组合成一个连贯的整体，感觉就像是成品，而不是DIY入门套件.
...
* [mgdm.net/weblog/zsh-antigen/](https://mgdm.net/weblog/zsh-antigen/) -迈克尔·麦克林（Michael Maclean）关于从oh-my-zsh转换为抗原的文章.
* [Oh-my-zsh is the Disease and Antigen is the Vaccine](https://joshldavis.com/2014/07/26/oh-my-zsh-is-a-disease-antigen-is-the-vaccine/) -乔什·戴维斯（Josh Davis）对Antigen的介绍.
...
* [Oh-My-Zsh! A Work of CLI Magic](https://medium.com/wearetheledger/oh-my-zsh-made-for-cli-lovers-installation-guide-3131ca5491fb) -适用于Ubuntu的Michiel Mulders安装指南
...
* [256color](https://github.com/chrissicool/zsh-256color)  -增强了256色的终端环境. 它查看所选的“ TERM”环境变量，并查看是否有相应的ncurses术语，其中256种颜色可用. 结果是一个多色端子（如果有）.
* [abbr (olets)](https://github.com/olets/zsh-abbr) -管理自动扩展的缩写，当您打到太空时，它会受到鱼壳的启发而在行内扩展.
* [abbr-path](https://github.com/felixgravila/zsh-abbr-path) -从“ oh-my-fish”主题中添加了“ theme_title_use_abbreviated_pa​​th”参数的功能.
* [abbrev-alias](https://github.com/momo-lab/zsh-abbrev-alias) -提供类似于`vim`的缩写扩展功能.
* [accurev](https://github.com/dalefukami/accurev-zsh) -适用于的ZSH插件 [accurev](https://www.microfocus.com/en-us/products/accurev/overview).
* [actiona](https://github.com/matthieusb/act) -使通话更轻松 [actiona](https://github.com/Jmgr/actiona) 命令行中的脚本. 包括制表符补全.
...
* [alt-and-select](https://github.com/raisty/alt-and-select)  -将alt-c（复制），alt-v（粘贴），alt-x（剪切）键盘快捷键绑定到以下命令：copy-region-as-kill，yank，kill-region. 将execute命令重新映射到Alt-Shift-X.
...
* [antigen-git-rebase](https://github.com/smallhadroncollider/antigen-git-rebase) -Antigen / ZSH脚本可帮助`git`重新定标.
...
* [apple-touchbar](https://github.com/zsh-users/zsh-apple-touchbar) -在MacBook Pro中增加了对MacBook Pro触控栏的支持 [iTerm 2](https://iterm2.com).
* [appup](https://github.com/Cloudstek/zsh-plugin-appup)  -当在当前目录（例如您的应用程序）中检测到docker-compose.yml或Vagrantfile时，添加start，stop，up和down命令. 只需运行`up`并获取编码！
* [arc](https://github.com/anton-rudeshko/zsh-arc) -为Yandex版本控制系统添加别名.
* [arduino](https://github.com/raghur/zsh-arduino)  -添加脚本以从命令行构建，上载和监视arduino草图. 需要 [`jq`](https://stedolan.github.io/jq/).
...
* [async](https://github.com/mafredri/zsh-async)  -在ZSH中运行异步任务而无需任何外部工具的库. 允许您运行多个异步作业，强制执行唯一作业（同一作业的多个实例将不会运行），刷新所有当前正在运行的作业以及创建多个工作程序（每个工作程序都有自己的工作）.
...
* [auto-fortune-cowsay](https://github.com/babasbot/auto-fortune-cowsay-zsh) -在每个zsh起点上打印出一头母牛的ASCII图片，并说出随机的墓志铭.
...
* [autoenv](https://github.com/Tarrasch/zsh-autoenv) -如果目录包含`.env`文件，则在您`cd`进入该目录时将自动执行该文件.
...
* [autosuggestions](https://github.com/zsh-users/zsh-autosuggestions) - [Fish](https://fishshell.com/)像ZSH的快速/不打扰的自动建议.
...
* [aws2](https://github.com/drgr33n/oh-my-zsh_aws2-plugin) -提供对版本2的完成支持 [awscli](https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html) 和一些实用程序来管理AWS配置文件并在提示中显示它们.
...
* [base16](https://github.com/chriskempson/base16-shell)  -添加脚本以允许您更改外壳的默认ANSI颜色，但最重要的是，可以更改外壳256色空间的17到21的颜色（如果终端支持）. 使用此脚本可以保留外壳的原始明亮颜色（例如，明亮的绿色仍然是绿色等），同时为诸如以下的应用程序提供其他base16颜色 [Vim](https://www.vim.org).
* [baseballfunfacts](https://github.com/richardmoyer/baseballfunfacts)  -在您的外壳上打印与棒球相关的随机“有趣的事实”. 取决于安装的`fortune`和`cowsay`.
...
* [bash](https://github.com/chrissicool/zsh-bash)  -使ZSH更加兼容Bash. 它重新定义了source命令，使其行为更像`bash`. 它还启用了“ bash”补全功能.
...
* [bol](https://github.com/ikhurramraza/bol) -打开终端窗口时，打印随机报价.
* [boss-docker](https://github.com/bossjones/boss-docker-zsh-plugin) -在OSX上管理`docker`.
...
* [careful_rm](https://github.com/MikeDacre/careful_rm) -rm包装纸，可添加垃圾/回收物和有用的警告信息.
* [case](https://github.com/rtuin/zsh-case) -一个ZSH插件，添加了两个别名“ tolower”和“ toupper”来切换输出大小写.
* [cd-gitroot](https://github.com/mollifier/cd-gitroot) -ZSH插件，用于`cd`到`git`仓库根目录.
* [cd-reminder](https://github.com/bartboy011/cd-reminder) -在进入指定目录的cd时显示提醒.
...
* [chgo](https://github.com/sbfaulkner/chgo-plugin-zsh) -修改了chruby的克隆，使其易于在多个Go版本之间进行切换.
...
* [colorls](https://github.com/Kallahan23/zsh-colorls) -为一些colorls功能定义了一些有用的快捷方式.
* [colors (Tarrasch)](https://github.com/Tarrasch/zsh-colors)  -使从CLI着色文本变得更加容易.  `red foo`可以正常工作.
...
* [command-not-found](https://github.com/Tarrasch/zsh-command-not-found) -oh-my-zsh命令未找到插件的镜像，因此您不必包括oh-my-zsh的所有内容.
...
* [completion-generator](https://github.com/RobSis/zsh-completion-generator)  -此插件尝试从程序的帮助文本中读取选项列表，并自动生成完成功能. 请注意，这不会自动执行，您必须显式调用生成器以创建完成脚本.
...
* [directory-history](https://github.com/tymm/zsh-directory-history) -ZSH的每个目录历史记录，它以目录敏感的方式实现向前/向后导航以及子字符串搜索.
...
* [easy-motion](https://github.com/IngoHeimbach/zsh-easy-motion) -的港口 [vim-easymotion](https://github.com/easymotion/vim-easymotion) 对于ZSH.
* [ec2ssh](https://github.com/h3poteto/zsh-ec2ssh) -列出EC2实例和`ssh`登录到实例.
* [editing-workbench](https://github.com/psprint/zsh-editing-workbench) -增加了合理，复杂的命令行编辑（例如，增量历史记录_word_完成）.
...
* [emoji-fzf](https://github.com/pschmitt/emoji-fzf.zsh) -出色的可配置ZSH插件 [emoji-fzf](https://github.com/noahp/emoji-fzf) . 它受到了很大的启发 [emoji-cli](https://github.com/b4b4r07/emoji-cli).
* [emojis](https://github.com/MichaelAquilina/zsh-emojis) -在方便的变量中向您的环境添加许多ascii艺术表情符号.
* [enhancd](https://github.com/b4b4r07/enhancd) -一个简单的工具，通过记忆用户访问的所有目录并将其用于路径名解析，从而提供增强的cd命令.
...
* [explain-shell](https://github.com/gmatheu/shell-plugins) -在以下位置打开命令 [explainshell.com](https://explainshell.com).
...
* [flow-plugin](https://github.com/sandstorm/oh-my-zsh-flow-plugin) -这个插件使得TYPO3 Flow发行版的每个子目录中都可以使用`flow`命令.
...
* [fz](https://github.com/changyuheng/fz) -无缝地将模糊搜索添加到 [z](https://github.com/rupa/z)的标签页补全，可让您轻松地在历史记录中的目录之间跳转.
...
* [fzf-prezto](https://github.com/lildude/fzf-prezto) -在哪里找到Prezto插件 [fzf](https://github.com/junegunn/fzf) 已安装并启用其自动完成和键绑定.
...
* [gentoo](https://github.com/MattiaG-afk/gentoo-ohmyzsh) -添加了一些别名和功能，可用于Gentoo Linux.
...
* [git-acp](https://github.com/MenkeTechnologies/zsh-git-acp) -将当前命令行作为提交消息，然后使用一次击键运行git pull，添加，提交和推入.
...
* [git-it-on](https://github.com/peterhurford/git-it-on.zsh) -增加了在GitHub上当前分支中打开文件夹的功能.
...
* [git-sync](https://github.com/caarlos0/zsh-git-sync) -一个ZSH插件来同步git仓库并清理它们.
* [git](https://github.com/davidde/git)  -替换了普通的oh-my-zsh git插件. 提供了很多有用的别名和功能. 替换默认插件的动机是基于这样的事实，即它带有一些不一致的地方，这使得一些常用命令相当不直观，因此该插件使别名保持一致.
...
* [github-issues](https://github.com/zdharma/zsh-github-issues) -在Github上订阅项目，并在外壳中（在提示符下）接收有关新问题的通知.
...
* [gitstatus](https://github.com/Insert-Creative-Name-Here/gitstatus.zsh) -轻松在提示中显示您的`git`状态.
...
* [gpg-crypt](https://github.com/Czocher/gpg-crypt) -ZSH插件，用于加密和解密就位的文件或目录.
...
* [gvm (yerinle)](https://github.com/yerinle/zsh-gvm) -为gvm提供自动补全功能（Groovy环境管理器）.
* [hab](https://github.com/alexdesousa/hab) -更改到新目录时，如果发现文件.envrc中自动定义的OS环境变量，则会自动加载该环境变量.
...
* [history-filter](https://github.com/MichaelAquilina/zsh-history-filter)  -允许您指定模式，这些模式将自动将命令从插入到您的永久历史记录中排除. 对于防止写入机密信息特别有用.
...
* [homebrew](https://github.com/digitalraven/omz-homebrew) -插件 [homebrew](https://brew.sh) 补充了oh-my-zsh中内置的那个.
...
* [iterm2](https://github.com/laggardkernel/zsh-iterm2) -将iTerm的ZSH集成脚本打包到ZSH插件中，以避免污染您的$ HOME目录，而时间增加仅可忽略不计2ms.
* [jabba](https://github.com/2m/zsh-jabba) -为 [jabba](https://github.com/shyiko/jabba) Java版本管理器.
...
* [jenv-lazy](https://github.com/shihyuho/zsh-jenv-lazy) -用于延迟加载jEnv的ZSH插件.
...
* [last-working-dir-tmux](https://github.com/Curly-Mo/last-working-dir-tmux) -在每个tmux会话中全局跟踪上次使用的工作目录，并自动跳转到新的shell中.
...
* [lux](https://github.com/pndurette/zsh-lux)  -ZSH插件可通过“ lux”命令在macOS以及其他项目和应用程序的明暗模式之间切换. 高度可定制：可以通过定义变量来配置包含的项目. 高度可扩展：可以通过定义功能来添加项目. 包含`macos_is_dark`帮助函数，以确定主题中使用的macOS暗模式是否处于活动状态.
* [mac-packaging](https://github.com/Temikus/mac-packaging) -一套用于企业Mac打包的常用功能 [Munki](https://www.munki.org/munki/).
* [macos](https://github.com/joow/macos) -适用于macOS的ZSH插件.
* [mage2docker](https://github.com/lukaszolszewski/mage2docker) -使使用Docker和Magento 2变得容易.在容器上的Magento 2中，加速并简化了诸如清理缓存，设置升级，编译di等常用命令.
...
* [markgate](https://github.com/zakariaGatter/MarkGate) -允许您标记目录，以便您可以直接跳到目录.
...
* [mfunc](https://github.com/hlohm/mfunc)  -允许您即时定义持久性函数，而无需将其添加到配置文件中. 这些功能将永久可用，直到您将其删除.
...
* [navigation-tools](https://github.com/psprint/zsh-navigation-tools) -添加类似“ htop”的“ kill”，目录书签浏览器，多词增量历史记录搜索器等.
...
* [nodenv (c-uo)](https://github.com/C-uo/zsh-nodenv) -在您的工作目录中查找nodenv并在找到它时将其加载.
* [nodenv (jsahlen)](https://github.com/jsahlen/nodenv.plugin.zsh) -自动将nodenv及其补全加载到外壳中.
...
* [noreallyjustfuckingstopalready](https://github.com/eventi/noreallyjustfuckingstopalready) -macOS用户知道试图弄清楚实际上哪个命令会刷新其版本的macOS上的DNS缓存的麻烦，并且此插件使烦恼消失了.
...
* [notify (luismayta)](https://github.com/luismayta/zsh-notify) -具有自动安装依赖项和r2d2声音的ZSH通知.
* [notify (marzocchi)](https://github.com/marzocchi/zsh-notify) -适用于ZSH的插件（在macOS和Linux上），如果终端应用程序在后台（或命令终端）在命令以非零退出状态终止或花费超过30秒的时间才能完成时，它将发布桌面通知标签无效）.
...
* [nvm-auto](https://github.com/dijitalmunky/nvm-auto) -旨在尽可能减少键入nvm use的需求，尤其是如果您经常在node.js版本之间切换并在项目中使用`.nvmrc`文件来管理项目所需的节点版本时，尤其如此.
...
* [open-pr](https://github.com/caarlos0/zsh-open-pr) -一个ZSH插件，用于从命令行打开请求请求.
...
* [opt-path](https://github.com/jreese/zsh-opt-path) -自动将〜/ opt子路径添加到您的$ PATH中.
* [osx-autoproxy](https://github.com/SukkaW/zsh-osx-autoproxy) -根据macOS的系统偏好设置来配置代理环境变量.
* [osx-dev](https://github.com/marshallmick007/osx-dev-zsh-plugin) -该插件添加了一些命令，用于在macOS安装中维护各种服务器程序.
* [osx](https://github.com/mwilliammyers/plugin-osx) -添加一些与macOS相关的常见别名和功能.
* [p10k-promptconfig](https://github.com/doctormemes/p10k-promptconfig) -通过在.zshrc文件中定义`P10K_PROMPT`变量，增加了在Powerlevel10k提示主题配置文件之间轻松切换的功能.
...
* [plugin-rails](https://github.com/paraqles/zsh-plugin-rails) -用于Rails的ZSH插件.
...
* [pnpm](https://github.com/ntnyq/omz-plugin-pnpm) -为常用的Pnpm命令添加有用的别名.
...
* [rbenv (Meroje)](https://github.com/Meroje/zsh-rbenv) -灵感来自 [https://github.com/lukechilds/zsh-nvm/](https://github.com/lukechilds/zsh-nvm/)，使使用ruby rbenvs更加容易.
* [rbenv (jsahlen)](https://github.com/jsahlen/rbenv.plugin.zsh) -基于原始的oh-my-zsh rbenv插件的变体.
* [rc-files](https://github.com/0b10/rc-files) -添加了用于编辑各种rc文件的快捷方式功能.
...
* [reload](https://github.com/aubreypwd/zsh-plugin-reload) -新增功能可快速重新加载您的.zshrc.
...
* [robo](https://github.com/shengyou/robo-zsh-plugin) -的ZSH插件 [Robo](https://robo.li/).
...
* [safe-kubectl](https://github.com/benjefferies/safe-kubectl) -在运行`kubectl`时增加安全性，方法是在自上次`kubectl`命令以来可定义的秒数后警告您所处的上下文.
* [safe-paste](https://github.com/oz/safe-paste)  -一个安全粘贴的插件. 见康拉德·欧文（Conrad Irwin） [bracketed-paste](https://cirw.in/blog/bracketed-paste) 博客文章.
...
* [saneopt](https://github.com/willghatch/zsh-saneopt) -本着Vim明智的精神，Sane缺省使用ZSH选项.
* [schroot](https://github.com/fshp/schroot.plugin.zsh) -在提示中显示当前的“ chroot”名称.
* [search-directory-history](https://github.com/cmaahs/search-directory-history) -允许对使用目录创建的每个目录的历史记录进行复杂搜索 [per-directory-history](https://github.com/jimhester/per-directory-history) 插入.
...
* [send](https://github.com/robertzk/send.zsh) -单一命令git add，git commit以及git push，以更快地执行git工作流程.
...
* [simpleserver](https://github.com/sathish09/zsh_plugins/tree/master/simpleserver) -用于轻松启动python`SimpleHTTPServer`和`SimpleHTTPSServer`的插件.
* [skim](https://github.com/hackerchai/skim-zsh) -增加了对 [skim](https://github.com/lotabout/skim)
* [slugify](https://github.com/lashoun/slugify) -将文件名和目录转换为Web友好格式.
...
* [sshukh](https://github.com/anatolykopyl/sshukh-zsh-plugin) -当您将“ ssh”插入服务器时，将更新“ known_hosts”文件.
...
* [sudo](https://github.com/hcgraf/zsh-sudo)  -oh-my-zsh的`sudo`插件，提取为独立插件. 在emacs-mode或vi-command模式下按* ESC-ESC *可以在当前/上一个命令之前切换`sudo`.
* [suffix-alias](https://github.com/srijanshetty/zsh-suffix-alias) -使用ZSH的后缀别名直接在外壳中打开文件.
* [svn-n-zsh](https://github.com/khrt/svn-n-zsh-plugin) -重写股票oh-my-zsh svn插件.
* [switch-git](https://github.com/robin-mbg/switch-git)  -在git仓库之间轻松切换. 只需输入`sgr<some part of you repo's name>  `，按Enter键，您就在那里.
...
* [terminal-title](https://github.com/AnimiVulpis/zsh-terminal-title) -添加了一个“ set-term-title”功能，您可以使用该功能为终端窗口加标题.
...
* [terraform (jsporna)](https://github.com/jsporna/terraform-zsh-plugin)  -使用别名和制表符补全扩展了原始的oh-my-zsh插件. 添加工作空间（如果不是默认值）以进行提示.
...
* [terraform (thuandt)](https://github.com/thuandt/zsh-terraform) -为terraform添加便利别名，并为terraform和terragrunt添加完成.
...
* [thefuck](https://github.com/laggardkernel/thefuck) -负载 [thefuck](https://github.com/nvbn/thefuck) （用于纠正您先前命令的工具）具有缓存支持，可显着减少加载时间.
...
* [tmux-vim-integration](https://github.com/jsahlen/tmux-vim-integration.plugin.zsh) -在正在运行的`vim`（或NeoVim）会话中，从相邻的`tmux`窗格中打开文件.
...
* [toggle-command-prefix](https://github.com/xPMo/zsh-toggle-command-prefix)  -添加小部件以将前缀切换为命令. 绑定Alt + s以默认情况下以`sudo`前缀命令.
* [traista](https://github.com/odgon/traista)  -包含git状态修饰和上次命令运行的颜色编码退出状态. 使用深色终端主题效果更好.
...
* [up (peterhurford)](https://github.com/peterhurford/up.zsh) -将up命令添加到`cd`的多个级别上.
* [update-zsh](https://github.com/AndrewHaluza/zsh-update-plugin)  -更新自定义的oh-my-zsh插件. 仅适用于oh-my-zsh框架.
* [url-highlighter](https://github.com/ascii-soup/zsh-url-highlighter)  -ZSH语法荧光笔的插件，如果URL以“良好”状态响应，则将URL变为绿色，否则将其变为红色. 对于检查URL错字很有用.
* [uvenv](https://github.com/vincentto13/uvenv.plugin.zsh) -扩展了原始的oh-my-zsh venv模块的功能.
* [vagrant-box-wrapper](https://github.com/evanthegrayt/vagrant-box-wrapper) -的包装器插件 [vagrant](https://www.vagrantup.com/) 允许从box目录外部调用`vagrant`命令. 该插件还附带了一些额外的命令，这些命令可帮助管理多个框以及自定义制表符完成功能.
* [vanilli.sh](https://github.com/yous/vanilli.sh) -Shell配置的轻量级起点.
* [vapor](https://github.com/notf0und/zsh-vapor) -用于zsh的Laravel Vapor插件，可自动完成，可帮助您从项目树中的任何位置运行蒸气！
* [vcshr](https://github.com/aubreypwd/zsh-plugin-vcshr) -帮助vcsh用户需要使用vcsh来在Github仓库中自动安装~~ .zshrc等.
* [velocity](https://github.com/rahulsalvi/velocity-python) -基于Powerline的ZSH和 [tmux](https://tmux.github.io).
* [venv-lite](https://github.com/gimbo/venv-lite.zsh) -的超轻量级克隆 [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) ; 它几乎希望您正在使用 [pyenv](https://github.com/pyenv/pyenv) （尽管您没有*必须*），并且因为它基于 [`venv` module](https://docs.python.org/3/library/venv.html)，（creation）仅适用于python&gt; = 3.3.
...
* [vimman](https://github.com/yonchu/vimman) -在ZSH中查看`vim`插件手册（帮助），如`man`.
...
* [vox](https://github.com/andrewbonnington/vox.plugin.zsh) -oh-my-zsh插件来控制 [VOX](https://vox.rocks/)，这是一款适用于macOS的轻巧，功能齐全的音频播放器，可以播放多种格式，包括FLAC和Ogg Vorbis.
...
* [watch](https://github.com/enrico9034/zsh-watch-plugin) -通过按“ CTRL + W”，可以在watch前面轻松添加当前或以前的命令的前缀.
...
* [whobrokemycode](https://github.com/cameronbroe/whobrokemycode) -使用`git blame`突出显示文件中特定行最后更改的位置.
...
* [ugit](https://github.com/Bhupesh-V/ugit) -让您撤消上一次的git操作.
...
* [yapipenv](https://github.com/AnonGuy/yapipenv.zsh) -如果`pipenv`检测到目录的pip环境，则自动激活该目录的pip环境.
...
* [you-should-use](https://github.com/MichaelAquilina/zsh-you-should-use) -ZSH插件，可提醒您使用定义的别名.
...
* [z-a-man](https://github.com/zinit-zsh/z-a-man) - [Zinit](https://github.com/zdharma/zinit) 扩展，可为所有插件和代码片段生成手册页.
* [z-a-meta-plugins](https://github.com/zinit-zsh/z-a-meta-plugins) -安装带有单个标签的插件组（[Zinit](https://github.com/zdharma/zinit) 只要）.
* [z-a-patch-dl](https://github.com/zinit-zsh/z-a-patch-dl) - [Zinit](https://github.com/zdharma/zinit) 该扩展程序可下载文件并通过提供的`dl&#39;和`patch&#39;` ice应用补丁.
* [z-a-rust](https://github.com/zinit-zsh/z-a-rust) - [Zinit](https://github.com/zdharma/zinit) 该扩展程序可在插件目录中安装防锈和防潮包装.
* [z-a-submods](https://github.com/zinit-zsh/z-a-submods) - [Zinit](https://github.com/zdharma/zinit) 扩展允许在插件或代码片段内安装和管理其他子模块.
* [z-a-test](https://github.com/zinit-zsh/z-a-test) - [Zinit](https://github.com/zdharma/zinit) 在安装和更新插件或代码段后运行测试的扩展程序（例如，通过“ make test”进行测试）（如果找到的话）.
* [z-a-unscope](https://github.com/zinit-zsh/z-a-unscope) -允许为安装插件 [Zinit](https://github.com/zdharma/zinit) 无需通过查询Github API来指定用户名.
* [z.lua](https://github.com/skywind3000/z.lua)  -命令行工具，可帮助您通过学习习惯来更快地导航. 的替代品 [z.sh](https://github.com/rupa/z)  Windows和posix shell的支持和各种改进. 比fasd和autojump快10倍，比fasd和autojump快3倍 [z.sh](https://github.com/rupa/z).
* [zabb](https://github.com/Mellbourn/zabb) -`zabb`是一个命令，它试图找出可供以下用户使用的目录的最简短的记忆缩写： [z](https://github.com/ajeetdsouza/zoxide) 明确跳转到该目录.
...
* [zcolors](https://github.com/marlonrichert/zcolors) -使用您的$ LS_COLORS为Git和您的Zsh提示符，补全和 [ZSH syntax highlighting](https://github.com/zsh-users/zsh-syntax-highlighting).
...
* [zoxide](https://github.com/ajeetdsouza/zoxide) -快速替代cd的一种方法，它可以学习您的习惯.
...
* [zsnapac](https://github.com/johnramsden/zsh-zsnapac) -用于在Arch Linux上获取ZFS升级前/升级后快照的插件.
* [zsnapshot](https://github.com/psprint/zsnapshot) -添加了将当前ZSH状态转储到文件中的命令，以供以后通过获取快照文件进行还原时使用.
...
* [aliyun](https://github.com/thuandt/zsh-aliyun) -为 [Aliyun CLI](https://github.com/aliyun/aliyun-cli).
...
* [batect](https://github.com/batect/batect-zsh-completion/) -为添加标签补全 [batect](https://batect.dev/) 构建系统.
...
* [bw](https://github.com/CupricReki/zsh-bw-completion) -为完成添加 [Bitwarden](https://bitwarden.com/).
* [cabal-completion](https://github.com/ehamberg/zsh-cabal-completion) -为卡巴拉添加标签页补全.
* [cabal](https://github.com/d12frosted/cabal.plugin.zsh) -为cabal添加了自动补全功能.
* [carapace](https://github.com/rsteube/carapace)  -用于Bash，精灵，鱼，油，Powershell，Xonsh和ZSH的完成生成器. 注意-不是动态的，您必须显式运行它以生成命令的补全.
* [cargo](https://github.com/MenkeTechnologies/zsh-cargo-completion) -原始OMZ货物装箱的所有功能，并通过`cargo add`中的`cargo search`对远程箱子提供了额外支持.
...
* [cod](https://github.com/dim-an/cod) -bash / fish / ZSH的完成恶魔，当看到您使用--help运行某些内容时，它会即时创建完成功能.
* [codeception](https://github.com/shengyou/codeception-zsh-plugin) -为“代码接受测试框架”添加命令完成.
* [codemachine](https://github.com/CodeMonkeyMike/ZshTheme-CodeMachine) -显示git信息，无论您是否通过`ssh`登录，返回上一条命令的代码.
* [comonicon](https://github.com/Roger-luo/ComoniconZSHCompletion.jl) -的制表符补全 [comonicon](https://github.com/Roger-luo/Comonicon.jl).
* [completions](https://github.com/zsh-users/zsh-completions) -ZSH额外完成的集合.
* [conda](https://github.com/esc/conda-zsh-completion) -Conda的ZSH标签完成.
...
* [docker (chr-fritz)](https://github.com/chr-fritz/docker-completion.zshplugin) -直接从Docker for Mac **加载docker ZSH选项卡完成.
...
* [docker-enter-completion](https://github.com/primait/docker-enter-completion) -的命令完成 [docker-enter](https://github.com/jpetazzo/nsenter).
* [dotnet](https://github.com/MenkeTechnologies/zsh-dotnet-completion) -Dotnet选项卡完成.
* [dropbox](https://github.com/zpm-zsh/dropbox) -Zsh的Dropbox插件，提供`dropbox-cli`和`dropbox-uploader`命令.
...
* [extract (le0me55i)](https://github.com/le0me55i/zsh-extract) -定义一个称为提取的函数，该函数提取您传递给它的存档文件，并支持多种存档文件类型.
...
* [fzf-gcloud](https://github.com/mbhynes/fzf-gcloud) -模糊完成以导航和预览所有Google Cloud SDK`gcloud` CLI命令
* [fzf-tab-completion](https://github.com/lincheney/fzf-tab-completion) -使用GNU Readline为ZSH，bash和应用程序添加制表符补全.
...
* [gcloud](https://github.com/littleq0903/gcloud-zsh-completion) -添加Google Cloud SDK的完成内容.
* [gentoo](https://github.com/gentoo/gentoo-zsh-completions) -为上游缺少完成脚本的各种Gentoo工具提供ZSH完成支持.
* [git-annex](https://github.com/Schnouki/git-annex-zsh-completion) -允许大多数git-annex命令的制表符完成.
...
* [grid5000](https://github.com/pmorillon/grid5000-zsh-plugin) -Grid 5000插件-添加主题，自动填充.
* [gulp (akoenig)](https://github.com/akoenig/gulp.plugin.zsh) -在Z-Shell（ZSH）中自动完成gulp.js任务.
* [gulp (srijanshetty)](https://github.com/srijanshetty/gulp-autocompletion-zsh) -自动完成的大口吃.
...
* [helmfile](https://github.com/Downager/zsh-helmfile) -为`helm`添加自动补全功能.
...
* [joe](https://github.com/corvofeng/joe-completion) -为完成添加 [joe](https://github.com/karan/joe) gitignore编辑器.
...
* [msfvenom](https://github.com/Green-m/msfvenom-zsh-completion) -Metasploit的制表符补全.
...
* [newman](https://github.com/selop/newman-autocomplete) -为 [Newman CLI](https://github.com/postmanlabs/newman).
...
* [nx](https://github.com/jscutlery/nx-completion)  -完成nx. 需要 [`jq`](https://stedolan.github.io/jq/).
...
* [quickjump](https://github.com/fikovnik/zsh-quickjump) -添加了标签补全支持 [skim](https://github.com/lotabout/skim) 用于最近使用的文件和目录 [fasd](https://github.com/whjvenyl/fasd).
...
* [s3cmd](https://github.com/FFKL/s3cmd-zsh-plugin) -为添加标签补全 [s3cmd](https://s3tools.org/s3cmd).
...
* [surf](https://github.com/beardcoder/surf.plugin.zsh) -添加完成的冲浪.
* [symphony (TheGrowingPlant)](https://github.com/TheGrowingPlant/symfony.plugin.zsh) -Symfony 3和4命令的自动补全功能.
...
* [tinygo](https://github.com/sago35/tinygo-autocmpl) -为tinygo添加制表符补全.
...
* [yabai](https://github.com/Amar1729/yabai-zsh-completions) -为macOS添加补全 [yabai](https://github.com/koekeishiya/yabai/) 平铺窗口管理器.
...
* [000](https://github.com/Abid-Ahmad/oh-my-zsh-000-theme) -多行提示，带有用户名，主机名，完整路径，返回状态和git装饰.
* [0i0](https://github.com/0i0/0i0.zsh-theme) -针对深色终端窗口进行了优化，使用nerdfont`git`状态装饰.
* [14degree](https://github.com/saims0n/14degree-zsh-theme/) -包括git，virtualenv和rvm状态装饰.
...
* [af-magic-dynamic](https://github.com/rslavin/af-magic-dynamic) -的修改版本 [af-magic](https://github.com/andyfleming/oh-my-zsh/blob/master/themes/af-magic.zsh-theme) 动态路径缩短.
...
* [agitnoster](https://github.com/dbestevez/agitnoster-theme) - 基于 [agnoster](https://gist.github.com/3712874) 包含在主题中 [Oh My Zsh](https://github.com/ohmyzsh/ohmyzsh) 和 [bash-git-prompt](https://github.com/magicmonty/bash-git-prompt) . 显示有关`git`状态的详细信息.
...
* [agnoster-gentoo](https://github.com/r7l/agnoster-gentoo-zsh-theme) -的Gentoo风味版本 [Agnoster ZSH Theme](https://github.com/agnoster/agnoster-zsh-theme) 包括user @ hostname和`git`状态修饰. 使用unicode字体效果更好.
...
* [alpharized](https://github.com/NicoSantangelo/Alpharized) -经过优化可与 [solarized](http://ethanschoonover.com/solarized) 黑暗的终端. 这是 [avit theme](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/avit.zsh-theme).
...
* [antsy](https://github.com/jeffmhubbard/antsy-zsh-theme) -显示`git`分支和状态修饰，virtualenv，退出状态，作业计数和vi-mode指示器.
...
* [arael](https://github.com/aknackd/zsh-themes) -的叉子 [gallifrey](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/gallifrey.zsh-theme).
...
* [aronhoyer](https://github.com/aronhoyer/zsh-theme) -带有右侧git状态装饰的简约主题.
...
* [bastard](https://github.com/jsundqvist/bastard.zsh-theme) -的修改版本 [gitster](https://github.com/zimfw/gitster) 的主题 [ZIM](https://github.com/zimfw/zimfw).
* [bearable](https://github.com/JanmanX/bearable-zsh) -适用于深色终端背景.
* [bedbugs](https://github.com/justino/zsh-theme-bedbugs) -灵感来自 [Agnoster](https://gist.github.com/agnoster/3712874)，此多行提示包括`git`状态信息，后台作业计数，工作目录，用户和主机名，Python virtualenv（如果存在），最后一个命令的彩色返回值以及root / user sigil.
...
* [birav2](https://github.com/shahid64/birav2-theme) - 基于 [bira](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/bira.zsh-theme) . 包括git，rvm和virtualenv状态装饰.
* [bklyn](https://github.com/gporrata/bklyn-zsh) -的变体 [Powerlevel9k](https://github.com/bhilburn/powerlevel9k) 与应用自定义.
* [black-Void](https://github.com/black7375/BlaCk-Void-Zsh) -包括帐户信息，root用户，使用ssh，目录批处理，写入权限，vcs信息修饰.
...
* [blinks (max13ft)](https://github.com/max13fr/blinks.zsh-theme) -增加了对oh-my-zsh的支持 [blink](https://github.com/max13fr/blinks.zsh-theme) 主题.
...
* [bogo](https://github.com/cubasepp/zsh-bogo-theme) -灵感来自 [zeta](https://github.com/skylerlee/zeta-zsh-theme) . 包括git和ruby版本的装饰.
...
* [bubblegum](https://github.com/ice-bear-forever/bubblegum-zsh) -极简主义的亮粉红色主题，带有一个三角形标志符号和您的工作目录，仅此而已-为您提供了最干净的外壳.
* [bubblified (hohmannr)](https://github.com/hohmannr/bubblified) -灵感来自 [agnoster](https://github.com/agnoster/agnoster-zsh-theme) . 最适合 [nerdfonts](https://github.com/ryanoasis/nerd-fonts).
...
* [bureau-env](https://github.com/angus-lherrou/bureau-env) -Oh-My-Zsh的修改 [Bureau](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/bureau.zsh-theme) 主题，在git块的左侧添加了一个Python虚拟环境标签.
...
* [celestialorb](https://github.com/celestialorb/zsh-theme)  -@celestialorb创作的以电力线为主题的主题. 包括git状态装饰，Kubernetes集群信息（如果有），当前的AWS配置文件和区域以及活动的virtualenv.
...
* [charged](https://github.com/robwierzbowski/charged-zsh-theme) -针对以下内容进行了优化的ZSH提示： [solarized](https://github.com/altercation/solarized) 黑暗的终端主题.
...
* [chrisandrew.cl](https://github.com/chrisandrewcl/chrisandrew.cl.zsh-theme)  -包括git装饰. 需要电力线兼容的终端字体.
...
* [clean (patr1ot)](https://github.com/Patr1ot/clean.zsh-theme) -上游叉 [clean](https://github.com/ohmyzsh/ohmyzsh/wiki/Themes#clean) 加上主机信息.
* [cleansh](https://github.com/diegoos/cleansh)  -极简主义，包括`git`，Ruby，node和Python版本状态修饰. 使用标准字体.
...
* [cmder](https://github.com/potasiyam/cmder-zsh-theme) -与Cmder主题相匹配的ZSH主题，Cmder是Windows的流行终端仿真器.
...
* [collon](https://github.com/lambdalisue/collon.zsh)  -具有`git`状态装饰，cwd，时间，主机，上一个命令的退出状态的轻量级主题. 不需要特殊字体.
...
* [crème fraîche](https://github.com/koenwoortman/creme-fraiche-zsh-theme) -最适合在浅色终端背景下使用，包括“ git”和“ vi”模式的状态装饰.
...
* [daily](https://github.com/ghlin/zsh-theme-daily) -包括git和ssh状态装饰.
...
* [darkblood-modular](https://github.com/InAnimaTe/darkblood-modular) -此版本受欢迎 [darkblood](https://github.com/BinaryMuse/oh-my-zsh/blob/binarymuse/themes/darkblood.zsh-theme) 主题已通过几乎完全重写的模块化和一些新功能得到了增强.
...
* [dtheme](https://github.com/OlukaDenis/DTheme)  -针对使用日晒终端配色方案和git的用户进行了优化. 与unicode字体配合使用效果最佳.
* [duckster](https://github.com/ducky/duckster) -Gitster ZSH主题的叉子，更新鲜.
* [ducula](https://github.com/janjoswig/Ducula)  -受德古拉（Dracula）项目的启发. 包括git状态修饰，用户名和主机名缩写，虚拟环境，当前工作目录，最后命令的返回状态和时间.
...
* [eckig](https://github.com/fouladi/eckig)  -带有utf-8图标的简约主题. 包括`git`状态装饰和时钟.
* [eggshausted](https://github.com/inutano/eggshausted) -适用于厌倦了获取错误的人们的`git`意识主题.
...
* [emoji](https://github.com/masaakifuruki/emoji.zsh-theme) - 基于  [robbyrussell](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/robbyrussell.zsh-theme) 带有git提示符号的oh-my-zsh主题用表情符号替换，以提高清晰度.
* [endless-dog](https://github.com/qwelyt/endless-dog) -模仿grml-zsh-config的oh-my-zsh兼容主题.
...
* [eucalyptus](https://github.com/relastle/eucalyptus) -简单的一线主题，为极简主义的Vi-mode用户带来灵感 [agnoster](https://github.com/agnoster/agnoster-zsh-theme) 和 [powerlevel9k](https://github.com/bhilburn/powerlevel9k). Includes `git` status indicator, `vi`-mode indicator, current directory 和 current path.
...
* [fluent-git](https://github.com/RobertKozak/fluent-git) -显示上一次执行命令的时间，错误代码，主机名，用户名，`git`状态，kubernetes集群和名称空间，路径和ssh连接状态.
* [forerunner](https://github.com/OpenReplyDE/zsh-forerunner) -的自定义设置 [powerlevel9k](https://github.com/bhilburn/powerlevel9k) . 包括`git`状态装饰.
...
* [friendly-fiesta](https://github.com/bruino/friendly-fiesta) -的叉子 [terminal-party](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/terminalparty.zsh-theme) 主题.
...
* [frlo](https://github.com/fiorillo/frlo) -使用计算机的主机名提出一个（希望）唯一的三色主题以显示在提示中，因此您一眼就能知道您登录的是哪台计算机.
...
* [furry-umbrella](https://github.com/kb10uy/zsh-theme-furry-umbrella) -色彩丰富的主题，在深色背景上效果更好.
...
* [garrett](https://github.com/chauncey-garrett/zsh-prompt-garrett) -Prezto会在需要时提示您所需的信息.
* [gawaine](https://github.com/nicolaracco/gawaine.zsh-theme)  -尼古拉·拉科（Nicola Racco）的主题. 需要`rvm`和`git`插件.
...
* [geometry](https://github.com/geometry-zsh/geometry) -最小的ZSH主题，可以在运行中将任何功能添加到左侧提示或（异步）右侧提示.
* [ghoti](https://github.com/lonr/ghoti)  -模仿“鱼壳”默认提示. 包括git装饰.
* [gideon](https://github.com/userhiren/oh-my-zsh-gideon-theme) -灵感来自 [avit](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/avit.zsh-theme)，包括`git`装饰，IP地址，主机和路径.
* [gimbo](https://github.com/gimbo/gimbo.zsh-theme) -的变体 [purepower](https://github.com/romkatv/dotfiles-public/blob/master/.purepower) 具有更多功能，一点点吸引人的内容和上下文相关的额外线条. 包括git状态修饰，历史记录编号，用户名/主机名上下文，目录状态，最后一个命令（如果失败）的状态以及Python virtualenv名称（如果存在）.
...
* [gitster (zimfw)](https://github.com/zimfw/gitster) -shashankmehta的Zim叉子 [gitster](https://github.com/shashankmehta/dotfiles/blob/master/thesetup/zsh/.oh-my-zsh/custom/themes/gitster.zsh-theme) 提示主题
...
* [gnrnzh](https://github.com/PaoloneM/gnrnzh-zsh-theme) -定制 [gnzh.zsh-theme](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/gnzh.zsh-theme) 来自oh-my-zsh.
...
* [handy](https://github.com/HanleyLee/Handy) -带有`git`装饰的轻巧提示.
...
* [hematite](https://github.com/bigdave/hematite) -极简主义促销，尝试仅显示在给定时间有效的状态修饰.
...
* [hhktony](https://github.com/hhktony/hhktony.zsh-theme) -受到robbyrussell主题+ ssh连接状态提示的启发.
...
* [humbled](https://github.com/saravanabalagi/zsh-theme-humbled) -一个干净而谦虚的主题，支持condaenv，virtualenv和git状态装饰，使其与工作目录保持粗体对齐.
* [hyper](https://github.com/willmendesneto/hyper-oh-my-zsh) -旨在与超级终端主题配合使用，包括`git`状态装饰.
...
* [illuvia-gitster](https://github.com/lopezator/lluvia-gitster) -的叉子 [ergenekonyigit/lambda-gitster](https://github.com/ergenekonyigit/lambda-gitster) 改进了间距并更新了图标. 包含`git`状态信息.
...
* [jc](https://github.com/jclementex/jc-zsh-theme) -对于黑暗的终端背景，请包含`git`状态信息.
* [jcl](https://github.com/jasonlewis/jcl-zsh-theme) -宽松地基于ys主题.
* [jerome](https://github.com/jeromescuggs/jerome-theme) -基于的多彩主题 [dieter](https://github.com/jeromescuggs/jerome-theme) 主题，但具有黄色主机名. 包括git装饰.
...
* [jose](https://github.com/tmjoseantonio/shrug-zsh-theme) -灵感来自 [beer-theme](https://github.com/tcnksm/oh-my-zsh-beer-theme)，包含git状态.
* [jovial](https://github.com/zthxxx/jovial) -显示主机，用户，路径，开发环境，`git`分支，其中python venv是活动的.
* [jpegleg](https://github.com/jpegleg/zshrc) -与黑血主题相似，包括时间戳和git装饰.
* [judgedim](https://github.com/judgedim/oh-my-zsh-judgedim-theme) -极简提示.
* [just-another](https://github.com/supertassu/another-theme) -只是另一个主题，当您被丢到另一台机器上时，其主机名为.
...
* [kido](https://github.com/KidoThunder/kido-zsh-theme)  -基于“ ys”和“ robbyrussell”主题. 包括用于最后运行命令的退出代码，python virtualenv和VCS状态的修饰符.
...
* [kumavis](https://github.com/kumavis/kumavis-zsh-theme)  -优化的Agnoster前叉，适用于日晒终端. 需要电力线兼容的字体.
...
* [lambda-blazinggit](https://github.com/zalefin/lambda-blazinggit)  -包括快速，详细的git信息. 需要书呆子字体和 [gitstatus](https://github.com/romkatv/gitstatus) 插入.
...
* [lambda-mod](https://github.com/halfo/lambda-mod-zsh-theme) -一个简单的ZSH主题，针对git用法进行了优化.
* [lambda-pure](https://github.com/marszall87/lambda-pure) -基于Pure的最小ZSH主题，并添加了NodeJS版本.
* [lambda-v](https://github.com/vkaracic/lambdav-zsh-theme) -Lambda和Fishy主题的组合，包括`git`状态装饰.
* [lambda-zen](https://github.com/seamile/lambda-zen) -受启发 [lambda mod theme](https://github.com/halfo/lambda-mod-zsh-theme) 图形化的git状态修饰.
* [lambder](https://github.com/avillen/zsh-theme-lambder) -包含git状态装饰，最适合深色终端主题.
...
* [lighthaus](https://github.com/lighthaus-theme/zsh) -补充您的提示 [Lighthaus](https://github.com/lighthaus-theme/lighthaus) 主题. 显示`git`信息，github / gitlab徽标，并在发生更改时显示更改.
...
* [limpide](https://github.com/shooteram/limpide) -的修改版本 [miloshadzic](https://github.com/ohmyzsh/ohmyzsh/wiki/themes#miloshadzic) 显示父目录和当前目录的主题.
* [linuxer](https://github.com/patrick330602/linuxer) -受到Yaris Alex Gutierrez的启发 [classyTouch](https://github.com/yarisgutierrez/classyTouch_oh-my-zsh)，Yad Smood的y和 [Bureau](https://github.com/isqua/bureau) 主题.
* [liquidprompt](https://github.com/nojhan/liquidprompt)  -功能齐全且经过精心设计的自适应提示，在您需要时会提供有用的信息. 它向您显示您在需要时需要的东西. 您会注意到变化时会发生什么变化，从而节省了时间和沮丧感.
...
* [lukerandall-extended](https://github.com/mpyw/oh-my-zsh-lukerandall-extended) -的扩展版本 [lukerandall](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/lukerandall.zsh-theme) 主题. 包括用于装饰git状态和上一次运行命令的状态.
...
* [magicmace](https://github.com/zimfw/magicmace) -灵感来自 [xero's zsh prompt](http://code.xero.nu/dotfiles) 和 [eriner's prompt](https://github.com/zimfw/eriner). Includes status codes for active python `venv`, exit status of last comm和, shortened working directory, `git` status decorations.
...
* [mdmini](https://github.com/MarioDena/MDmini) -包括git和ssh状态装饰.
...
* [mh-fzj](https://github.com/mh-firouzjaah/mh-fzj-oh-my-zsh-theme-v1) -包括rvm和git状态装饰.
* [michaelpass](https://github.com/michaelpass/michaelpass.zsh-theme) -POSIX友好的跨平台 [alanpeabody](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/alanpeabody.zsh-theme) 带有方便的时间戳和全面的git / ruby​​支持的mod.
...
* [mindful-space](https://github.com/syndbg/mindful-space-zsh-theme) -ZSH主题，充分考虑了空间.
...
* [minimal (5amu)](https://github.com/5amu/minimal-prompt)  -最小提示，使用nerdfonts. 包括git和vpn状态装饰.
* [minimal (casalinovalerio)](https://github.com/casalinovalerio/minimal-prompt) -最小提示，包括`git`和vpn状态修饰.
* [minimal (glsorre)](https://github.com/glsorre/minimal/) -最小化的异步ZSH主题已优化，可与 [Fira Code](https://github.com/tonsky/FiraCode) 字体和 [Solarized Light](https://ethanschoonover.com/solarized) 终端主题.
...
* [miramare](https://github.com/franbach/oh-my-deepin-miramare)  -包括`git`状态装饰. 最适合 [Deepin Terminal](https://www.deepin.org/en/original/deepin-terminal/).
...
* [modesty](https://github.com/saravanabalagi/zsh-theme-modesty) -一个干净而谦虚的zsh主题，带有“ condaenv”，“ virtualenv”和“ git”状态装饰，排列整齐，正确对齐.
...
* [moux](https://github.com/gagbo/moux) -在黑暗的终端背景下效果很好，包括在RPROMPT中的git装饰.
* [multi-shell-repo-prompt](https://github.com/dotcode/multi-shell-repo-prompt) -提供有关您所在存储库的信息（在您的提示下）.当前适用于 [Git](https://git-scm.com/) 和 [Mercurial](https://www.mercurial-scm.org/)， 在下面 [ZSH](https://en.wikipedia.org/wiki/Zsh) 也 [bash](https://en.wikipedia.org/wiki/Bash_%28Unix_shell%29).
...
* [nctu](https://github.com/leovincentseles/nctu.zsh-theme)  -轻巧的主题，着重速度. 包括`git`状态装饰.
...
* [newt](https://github.com/softmoth/zsh-prompt-newt)  -胖与快的主题–由内而外的精美外观，正确完成的样式段. 可自定义的，包括`git`，用户名，执行时间，目录，后台作业和编辑模式修饰.
...
* [nikitakot](https://github.com/nikitakot/nikitakot-oh-my-zsh-theme)  -小而简单的oh-my-zsh主题. 显示当前目录和后面的2个目录，分别是git和nodejs状态修饰.
...
* [omuse](https://github.com/ouuan/omuse-zsh-theme) -基于Oh-My-ZSH [amuse](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/amuse.zsh-theme) . 装饰有git状态，时间，绝对pwd，RAM使用率，上一条命令所用的时间以及上一条命令退出状态.
...
* [p9k-theme-pastel](https://github.com/iboyperson/p9k-theme-pastel) -主题为 [powerlevel10k](https://github.com/romkatv/powerlevel10k) 提示，重点放在简单性上，同时仍然获得重要信息.
...
* [pastel](https://github.com/iboyperson/pastel) -灵感来自ZSH的主题 [sugar-free](https://github.com/cbrock/sugar-free) . 包括git装饰.
...
* [powerlevel10k](https://github.com/romkatv/powerlevel10k) -快速重新实现 [powerlevel9k](https://github.com/bhilburn/powerlevel9k)  ZSH主题. 可以用作powerlevel9k的直接替代品，如果提供相同的配置选项，它将生成相同的提示，但速度更快.
...
* [powerlevelHipstersmoothie](https://github.com/hipstersmoothie/PowerlevelHipstersmoothie) -的附加组件 [powerlevel9k](https://github.com/bhilburn/powerlevel9k).
* [powerline (brucehsu)](https://github.com/brucehsu/oh-my-zsh-powerline-theme) -电力线的两行版本：一条用于提供信息，一条用于输入.
* [powerline (jeremy)](https://github.com/jeremyFreeAgent/oh-my-zsh-powerline-theme)  -关于电力线主题的另一种观点. 可以很好地配置，但是至少需要256种具有颜色的终端以及与电力线兼容的终端字体.
...
* [powerline-go](https://github.com/justjanne/powerline-go)  -使用golang编写的美观而有用的低延迟提示. 包括`git`和`hg`状态修饰符，上一次运行命令的退出状态，当前Python virtualenv，无论您是否处于 [nix](https://nixos.org/) 外壳，并且易于扩展.
...
* [predawn-shell](https://github.com/jamiewilson/predawn-shell) -针对黑暗终端主题进​​行了优化的主题.
...
* [punctual](https://github.com/dannynimmo/punctual-zsh-theme) -易于定制，受以下因素影响 [spaceship](https://github.com/denysdovhan/spaceship-prompt).
* [pure-agnoster](https://github.com/yourfin/pure-agnoster)  -纯粹和无知的混搭. 具有`git`装饰，可在深色和浅色终端背景下正常使用.
* [pure](https://github.com/sindresorhus/pure)  -漂亮，最小和快速的ZSH提示. 包括git状态修饰，如果最后一个命令失败，则提示变为红色；位于远程会话或容器中时，用户名和主机修饰，以及进程正在运行时，当前文件夹和命令.
...
* [qi3ber2](https://github.com/nichus/qi3ber2)  -黑暗的多行主题. 包括`git`，平均负载和最后一个命令修饰符的退出代码.
* [qoomon](https://github.com/qoomon/zsh-theme-qoomon)  -针对深色背景进行了优化，包括`git`信息. 主题回购包括iTerm 2和终端颜色设置.
* [quewui](https://github.com/kauefontes/oh-my-quewui)  -针对深色终端主题进​​行了优化的简单主题. 包括当前时间，用户，目录和git状态的状态修饰.
* [r3nic1e](https://github.com/r3nic1e/r3nic1e) - [Agnoster](https://github.com/agnoster/agnoster-zsh-theme) 带有电池状态，git / hg状态，时间，kubernetes上下文和名称空间，最后一个命令的非零退出代码和日期修饰的变体. 需要电力线字体.
...
* [raspberrysh](https://github.com/MaxMalinowski/raspberrysh) -包括git，python，时间，当前主机和路径装饰.
* [raytek](https://github.com/Raytek/raytek-zsh-theme) -简单多彩的主题，带有git状态装饰.
* [rb](https://github.com/rberenguel/rb-zsh-theme) -基于电力线的ZSH主题，基于 [Agnoster](https://gist.github.com/agnoster/3712874) ，针对git和日晒终端机进行了优化. 需要兼容Powerline的字体.
...
* [robbyrussell-WIP](https://github.com/ecbrodie/robbyrussell-WIP-theme) -用输出装饰`robbyrussell`主题，以指示** WIP **提交.
...
* [rocket](https://github.com/Alexandresl/rocket-zsh-theme) -简约主题，包括git和hg状态修饰.
...
* [rummik](https://github.com/rummik/zsh-theme)  -@rummik的主题. 技术支持 [psmin](https://gitlab.com/zick.kim/zsh/zsh-psmin)，以及提示中的git状态信息.
...
* [s1ck94](https://github.com/zimfw/s1ck94)  -S1cK94的（最早淘汰，现在已灭绝）的最小提示叉. 显示用户是否为root，后台作业状态，vi模式，上一个命令的退出状态以及git状态修饰.
...
* [seeker](https://github.com/tonyseek/oh-my-zsh-seeker-theme) -此主题使用许多特殊的unicode字符看中，但如果没有良好支持的字体，则可能会引起一些问题.
...
* [seppuku](https://github.com/Helianthella/seppuku) -干净而最小的主题灵感来自 [cloud](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/cloud.zsh-theme).
* [sepshell](https://github.com/sepehr/sepshell) -基于旧的丢失的taybalt主题的简洁，最小的ZSH主题，带有`git`对分/合并/变基模式和可配置的提示符号.
* [seti_UX](https://github.com/ginfuru/iTerm-Seti_UX) -一个简单的与omz兼容的主题，并带有相应的iTerm 2配色方案.
...
* [shini](https://github.com/bashelled/shini)  -一个很小的主题，只会大声喊叫. 包括目录，用户名，主机名，时间和git装饰.
...
* [simple (pavdmyt)](https://github.com/pavdmyt/simple-oh-my-zsh-theme) -基于的极简主义主题 [robbyrussel](https://github.com/ohmyzsh/ohmyzsh/wiki/Themes#robbyrussell) 将git状态信息嵌入iTerm的窗口标题栏中，而不是在提示中使用空格.
* [simple (savecoders)](https://github.com/Savecoders/simpleTheme-zsh-theme) -简单而极简的主题，带有git，用户名和执行状态修饰.
...
* [sit](https://github.com/svensen/sit.zsh-theme) -带有`git`，命令退出状态和路径修饰的简约主题.
...
* [sobole](https://github.com/sobolevn/sobole-zsh-theme)  -简约ZSH主题，灵感来自老式的爱好. 没有冗长的头，没有表情符号，没有烦躁的微调，也没有其他视觉噪音. 同时具有亮和暗模式.
...
* [solarizsh](https://github.com/paddykontschak/Solarizsh) -用于robbyrussell的oh-my-zsh主题的颜色修复 [solarized](https://github.com/altercation/solarized) 终端.
* [spaceship](https://github.com/denysdovhan/spaceship-prompt) -具有`git`，`nvm`，rvm / rbenv / chruby，python，`ssh`和其他有用状态指示符的主题.
* [spowerline](https://mbauhardt.github.io/spowerline/) -由斯卡拉撰写，灵感来自 [Agnoster](https://github.com/agnoster/agnoster-zsh-theme), [tmux](https://tmux.github.io) 电源线，vim电源线和vim状态插件.
...
* [sublime](https://github.com/pjmp/sublime) -带有`git`状态装饰的崇高，干净，简约的ZSH主题.
...
* [sulfurium](https://github.com/Sulfurium/zsh-theme) -SulphuriumOS的官方ZSH主题.
...
* [type0](https://github.com/MikereDD/type0_zsh-theme) -灵感来自 [classyTouch](https://github.com/yarisgutierrez/classyTouch_oh-my-zsh) 由yarisgutierrez. 包括git装饰.
* [typewritten](https://github.com/reobin/typewritten)  -最小且内容丰富的主题，为重要内容留有余地. 异步`git`装饰是否更新速度.
* [ubunly](https://github.com/alejandromume/ubunly-zsh-theme)  -模仿Kali Linux控制台. 注意-该主题还重新绑定了许多键，并设置了一些ZSH选项，主题可能应该单独使用.
...
* [vitesse](https://github.com/rafaeldellaquila/zsh-vitesse-theme/blob/master/img/preview.png) -受VS Code启发 [Vitesse](https://github.com/antfu/vscode-theme-vitesse) 主题. 包括`git`状态装饰.
* [vulcan](https://github.com/Bruceboy/vulcan-zsh-theme)  -最小的主题让人联想到默认的`bash`主题. 包括git装饰.
...
* [xris47](https://github.com/ivan-ristovic/xris47.zsh-theme)  -快速，简单，精简的主题. 最适合 [tmux](https://github.com/tmux/tmux/wiki) 和 [vim-airline](https://github.com/vim-airline/vim-airline).
...
* [yazpt](https://github.com/jakshin/yazpt) -一个干净，快速，美观的ZSH提示主题，该主题周到地包含了Git / Subversion / TFVC状态信息，并与诸如Oh My Zsh之类的流行插件管理器集成在一起，并且易于自定义和扩展.
...
* [yuki](https://github.com/yuki-torii/yuki-zsh-theme) -深色优化的ZSH主题.
...
* [zshpower](https://github.com/snakypy/zshpower)  -针对python开发人员进行了优化. 包括git和pyenv状态修饰，用户名和主机. 尝试安装其他插件和字体，因此请在安装前阅读其说明.
...
* [zys](https://github.com/ZYSzys/zys-zsh-theme) - 如同 [Agnoster](https://github.com/agnoster/agnoster-zsh-theme)，旨在通过电力线美学在上下文中公开信息.
...
* [Iosevka](https://github.com/be5invis/Iosevka)  -编码员的字样，由代码构建. 高度可定制的.
...
这些插件中的大多数都可以通过在.zshrc文件中添加`antigen bundle githubuser / reponame`来安装.  Antigen将在您下次启动`zsh`时自动为您处理克隆插件. 您也可以在将其添加到.zshrc中之前，使用“ antigen bundle githubuser / reponame”将其添加到正在运行的ZSH中进行测试.
...
### [Zgen](https://github.com/tarjoilija/zgen)
 ...
 Zgen没有得到积极维护. 考虑切换到 [Zgenom](https://github.com/jandamm/zgenom) fork, which is.

这些插件中的大多数都可以通过在您的.zshrc文件中添加`zgen load githubuser / reponame`来安装，其功能与您执行其他`zgen load`调用时相同.

当您执行“ zgen保存”时，Zgen将自动为您克隆插件存储库.

### [Zgenom](https://github.com/jandamm/zgenom)

这些插件中的大多数都可以通过将zgenom load githubuser / reponame添加到您的.zshrc文件中来进行安装，其功能与您执行其他zgenom load调用时所使用的功能相同.

当您执行“ zgenom保存”时，Zgenom将自动为您克隆插件存储库.
...
* [awesome-devenv](https://github.com/jondot/awesome-devenv) -精选的工具，资源和工作流程技巧精选清单，构成了很棒的开发环境
* [awesome-sysadmin](https://github.com/n1trux/awesome-sysadmin) -精选的优秀开源sysadmin资源列表
